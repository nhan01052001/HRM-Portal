...
export default class VnrRenderList extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            //valueField: "ID",
    ...
    _renderFooterLoading = () => {
        return (
            <View
                style={stylesScreenDetailV3.loadingFooter}
            >
    ...
    renderDataOfGroup = (data, isGroup, indexInDataSource) => {
        const { dataSource, isPullToRefresh, isOpenAction, isDisableSelectItem } = this.state,
            { detail, rowActions, renderConfig, componentDynamic } = this.props;

        return (
            <ScrollView>
                {data.map((value, index) => {
                    if (componentDynamic?.props)
                        componentDynamic.props.value.props = {
    ...
    render() {
        const {
            ...
            { api, detail, rowActions, onCreate, scrollYAnimatedValue, renderConfig, componentDynamic,
                isShowBtnCreate,
                keyPermissions } = this.props,
            permissionBtnCreate =
                PermissionForAppMobile && keyPermissions &&
                    PermissionForAppMobile.value[keyPermissions] &&
                    PermissionForAppMobile.value[keyPermissions]['Create']
                    ? true
                    : false;
        ...
        if (isLoading) {
            let typeLoading =
                detail.screenName.includes('Approve')
            ...
            contentList = (
                <FlatList
                    ...
                    renderItem={({ item, index }) => {
                        if (componentDynamic?.props?.value && !item?.isGroup)
                            componentDynamic.props.value.props = {
                                renderConfig: renderConfig,
                                key: index,
                                currentDetail: detail,
        ...
        let isShowBtn = permissionBtnCreate && isShowBtnCreate;
        return (
            <View
                style={[
