...
export default function shouldComponentUpdate(nextProps, nextState) {
  ...
  // re-render component when state isVisibleWeek change (custom)
  if (nextState.isVisibleWeek !== this.state.isVisibleWeek) {
    shouldUpdate = {
      update: true,
      field: 'current'
    };
  ...
  // re-render component when state isVisibleWeek change (custom)
  if (nextState.isVisibleMonth !== this.state.isVisibleMonth) {
    shouldUpdate = {
      update: true,
      field: 'current'
    };
