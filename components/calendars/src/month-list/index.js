...
import {
  FlatList, Platform, Dimensions, ActivityIndicator, View,
  StyleSheet,
  ...
  renderMonthItem = (item, index) => {
    return (
      <Item
        ...
        isToday={this.props.month.getMonth() === item.value ? true : false}
      />
    )
  }
    ...
    return (
      <View style={styles.viewMonth}>
        <TouchableOpacity
          ...
          style={styles.viewMonth__Next}
        >
          <IconBack size={Size.iconSize} color={Colors.grey} />
        </TouchableOpacity>

        <View style={styles.viewMonth__center}>
          <ScrollView
            horizontal={true}
          ...
          >
            <View style={{
              ...
              {this.state.rows.map((item, index) => this.renderMonthItem(item, index))}
            </View>
          </ScrollView>
          {/* <FlatList
            ...
            renderItem={({ item, index }) => this.renderMonthItem(item, index)}
            numColumns={4}
