    ...
    onPress = (dataItem, indexItem) => {
        ...
        if (actionUpdateIsSeen && actionUpdateIsSeen.onPress) {
            ...
            if (findItem && findItem.Status !== 'E_SEEN') {
                ...
                // const lengthItemIsSeen = dataItem.lstID ? dataItem.lstID.length : 0;
                if (countBadgesNotify != null && countBadgesNotify > 0) setNumberBadgesNotify(countBadgesNotify - 1);
            }
        }
        ...
        if (pushScreenName && pushScreenName !== null) {
            ...
            if (finalPushScreenName != null && finalPushScreenName.includes('ViewDetail')) {
                let tempFinalPushScreenName = NotificationsService.handleViewDetailFormNotification(finalPushScreenName, finalScreenNameConfig);
                if (tempFinalPushScreenName != null && tempFinalPushScreenName.includes('TopTab')) {
                    finalPushScreenName = tempFinalPushScreenName;
            ...
            viewContent = (
                <View style={styles.container}>
                    ...
                    {dataGroup.map((dataItem, index) => {
                        const makeID = `${dataItem.lstID.join('')}${index}`;
