        ...
        if (actionEvaluatePer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_EVALUATION,
                    onPress: (item) => EvaPerformanceWaitBusinessFunction.businessEvaluationRecord(item),
                    ...actionEvaluate
                }
            ];
        }
...
export const EvaPerformanceWaitBusinessFunction = {
    ...
    businessEvaluationRecord: (item) => {
        ...
        HttpService.Get('[URI_POR]/New_PersonalWaitingEvaluation/GetById?ID=' + item.ID).then((res) => {
