...
let configList = null,
    enumName = null,
    salRejectedSubmitPITFinalization = null,
    salRejectedSubmitPITFinalizationViewDetail = null,
    salRejectedSubmitPITFinalizationKeyTask = null,
    pageSizeList = 20;

class SalRejectedSubmitPITFinalization extends Component {
    constructor(props) {
        super(props);
        ...
        this.willFocusScreen = this.props.navigation.addListener('willFocus', () => {
            ...
            if (SalSubmitPITFinalizationBusinessFunction.checkForReLoadScreen[salRejectedSubmitPITFinalization]) {
                this.reload();
            }
        });
    }
    ...
    reload = async (paramsFilter) => {
        ...
        SalSubmitPITFinalizationBusinessFunction.checkForReLoadScreen[salRejectedSubmitPITFinalization] = false;
        ...
        this.setState(
            ...
            () => {
                ...
                startTask({
                    keyTask: salRejectedSubmitPITFinalizationKeyTask,
                    payload: {
                        ...dataBody,
                        keyQuery: keyQuery,
                        isCompare: false,
    ...
    paramsDefault = async () => {
        if (!configList[salRejectedSubmitPITFinalization]) {
            configList[salRejectedSubmitPITFinalization] = {
                Api: {
        ...
        const _configList = configList[salRejectedSubmitPITFinalization],
            filter = _configList[enumName.E_Filter],
            renderRow = _configList[enumName.E_Row],
            dataFromParams = this.checkDataFormNotify();

        const dataRowActionAndSelected = generateRowActionAndSelected(salRejectedSubmitPITFinalization);
        let _params = {
            ...dataFromParams,
            IsPortalNew: true,
            filter: filter,
    ...
    UNSAFE_componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.reloadScreenName == salRejectedSubmitPITFinalizationKeyTask) {
    ...
    async componentDidMount() {
        salRejectedSubmitPITFinalization = ScreenName.SalRejectedSubmitPITFinalization;
        salRejectedSubmitPITFinalizationViewDetail = ScreenName.SalSubmitPITFinalizationViewDetail;
        salRejectedSubmitPITFinalizationKeyTask = EnumTask.KT_SalRejectedSubmitPITFinalization;
        SalSubmitPITFinalizationBusinessFunction.checkForReLoadScreen[salRejectedSubmitPITFinalization] = false;
        //set by config
        ...
        this.setState(ConfigListFilter.value[salRejectedSubmitPITFinalization] ? _paramsDefault : paramStore);
    ...
    render() {
        ...
        return (
            <SafeAreaViewDetail style={styleSafeAreaView.style}>
                {salRejectedSubmitPITFinalization && salRejectedSubmitPITFinalizationViewDetail && enumName && (
                        ...
                        <View style={[styleSheets.container]}>
                            {keyQuery && (
                                <SalPITFinalizationList
                                    detail={{
                                        dataLocal: false,
                                        screenDetail: salRejectedSubmitPITFinalizationViewDetail,
                                        screenName: salRejectedSubmitPITFinalization,
                                        screenNameRender: ScreenName.SalRejectedSubmitPITFinalization
                                    }}
                                    scrollYAnimatedValue={this.scrollYAnimatedValue}
                                    rowActions={rowActions}
                                    selected={selected}
                                    reloadScreenList={this.reload.bind(this)}
                                    keyDataLocal={salRejectedSubmitPITFinalizationKeyTask}
...
export default connect(mapStateToProps, null)(SalRejectedSubmitPITFinalization);
