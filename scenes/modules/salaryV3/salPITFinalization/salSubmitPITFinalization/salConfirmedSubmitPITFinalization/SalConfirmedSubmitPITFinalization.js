...
let configList = null,
    enumName = null,
    salConfirmedSubmitPITFinalization = null,
    salConfirmedSubmitPITFinalizationViewDetail = null,
    salConfirmedSubmitPITFinalizationKeyTask = null,
    pageSizeList = 20;

class SalConfirmedSubmitPITFinalization extends Component {
    constructor(props) {
        super(props);
        ...
        this.willFocusScreen = this.props.navigation.addListener('willFocus', () => {
            ...
            if (SalSubmitPITFinalizationBusinessFunction.checkForReLoadScreen[salConfirmedSubmitPITFinalization]) {
                this.reload();
            }
        });
    }
    ...
    reload = async (paramsFilter) => {
        ...
        SalSubmitPITFinalizationBusinessFunction.checkForReLoadScreen[salConfirmedSubmitPITFinalization] = false;
        ...
        this.setState(
            ...
            () => {
                ...
                startTask({
                    keyTask: salConfirmedSubmitPITFinalizationKeyTask,
                    payload: {
                        ...dataBody,
                        keyQuery: keyQuery,
                        isCompare: false,
    ...
    paramsDefault = async () => {
        if (!configList[salConfirmedSubmitPITFinalization]) {
            configList[salConfirmedSubmitPITFinalization] = {
                Api: {
        ...
        const _configList = configList[salConfirmedSubmitPITFinalization],
            filter = _configList[enumName.E_Filter],
            renderRow = _configList[enumName.E_Row],
            dataFromParams = this.checkDataFormNotify();

        const dataRowActionAndSelected = generateRowActionAndSelected(salConfirmedSubmitPITFinalization);
        let _params = {
            ...dataFromParams,
            IsPortalNew: true,
            filter: filter,
    ...
    UNSAFE_componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.reloadScreenName == salConfirmedSubmitPITFinalizationKeyTask) {
    ...
    async componentDidMount() {
        salConfirmedSubmitPITFinalization = ScreenName.SalConfirmedSubmitPITFinalization;
        salConfirmedSubmitPITFinalizationViewDetail = ScreenName.SalSubmitPITFinalizationViewDetail;
        salConfirmedSubmitPITFinalizationKeyTask = EnumTask.KT_SalConfirmedSubmitPITFinalization;
        SalSubmitPITFinalizationBusinessFunction.checkForReLoadScreen[salConfirmedSubmitPITFinalization] = false;
        //set by config
        ...
        this.setState(ConfigListFilter.value[salConfirmedSubmitPITFinalization] ? _paramsDefault : paramStore);
    ...
    render() {
        ...
        return (
            <SafeAreaViewDetail style={styleSafeAreaView.style}>
                {salConfirmedSubmitPITFinalization && salConfirmedSubmitPITFinalizationViewDetail && enumName && (
                        ...
                        <View style={[styleSheets.container]}>
                            {keyQuery && (
                                <SalPITFinalizationList
                                    detail={{
                                        dataLocal: false,
                                        screenDetail: salConfirmedSubmitPITFinalizationViewDetail,
                                        screenName: salConfirmedSubmitPITFinalization,
                                        screenNameRender: ScreenName.SalConfirmedSubmitPITFinalization
                                    }}
                                    scrollYAnimatedValue={this.scrollYAnimatedValue}
                                    rowActions={rowActions}
                                    selected={selected}
                                    reloadScreenList={this.reload.bind(this)}
                                    keyDataLocal={salConfirmedSubmitPITFinalizationKeyTask}
...
export default connect(mapStateToProps, null)(SalConfirmedSubmitPITFinalization);
