...
let configList = null,
    enumName = null,
    salCanceledSubmitPITFinalization = null,
    salCanceledSubmitPITFinalizationViewDetail = null,
    salCanceledSubmitPITFinalizationKeyTask = null,
    pageSizeList = 20;

class SalCanceledSubmitPITFinalization extends Component {
    constructor(props) {
        super(props);
        ...
        this.willFocusScreen = this.props.navigation.addListener('willFocus', () => {
            ...
            if (SalSubmitPITFinalizationBusinessFunction.checkForReLoadScreen[salCanceledSubmitPITFinalization]) {
                this.reload();
            }
        });
    }
    ...
    reload = async (paramsFilter) => {
        ...
        SalSubmitPITFinalizationBusinessFunction.checkForReLoadScreen[salCanceledSubmitPITFinalization] = false;
        ...
        this.setState(
            ...
            () => {
                ...
                startTask({
                    keyTask: salCanceledSubmitPITFinalizationKeyTask,
                    payload: {
                        ...dataBody,
                        keyQuery: keyQuery,
                        isCompare: false,
    ...
    paramsDefault = async () => {
        if (!configList[salCanceledSubmitPITFinalization]) {
            configList[salCanceledSubmitPITFinalization] = {
                Api: {
        ...
        const _configList = configList[salCanceledSubmitPITFinalization],
            filter = _configList[enumName.E_Filter],
            renderRow = _configList[enumName.E_Row],
            dataFromParams = this.checkDataFormNotify();

        const dataRowActionAndSelected = generateRowActionAndSelected(salCanceledSubmitPITFinalization);
        let _params = {
            ...dataFromParams,
            IsPortalNew: true,
            filter: filter,
    ...
    UNSAFE_componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.reloadScreenName == salCanceledSubmitPITFinalizationKeyTask) {
    ...
    async componentDidMount() {
        salCanceledSubmitPITFinalization = ScreenName.SalCanceledSubmitPITFinalization;
        salCanceledSubmitPITFinalizationViewDetail = ScreenName.SalSubmitPITFinalizationViewDetail;
        salCanceledSubmitPITFinalizationKeyTask = EnumTask.KT_SalCanceledSubmitPITFinalization;
        SalSubmitPITFinalizationBusinessFunction.checkForReLoadScreen[salCanceledSubmitPITFinalization] = false;
        //set by config
        ...
        this.setState(ConfigListFilter.value[salCanceledSubmitPITFinalization] ? _paramsDefault : paramStore);
    ...
    render() {
        ...
        return (
            <SafeAreaViewDetail style={styleSafeAreaView.style}>
                {salCanceledSubmitPITFinalization && salCanceledSubmitPITFinalizationViewDetail && enumName && (
                        ...
                        <View style={[styleSheets.container]}>
                            {keyQuery && (
                                <SalPITFinalizationList
                                    detail={{
                                        dataLocal: false,
                                        screenDetail: salCanceledSubmitPITFinalizationViewDetail,
                                        screenName: salCanceledSubmitPITFinalization,
                                        screenNameRender: ScreenName.SalCanceledSubmitPITFinalization
                                    }}
                                    scrollYAnimatedValue={this.scrollYAnimatedValue}
                                    rowActions={rowActions}
                                    selected={selected}
                                    reloadScreenList={this.reload.bind(this)}
                                    keyDataLocal={salCanceledSubmitPITFinalizationKeyTask}
...
export default connect(mapStateToProps, null)(SalCanceledSubmitPITFinalization);
