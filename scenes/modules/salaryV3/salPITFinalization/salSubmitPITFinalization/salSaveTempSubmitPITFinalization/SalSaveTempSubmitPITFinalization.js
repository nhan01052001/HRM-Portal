...
let configList = null,
    enumName = null,
    salSaveTempSubmitPITFinalization = null,
    salSaveTempSubmitPITFinalizationViewDetail = null,
    salSaveTempSubmitPITFinalizationKeyTask = null,
    pageSizeList = 20;

class SalSaveTempSubmitPITFinalization extends Component {
    constructor(props) {
        super(props);
        ...
        this.willFocusScreen = this.props.navigation.addListener('willFocus', () => {
            ...
            if (SalSubmitPITFinalizationBusinessFunction.checkForReLoadScreen[salSaveTempSubmitPITFinalization]) {
                this.reload();
            }
        });
    }
    ...
    reload = async (paramsFilter) => {
        ...
        SalSubmitPITFinalizationBusinessFunction.checkForReLoadScreen[salSaveTempSubmitPITFinalization] = false;
        ...
        this.setState(
            ...
            () => {
                ...
                startTask({
                    keyTask: salSaveTempSubmitPITFinalizationKeyTask,
                    payload: {
                        ...dataBody,
                        keyQuery: keyQuery,
                        isCompare: false,
    ...
    paramsDefault = async () => {
        if (!configList[salSaveTempSubmitPITFinalization]) {
            configList[salSaveTempSubmitPITFinalization] = {
                Api: {
                ...
                BusinessAction: [
                    {
                        Type: 'E_MODIFY',
                        Resource: {
                            Name: 'New_Sal_PITFinalizationDelegatee_New_Index_V3',
                            Rule: 'Modify'
                        },
                        Confirm: {
                            isInputText: true,
                    ...
                    },
                    {
                        Type: 'E_SENDMAIL',
                        Resource: {
                            Name: 'New_Sal_PITFinalizationDelegatee_New_Index_V3_btnSendRequest',
                            Rule: 'View'
                        }
                    }
                ]
        ...
        const _configList = configList[salSaveTempSubmitPITFinalization],
            filter = _configList[enumName.E_Filter],
            renderRow = _configList[enumName.E_Row],
            dataFromParams = this.checkDataFormNotify();

        const dataRowActionAndSelected = generateRowActionAndSelected(salSaveTempSubmitPITFinalization);
        let _params = {
            ...dataFromParams,
            IsPortalNew: true,
            filter: filter,
    ...
    UNSAFE_componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.reloadScreenName == salSaveTempSubmitPITFinalizationKeyTask) {
    ...
    async componentDidMount() {
        salSaveTempSubmitPITFinalization = ScreenName.SalSaveTempSubmitPITFinalization;
        salSaveTempSubmitPITFinalizationViewDetail = ScreenName.SalSubmitPITFinalizationViewDetail;
        salSaveTempSubmitPITFinalizationKeyTask = EnumTask.KT_SalSaveTempSubmitPITFinalization;
        SalSubmitPITFinalizationBusinessFunction.checkForReLoadScreen[salSaveTempSubmitPITFinalization] = false;
        //set by config
        ...
        this.setState(ConfigListFilter.value[salSaveTempSubmitPITFinalization] ? _paramsDefault : paramStore);
    ...
    render() {
        ...
        return (
            <SafeAreaViewDetail style={styleSafeAreaView.style}>
                {salSaveTempSubmitPITFinalization && salSaveTempSubmitPITFinalizationViewDetail && enumName && (
                        ...
                        <View style={[styleSheets.container]}>
                            {keyQuery && (
                                <SalPITFinalizationList
                                    detail={{
                                        dataLocal: false,
                                        screenDetail: salSaveTempSubmitPITFinalizationViewDetail,
                                        screenName: salSaveTempSubmitPITFinalization,
                                        screenNameRender: ScreenName.SalSubmitPITFinalization
                                    }}
                                    scrollYAnimatedValue={this.scrollYAnimatedValue}
                                    rowActions={rowActions}
                                    selected={selected}
                                    reloadScreenList={this.reload.bind(this)}
                                    keyDataLocal={salSaveTempSubmitPITFinalizationKeyTask}
...
export default connect(mapStateToProps, null)(SalSaveTempSubmitPITFinalization);
