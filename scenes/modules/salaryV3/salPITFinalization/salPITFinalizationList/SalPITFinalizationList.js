...
import SalPITFinalizationListItem from './SalPITFinalizationListItem';
...
export default class SalPITFinalizationList extends VnrRenderList {
    remoteData = (param = {}) => {
        ...
        if (keyDataLocal) {
            getDataLocal(keyDataLocal)
                .then((resData) => {
                    ...
                    if (
                        resTemp?.Status === EnumName.E_SUCCESS &&
                        Array.isArray(resTemp?.Data?.Data) &&
                        resTemp?.Data?.Data.length > 0
                    ) {
                        const res = resTemp?.Data?.Data;
                        let data = [],
                            dataNoGroup = [],
                            total = 1;
                    ...
                    } else if (resTemp === EnumName.E_EMPTYDATA || (resTemp?.Status === EnumName.E_SUCCESS &&
                        Array.isArray(resTemp?.Data?.Data) &&
                        resTemp?.Data?.Data.length === 0)) {
                        this.setState({
                            itemSelected: [],
    ...
    render() {
        const {
            ...
            { api, detail, rowActions, onCreate, scrollYAnimatedValue, renderConfig, isShowHideCreate } = this.props,
            permissionBtnCreate =
                PermissionForAppMobile &&
                    PermissionForAppMobile.value['New_Sal_PITFinalizationDelegatee_New_Index_V3'] &&
                    PermissionForAppMobile.value['New_Sal_PITFinalizationDelegatee_New_Index_V3']['Create']
                    ? true
                    : false;
            ...
            contentList = (
                <FlatList
                    ...
                    style={
                        ...
                        this.isHaveFilter ? styles.styHaveFilter : styles.styNotHaveFilter
                    }
        ...
        let isShowBtn =
            permissionBtnCreate &&
            detail.screenNameRender === ScreenName.SalSubmitPITFinalization &&
            isShowHideCreate &&
...
const styles = {
    ...
    styHaveFilter: { marginTop: 0, paddingTop: 80 }
};
