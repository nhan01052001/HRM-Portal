...
const ENUM_STATUS = {
    E_TEMPSAVE: 'HRM_PortalApp_PITFinalization_tabPendingSend',
    E_WAITINGCONFIRM: 'HRM_PortalApp_PITFinalization_tabWaitingConfirmed',
    E_CONFIRMED: 'HRM_PortalApp_PITFinalization_tabConfirmed',
    E_REJECT: 'HRM_PortalApp_PITFinalization_tabRejected',
    E_CANCEL: 'HRM_PortalApp_PITFinalization_tabCanceled'
};

export default class SalPITFinalizationListItem extends VnrRenderListItem {
    renderStatusView = (dataItem, renderConfig) => {
        if (renderConfig && renderConfig.length > 0) {
            ...
            if (configStatus) {
                ...
                else if (dataItem[fieldStatus] && ENUM_STATUS[dataItem[fieldStatus]]) {
                    valueView = translate(ENUM_STATUS[dataItem[fieldStatus]]);
                }
                ...
                <View style={[styles.swipeableLayout, isDisableWhenConfig && styleSheets.opacity05]}>
                    <View style={styles.left_isCheckbox}>
                        {rowActions === null || (Array.isArray(rowActions) && rowActions.length === 0) ? null : (
                            <View style={[styles.styRadiusCheck, this.props.isSelect && styles.styViewIsSelected]}>
                                ...
                                <View style={styles.styViewTop}>
                                    <View style={styles.contentCenter}>
                                        <Text numberOfLines={1} style={[styleSheets.lable, styles.profileText]}>
                                            {dataItem.RegisterTypeView ? dataItem.RegisterTypeView : ''}
                                        </Text>
                                    </View>
