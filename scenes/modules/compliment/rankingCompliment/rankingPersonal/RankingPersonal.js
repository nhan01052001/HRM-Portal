...
let configList = null,
    enumName = null,
    rankingPersonal = null,
    rankingPersonalKeyTask = null,
    pageSizeList = 20;

class RankingPersonal extends Component {
    constructor(props) {
        super(props);
        ...
        this.willFocusScreen = this.props.navigation.addListener('willFocus', () => {
            ...
            if (HistoryOfComComplimentBusinessFunction.checkForReLoadScreen[rankingPersonal]) {
                this.reload();
            }
        });
    }
    ...
    reload = (paramsFilter) => {
        ...
        HistoryOfComComplimentBusinessFunction.checkForReLoadScreen[rankingPersonal] = false;

        // Gửi yêu cầu lọc dữ liệu
        this.setState(_paramsDefault, () => {
            const { dataBody, keyQuery } = this.state;
            startTask({
                keyTask: rankingPersonalKeyTask,
                payload: {
                    ...dataBody,
                    keyQuery: keyQuery,
                    isCompare: false,
    ...
    paramsDefault = () => {
        if (!rankingPersonal) {
            rankingPersonal = ScreenName.RankingPersonal;
        }
        if (!configList[rankingPersonal]) {
            configList[rankingPersonal] = {
                Api: {
                    urlApi: '[URI_HR]/Com_GetData/GetComplimentRankIndividual',
                    type: 'POST',
                    pageSize: 20
                },
                Order: [
        ...
        const _configList = configList[rankingPersonal],
            filter = _configList[enumName.E_Filter];

        const dataRowActionAndSelected = generateRowActionAndSelected(rankingPersonal);
        let _params = {
            // IsPortal: true,
            // sort: orderBy,
            IsPortalNew: true,
    ...
    UNSAFE_componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.reloadScreenName == rankingPersonalKeyTask) {
    ...
    componentDidMount() {
        ...
        rankingPersonal = ScreenName.RankingPersonal;
        rankingPersonalKeyTask = EnumTask.KT_RankingPersonal;
        HistoryOfComComplimentBusinessFunction.checkForReLoadScreen[rankingPersonal] = false;
        //set by config
    ...
    render() {
        ...
        ConfigListFilter.value[rankingPersonal] = {
            ...ConfigListFilter.value[rankingPersonal],
            RankingPersonal: {
                FilterCommon: {
                    fieldName: 'Key',
                            ...
                            {keyQuery && (
                                <ListRankingCompliment
                                    detail={{
                                        dataLocal: false,
                                        screenName: rankingPersonal,
                                        screenNameRender: ScreenName.RankingPersonal
                                    }}
                                    rowActions={rowActions}
                                    selected={selected}
                                    reloadScreenList={this.reload.bind(this)}
                                    keyDataLocal={rankingPersonalKeyTask}
...
export default connect(mapStateToProps, null)(RankingPersonal);
