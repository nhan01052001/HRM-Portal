...
let configList = null,
    enumName = null,
    rankingDepartment = null,
    rankingDepartmentKeyTask = null,
    pageSizeList = 20;

class RankingDepartment extends Component {
    constructor(props) {
        super(props);
        ...
        this.willFocusScreen = this.props.navigation.addListener('willFocus', () => {
            ...
            if (HistoryOfComComplimentBusinessFunction.checkForReLoadScreen[rankingDepartment]) {
                this.reload();
            }
        });
    }
    ...
    reload = (paramsFilter) => {
        ...
        HistoryOfComComplimentBusinessFunction.checkForReLoadScreen[rankingDepartment] = false;

        // Gửi yêu cầu lọc dữ liệu
        this.setState(_paramsDefault, () => {
            const { dataBody, keyQuery } = this.state;
            startTask({
                keyTask: rankingDepartmentKeyTask,
                payload: {
                    ...dataBody,
                    keyQuery: keyQuery,
                    isCompare: false,
    ...
    paramsDefault = () => {
        if (!rankingDepartment) {
            rankingDepartment = ScreenName.RankingDepartment;
        }
        if (!configList[rankingDepartment]) {
            configList[rankingDepartment] = {
                Api: {
                    urlApi: '[URI_HR]/Com_GetData/GetComplimentRankDepartment',
                    type: 'POST',
                    pageSize: 20
                },
                Order: [
        ...
        const _configList = configList[rankingDepartment],
            filter = _configList[enumName.E_Filter];

        const dataRowActionAndSelected = generateRowActionAndSelected(rankingDepartment);
        let _params = {
            // IsPortal: true,
            // sort: orderBy,
            IsPortalNew: true,
    ...
    UNSAFE_componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.reloadScreenName == rankingDepartmentKeyTask) {
    ...
    componentDidMount() {
        ...
        rankingDepartment = ScreenName.RankingDepartment;
        rankingDepartmentKeyTask = EnumTask.KT_RankingDepartment;
        HistoryOfComComplimentBusinessFunction.checkForReLoadScreen[rankingDepartment] = false;
        //set by config
                            ...
                            {keyQuery && (
                                <ListRankingCompliment
                                    detail={{
                                        dataLocal: false,
                                        screenName: rankingDepartment,
                                        screenNameRender: ScreenName.RankingDepartment
                                    }}
                                    rowActions={rowActions}
                                    selected={selected}
                                    reloadScreenList={this.reload.bind(this)}
                                    keyDataLocal={rankingDepartmentKeyTask}
...
export default connect(mapStateToProps, null)(RankingDepartment);
