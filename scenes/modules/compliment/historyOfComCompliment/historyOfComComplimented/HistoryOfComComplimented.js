...
let configList = null,
    enumName = null,
    historyOfComComplimented = null,
    attHistoryOfBeComComplimentKeyTask = null,
    pageSizeList = 20;

class HistoryOfComComplimented extends Component {
    constructor(props) {
        super(props);
        ...
        this.willFocusScreen = this.props.navigation.addListener('willFocus', () => {
            ...
            if (HistoryOfComComplimentBusinessFunction.checkForReLoadScreen[historyOfComComplimented]) {
                this.reload();
            }
        });
    }
    ...
    reload = (paramsFilter) => {
        ...
        HistoryOfComComplimentBusinessFunction.checkForReLoadScreen[historyOfComComplimented] = false;
        ...
        this.setState(_paramsDefault, () => {
            ...
            startTask({
                keyTask: attHistoryOfBeComComplimentKeyTask,
                payload: {
                    ...dataBody,
                    keyQuery: keyQuery,
                    isCompare: false,
    ...
    paramsDefault = () => {
        if (!historyOfComComplimented) {
            historyOfComComplimented = ScreenName.HistoryOfComComplimented;
        }
        if (!configList[historyOfComComplimented]) {
            configList[historyOfComComplimented] = {
                Api: {
        ...
        const _configList = configList[historyOfComComplimented],
            filter = _configList[enumName.E_Filter];

        const dataRowActionAndSelected = generateRowActionAndSelected(historyOfComComplimented);
        let _params = {
            // IsPortal: true,
            // sort: orderBy,
            IsPortalNew: true,
    ...
    UNSAFE_componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.reloadScreenName == attHistoryOfBeComComplimentKeyTask) {
    ...
    componentDidMount() {
        ...
        historyOfComComplimented = ScreenName.HistoryOfComComplimented;
        attHistoryOfBeComComplimentKeyTask = EnumTask.KT_HistoryOfComComplimented;
        HistoryOfComComplimentBusinessFunction.checkForReLoadScreen[historyOfComComplimented] = false;
        //set by config
    ...
    render() {
        ...
        return (
            <SafeAreaViewDetail style={styleSafeAreaView.style}>
                {historyOfComComplimented && enumName && (
                    <View style={[styleSheets.containerGrey]}>
                        {ConfigListFilter.value[historyOfComComplimented] && (
                            <View style={[styleContentFilterDesign.styRightPicker, { backgroundColor: Colors.white }]}>
                                <VnrFilterCommon
                                    ...
                                    screenName={historyOfComComplimented}
                            ...
                            {keyQuery && (
                                <ListHistoryOfComCompliment
                                    detail={{
                                        dataLocal: false,
                                        screenName: historyOfComComplimented,
                                        screenNameRender: ScreenName.HistoryOfComComplimented
                                    }}
                                    rowActions={rowActions}
                                    selected={selected}
                                    reloadScreenList={this.reload.bind(this)}
                                    keyDataLocal={attHistoryOfBeComComplimentKeyTask}
...
export default connect(mapStateToProps, null)(HistoryOfComComplimented);
