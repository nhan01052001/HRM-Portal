...
export const generateRowActionAndSelected = () => {
    ...
    if (ConfigList.value != null && ConfigList.value != undefined) {
        const _configList = ConfigList.value[hreSubmitRequirementRecruitment],
            ...
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    onPress: (item, dataBody) =>
                        HreSubmitRequirementRecruitmentBusinessFunction.businessSendMailRecords(
                            [{ ...item }],
                            dataBody
                        ),
                    ...actionSendMail
            ...
            _selected = [
                ..._selected,
                {
                    ...
                    onPress: (items, dataBody) =>
                        HreSubmitRequirementRecruitmentBusinessFunction.businessSendMailRecords(items, dataBody),
                    ...actionSendMail
                }
            ];
        }
        ...
        if (actionEditPer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_MODIFY,
                    onPress: (item) =>
                        HreSubmitRequirementRecruitmentBusinessFunction.businessModifyRecord({ ...item }),
                    ...actionEdit
                }
            ];
        }
        ...
        if (actionDeletePer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_DELETE,
                    onPress: (item, dataBody) =>
                        HreSubmitRequirementRecruitmentBusinessFunction.businessDeleteRecords([{ ...item }], dataBody),
                    ...actionDelete
                }
            ];
...
export const HreSubmitRequirementRecruitmentBusinessFunction = {
            ...
            HttpService.Post('[URI_HR]/Por_GetData/BusinessAllowActionSendEmail', {
                ...
                try {
                    if (res && typeof res === enumName.E_object) {
                        ...
                        if (isValid == true) {
                            HreSubmitRequirementRecruitmentBusinessFunction.confirmSendMail(res);
            ...
            AlertSevice.alert({
                ...
                onConfirm: (reason) => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        HttpService.Post('[URI_HR]/Rec_GetData/processSendRequestRequirementRecruitment', {
                            host: _uriPor,
                            selectedIds: arrId,
                            reason
                        }).then((res) => {
            ...
            HttpService.Post('[URI_HR]/Por_GetData/BusinessAllowActionDelete', {
                ...
                try {
                    if (res && typeof res === enumName.E_object) {
                        ...
                        if (isValid) {
                            HreSubmitRequirementRecruitmentBusinessFunction.confirmDelete(res);
                        }
                        //không hợp lệ
                        else if (isValid == false && res.message && typeof res.message === enumName.E_string) {
                            ToasterSevice.showWarning(res.message, 4000);
            ...
            AlertSevice.alert({
                ...
                onConfirm: (reason) => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        HreSubmitRequirementRecruitmentBusinessFunction.setIsDelete({ Ids: strId, reason });
                    }
                }
            });
        } else {
            let strId = objValid.strResultID;
            HreSubmitRequirementRecruitmentBusinessFunction.setIsDelete({ Ids: strId });
        }
    },

    // need fix /New_Hre_RequirementRecruitment/RemoveSelectedRequirementRecruitment [done]
    setIsDelete: (objValid) => {
        VnrLoadingSevices.show();
        let selectedIds = objValid.Ids,
            reason = objValid.reason;
        HttpService.Post('[URI_HR]/Hre_GetDataV2/RequirementRecruitmentRemoveSelected', {
            selectedIds,
            reason
        }).then((res) => {
            VnrLoadingSevices.hide();
    ...
    businessModifyRecord: (item) => {
        ...
        VnrLoadingSevices.show();
        // need fix [done]
        HttpService.Post('[URI_HR]/Por_GetData/GetRequirementRecuitmentById', {
            id: item.ID,
            screenName: hreSubmitRequirementRecruitment
        }).then((res) => {
            VnrLoadingSevices.hide();
            try {
                if (res && res.BusinessAllowAction && res.BusinessAllowAction.indexOf('E_MODIFY') >= 0) {
                    const { reload } = _this;
                    _this.props.navigation.navigate(hreSubmitRequirementRecruitmentAddOrEdit, { record: item, reload });
                } else {
