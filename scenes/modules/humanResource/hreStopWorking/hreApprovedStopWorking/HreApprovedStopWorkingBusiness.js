...
export const generateRowActionAndSelected = () => {
    ...
    if (ConfigList.value != null && ConfigList.value != undefined) {
        const _configList = ConfigList.value[hreApprovedStopWorking],
            ...
            _rowActions = [
                {
                    ...
                    onPress: (item, dataBody) =>
                        HreApprovedStopWorkingBusinessFunction.businessCancelRecords([{ ...item }], dataBody),
                    ...actionCancel
                }
            ];
            ...
            _selected = [
                {
                    ...
                    onPress: (items, dataBody) =>
                        HreApprovedStopWorkingBusinessFunction.businessCancelRecords(items, dataBody),
                    ...actionCancel
                }
            ];
        }
...
export const HreApprovedStopWorkingBusinessFunction = {
    ...
    businessCancelRecords: (items, dataBody) => {
        if (items.length === 0 && !dataBody) {
            ...
            if (selectedID.length === 1) {
                ...
                HttpService.Post('[URI_HR]/Hre_GetData/BusinessAllowActionCancel', {
                    selectedID,
                    ...
                    try {
                        if (res && typeof res === enumName.E_object) {
                            ...
                            if (isValid) {
                                HreApprovedStopWorkingBusinessFunction.confirmCancel(res);
                            }
                            //không hợp lệ
            ...
            AlertSevice.alert({
                ...
                onConfirm: (reason) => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        HreApprovedStopWorkingBusinessFunction.setStatusCancel({ Ids: strId, reason });
                    }
                }
            });
        } else {
            let strId = objValid.strResultID;
            HreApprovedStopWorkingBusinessFunction.setStatusCancel({ Ids: strId });
        }
    },
    ...
    setStatusCancel: (objValid) => {
        ...
        VnrLoadingSevices.show();
        HttpService.Post('[URI_HR]/Hre_GetData/CancelStopWorkingPortal', {
            selectedIds,
            declineReason
        }).then((res) => {
            VnrLoadingSevices.hide();
            ...
            try {
                if (res && res !== '') {
                    ...
                    } else if (res.indexOf('NotFinishWorkList') >= 0) {
                        ToasterSevice.showWarning('NotFinishWorkList', 5000);
                    }
                    //update work history
                    // else if (res == 'HRM_HR_StopWorking_DoYouWantToUpdateWorkHistory') {
                    //     HreApprovedStopWorkingBusinessFunction.updateWorkHistory(res);
                    // }
                } else {
