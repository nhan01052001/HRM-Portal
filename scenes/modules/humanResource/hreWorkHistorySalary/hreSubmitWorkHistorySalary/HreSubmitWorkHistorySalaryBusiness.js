...
export const generateRowActionAndSelected = () => {
    ...
    if (ConfigList.value != null && ConfigList.value != undefined) {
        const _configList = ConfigList.value[hreSubmitWorkHistorySalary],
            ...
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    onPress: (item, dataBody) =>
                        HreSubmitWorkHistorySalaryBusinessFunction.businessSendMailRecords([{ ...item }], dataBody),
                    ...actionSendMail
                }
            ];
            ...
            _selected = [
                ..._selected,
                {
                    ...
                    onPress: (items, dataBody) =>
                        HreSubmitWorkHistorySalaryBusinessFunction.businessSendMailRecords(items, dataBody),
                    ...actionSendMail
                }
            ];
        }
        ...
        if (actionEditPer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_MODIFY,
                    onPress: (item) => HreSubmitWorkHistorySalaryBusinessFunction.businessModifyRecord({ ...item }),
                    ...actionEdit
                }
            ];
        }
        ...
        if (actionDeletePer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_DELETE,
                    onPress: (item, dataBody) =>
                        HreSubmitWorkHistorySalaryBusinessFunction.businessDeleteRecords([{ ...item }], dataBody),
                    ...actionDelete
                }
            ];
...
export const HreSubmitWorkHistorySalaryBusinessFunction = {
            ...
            HttpService.Post('[URI_HR]/Por_GetData/BusinessAllowActionSendEmail', {
                ...
                try {
                    if (res && typeof res === enumName.E_object) {
                        ...
                        if (isValid == true) {
                            HreSubmitWorkHistorySalaryBusinessFunction.confirmSendMail(res);
            ...
            AlertSevice.alert({
                ...
                onConfirm: (reason) => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        HttpService.Post('[URI_HR]/Hre_GetData/ProcessingSendMailWorkhistory', {
                            host: _uriPor,
                            selectedIds: arrId,
                            reason
                        }).then((res) => {
            ...
            HttpService.Post('[URI_HR]/Por_GetData/BusinessAllowActionDelete', {
                ...
                try {
                    if (res && typeof res === enumName.E_object) {
                        ...
                        if (isValid) {
                            HreSubmitWorkHistorySalaryBusinessFunction.confirmDelete(res);
                        }
                        //không hợp lệ
                        else if (isValid == false && res.message && typeof res.message === enumName.E_string) {
                            ToasterSevice.showWarning(res.message, 4000);
            ...
            AlertSevice.alert({
                ...
                onConfirm: (reason) => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        HreSubmitWorkHistorySalaryBusinessFunction.setIsDelete({ Ids: strId, reason });
                    }
                }
            });
        } else {
            let strId = objValid.strResultID;
            HreSubmitWorkHistorySalaryBusinessFunction.setIsDelete({ Ids: strId });
        }
    },

    // need fix /New_Hre_WorkHistorySalary/RemoveSelectedWorkhistorySalary [done]
    setIsDelete: (objValid) => {
        VnrLoadingSevices.show();
        let selectedIds = objValid.Ids,
            reason = objValid.reason;
        HttpService.Post('[URI_POR]/New_Hre_WorkHistorySalary/RemoveSelectedWorkhistorySalary', {
            selectedIds,
            reason
        }).then((res) => {
            VnrLoadingSevices.hide();
    ...
    businessModifyRecord: (item) => {
        ...
        VnrLoadingSevices.show();
        // need fix [done]
        HttpService.Post('[URI_HR]/Por_GetData/GetWorkHistoryById', {
            id: item.ID,
            screenName: hreSubmitWorkHistorySalary
        }).then((res) => {
            VnrLoadingSevices.hide();
            if (res[0]) {
                const item = res[0];
                try {
                    if (item && item.BusinessAllowAction && item.BusinessAllowAction.indexOf('E_MODIFY') >= 0) {
                        const { reload } = _this;
                        _this.props.navigation.navigate(hreSubmitWorkHistorySalaryAddOrEdit, { record: item, reload });
                    } else {
