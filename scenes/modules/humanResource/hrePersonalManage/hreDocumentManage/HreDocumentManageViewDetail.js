...
export default class HreDocumentManageViewDetail extends Component {
    constructor(props) {
        super(props);
        this.state = {
            dataItem: null,
    ...
    getDataItem = async () => {
        try {
            const _params = this.props.navigation.state.params,
                ...
                _configListDetail =
                    ConfigListDetail.value[ScreenName.HreDocumentManageViewDetail] != null
                        ? ConfigListDetail.value[ScreenName.HreDocumentManageViewDetail]
                        : configDefault;

            let _dataItem = dataItem;
            if (dataItem.dataGroupMaster && dataItem.dataGroupMaster.length > 0) {
                _dataItem = {
                    ...dataItem,
                    ...dataItem.dataGroupMaster[0]
                };
            }

            if (!Vnr_Function.CheckIsNullOrEmpty(dataItem)) {
                this.setState({ configListDetail: _configListDetail, dataItem: _dataItem });
            } else {
                ...
                <View style={styleSheets.container}>
                    <ScrollView style={contentScroll} showsVerticalScrollIndicator={false}>
                        <View style={containerItemDetail}>
                            ...
                            {dataItem.dataGroupMaster &&
                                dataItem.dataGroupMaster.map(item => {
                                    return [
                                        {
                                            TypeView: 'E_GROUP',
                                            DisplayKey: item.TypeOfReqDocumentName ? item.TypeOfReqDocumentName : '',
                                            DataType: 'string'
                                        },
                                        {
                                            Name: 'StatusView',
