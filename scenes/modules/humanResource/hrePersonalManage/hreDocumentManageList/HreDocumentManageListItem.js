...
export default class HrePersonalManageListItem extends React.Component {
    ...
    render() {
        ...
        const { dataGroupMaster } = dataItem;
        let _dataItem = dataItem;

        let countSibmited = 0,
            valueProgress = 0;

        if (dataGroupMaster && dataGroupMaster.length > 0) {
            _dataItem = {
                ..._dataItem,
                ...dataGroupMaster[0]
            };

            dataGroupMaster.map(item => {
                if (item.Status && item.Status !== 'E_NOTYETSUBMITTED') countSibmited += 1;
            });

            valueProgress = (countSibmited / dataGroupMaster.length) * 100;
        }
                    ...
                    <View style={styles.container}>
                        <View style={styles.styViewTop}>
                            ...
                            <View style={styles.contentCenter}>
                                ...
                                {_dataItem.OrgStructureName != null && (
                                    <Text numberOfLines={1} style={[styleSheets.text, styles.positionText]}>
                                        {_dataItem.OrgStructureName ? _dataItem.OrgStructureName : ''}
                                    </Text>
                                )}
                            </View>

                            {/* {this.renderStatusView(_dataItem, renderConfig)} */}
                        </View>
                        ...
                        <View style={styles.styProgress}>
                            ...
                            <View style={styles.styRightProgress}>
                                <Text style={styles.styTextValProgress}>
                                    {`${translate('HRM_System_AsynTask_PercentComplete')}: (${countSibmited}/${
                                        dataGroupMaster ? dataGroupMaster.length : 0
                                    })`}
                                </Text>
                            </View>
                        </View>

                        <View style={styles.styViewList}>
                            {dataGroupMaster &&
                                dataGroupMaster.length > 0 &&
                                dataGroupMaster.map((item, index) => {
                                    const col = {
                                        TypeView: 'E_COMMON',
                                        Name: 'StatusView',
                                        DisplayKey: item.TypeOfReqDocumentName,
                                        DataType: 'string'
                                    };

                                    return <VnrFormatStringTypeItem key={index} data={item} col={col} allConfig={_renderConfig} />;
                                })}
                        </View>
                    </View>
                </View>
