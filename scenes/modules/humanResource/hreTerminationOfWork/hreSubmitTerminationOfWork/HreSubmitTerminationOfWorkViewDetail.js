...
const configLeveApprove = [
    {
        TypeView: 'UserSubmit',
        Name: 'UserInfoName',
        DisplayKey: 'HRM_PortalApp_TerminationOfWork_ResigningEmployee',
        DataType: 'string'
    },
    {
...
export default class HreSubmitTerminationOfWorkViewDetail extends Component {
    constructor(props) {
        super(props);
        this.state = {
            dataItem: null,
            configListDetail: null,
            dataRowActionAndSelected: generateRowActionAndSelected(ScreenName.HreSubmitTerminationOfWork),
                ...
                if (response && response.Status == EnumName.E_SUCCESS) {
                    ...
                    if (!dataItem?.BusinessAllowAction) {
                        data.BusinessAllowAction = Vnr_Services.handleStatus(
                            data.Status,
                            data?.SendEmailStatus ? data?.SendEmailStatus : false
                        );
