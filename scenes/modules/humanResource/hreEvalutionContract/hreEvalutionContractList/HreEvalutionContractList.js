...
import HreEvalutionContractListItem from './HreEvalutionContractListItem';
import { ConfigListFilter } from '../../../../../assets/configProject/ConfigListFilter';
import Vnr_Services from '../../../../../utils/Vnr_Services';
import { ToasterSevice } from '../../../../../components/Toaster/Toaster';
const heightActionBottom = 45;

export default class HreEvalutionContractList extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            //valueField: "ID",
            ...
            getDataLocal(keyDataLocal)
                .then((resData) => {
                    ...
                    if (res && res !== EnumName.E_EMPTYDATA) {
                        ...
                        // API lúc Data : {}, lúc Data : []
                        if (res.Data && Array.isArray(res.Data)) res = res.Data;
                        else if (res.Data && typeof res.Data == 'object') res = res.Data;
                        ...
                        data.forEach((item) => {
                            item.isSelect = false;
                            //item.BusinessAllowAction = Vnr_Services.handleStatus(item.Status, item?.SendEmailStatus ? item?.SendEmailStatus : false);
                            item.itemStatus = Vnr_Services.formatStyleStatusApp(item.StatusPerFormance);
                            item.itemStatus2 = Vnr_Services.formatStyleStatusApp(item.Overdue);
    ...
    renderTitleGroup = (data, index) => {
        const EnumName = {
            E_NEW: 'ONBOARDING',
            E_QUIT: 'OFFBOARDING',
            E_REJECTOFFER: 'CANCEL ONBOARDING'
        };
        return (
            <ScrollView>
                {data.map((item, id) => {
            ...
            <ScrollView>
                {data.map((item, index) => {
                    return (
                        <View
                            key={index}
                            style={[styles.containBotton, index === data.length - 1 && { marginBottom: 12 }]}
                        >
                            <View style={styles.styleViewBorderButtom}>
                                <TouchableWithoutFeedback>
                                    <View style={{ flex: 1 }}>
                                        <HreEvalutionContractListItem
                                            key={index}
    ...
    render() {
        ...
        if (isLoading) {
            let typeLoading = EnumStatus.E_APPROVE;
                            ...
                            {item?.isGroup ? (
                                ...
                                <View
                                    style={[
                                        ...
                                        index === dataSource.length - 1 && { marginBottom: 12 }
                                    ]}
                                >
                                    <View style={styles.styleViewBorderButtom}>
                                        <View style={{ flex: 1 }}>
