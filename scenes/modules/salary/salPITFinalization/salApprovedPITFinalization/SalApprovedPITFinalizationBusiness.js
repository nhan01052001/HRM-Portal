...
const enumName = EnumName,
    ...
    SalApprovedPITFinalizationAddOrEdit = ScreenName.SalApprovedPITFinalizationAddOrEdit;
...
export const generateRowActionAndSelected = () => {
    ...
    if (ConfigList.value != null && ConfigList.value != undefined) {
        const _configList = ConfigList.value[SalApprovedPITFinalization],
            ...
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    onPress: (item, dataBody) =>
                        SalApprovedPITFinalizationBusinessFunction.businessSendMailRecords([{ ...item }], dataBody),
                    ...actionSendMail
                }
            ];
            ...
            _selected = [
                ..._selected,
                {
                    ...
                    onPress: (items, dataBody) =>
                        SalApprovedPITFinalizationBusinessFunction.businessSendMailRecords(items, dataBody),
                    ...actionSendMail
                }
            ];
        }
        ...
        if (actionEditPer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_MODIFY,
                    onPress: (item) => SalApprovedPITFinalizationBusinessFunction.businessModifyRecord({ ...item }),
                    ...actionEdit
                }
            ];
        }
        ...
        if (actionDeletePer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_DELETE,
                    onPress: (item, dataBody) =>
                        SalApprovedPITFinalizationBusinessFunction.businessDeleteRecords([{ ...item }], dataBody),
                    ...actionDelete
                }
            ];
        ...
        if (actionCancelPer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_CANCEL,
                    onPress: (item, dataBody) =>
                        SalApprovedPITFinalizationBusinessFunction.businessCancelRecords([{ ...item }], dataBody),
                    ...actionCancel
                }
            ];
...
export const SalApprovedPITFinalizationBusinessFunction = {
                            ...
                            HttpService.Post('[URI_HR]/Att_GetData/BusinessAllowActionSendEmail', {
                                ...
                                try {
                                    if (res && typeof res === enumName.E_object) {
                                        ...
                                        if (isValid == true) {
                                            SalApprovedPITFinalizationBusinessFunction.confirmSendMail(res);
                                        } else if (
                                            isValid == false &&
                                            res.message &&
                            ...
                            HttpService.Post('[URI_HR]/Att_GetData/BusinessAllowActionDelete', {
                                ...
                                try {
                                    if (res && typeof res === enumName.E_object) {
                                        ...
                                        if (isValid) {
                                            SalApprovedPITFinalizationBusinessFunction.confirmDelete(res);
                                        }
                                        //không hợp lệ
                                        else if (
                                            isValid == false &&
            ...
            AlertSevice.alert({
                ...
                onConfirm: (reason) => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        SalApprovedPITFinalizationBusinessFunction.setIsDelete({ Ids: strId, reason });
                    }
                }
            });
        } else {
            let strId = objValid.strResultID;
            SalApprovedPITFinalizationBusinessFunction.setIsDelete({ Ids: strId });
        }
    },
        ...
        HttpService.Post('[URI_HR]/Att_GetData/GetRegisterVehicleById', {
            ...
            try {
                if (res && res.BusinessAllowAction && res.BusinessAllowAction.indexOf('E_MODIFY') >= 0) {
                    const { reload } = _this;
                    _this.props.navigation.navigate(SalApprovedPITFinalizationAddOrEdit, { record: item, reload });
                } else {
                    ToasterSevice.showWarning('StatusNotAction');
                }
            } catch (error) {
    ...
    businessCancelRecords: (items, dataBody) => {
        if (items.length === 0 && !dataBody) {
            ...
            //     VnrLoadingSevices.show();
            //     // need fix [done]
            //     HttpService.Post(`[URI_HR]/Att_GetData/GetRegisterVehicleById`, { id: selectedID[0], screenName: SalApprovedPITFinalization })
            //         .then(res => {
            //             VnrLoadingSevices.hide();
            //             try {
            ...
            //                                     if (isValid) {
            //                                         SalApprovedPITFinalizationBusinessFunction.confirmCancel(res);
            //                                     }
            //                                     //không hợp lệ
            //                                     else if (isValid == false && res.message && typeof res.message === enumName.E_string) {
            //                                         ToasterSevice.showWarning(res.message, 4000);
    ...
    confirmCancel: (objValid) => {
        ...
        if (isConfirm) {
            ...
            AlertSevice.alert({
                ...
                onConfirm: (reason) => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        let strId = objValid.strResultID;
                        SalApprovedPITFinalizationBusinessFunction.setStatusCancel({ Ids: strId, reason });
                    }
                }
            });
        } else {
            //check khung giờ cho Phổ Đình
            let strId = objValid.strResultID;
            SalApprovedPITFinalizationBusinessFunction.setStatusCancel({ Ids: strId });
        }
    },
