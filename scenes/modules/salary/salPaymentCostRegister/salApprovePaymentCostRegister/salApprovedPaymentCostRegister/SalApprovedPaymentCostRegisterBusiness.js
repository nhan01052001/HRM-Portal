...
export const generateRowActionAndSelected = () => {
    ...
    if (ConfigList.value != null && ConfigList.value != undefined) {
        const _configList = ConfigList.value[attApprovedLeaveDay],
            ...
            _rowActions = [
                {
                    ...
                    onPress: (item, dataBody) =>
                        SalApprovedPaymentCostRegisterBusiness.businessCancelRecords([{ ...item }], dataBody),
                    ...actionCancel
                }
            ];
            ...
            _selected = [
                {
                    ...
                    onPress: (items, dataBody) =>
                        SalApprovedPaymentCostRegisterBusiness.businessCancelRecords(items, dataBody),
                    ...actionCancel
                }
            ];
        }
...
export const SalApprovedPaymentCostRegisterBusiness = {
            ...
            HttpService.Post('[URI_HR]/Att_GetData/BusinessAllowActionCancel', {
                ...
                try {
                    if (res && typeof res === enumName.E_object) {
                        ...
                        if (isValid) {
                            SalApprovedPaymentCostRegisterBusiness.confirmCancel(res);
                        }
                        //không hợp lệ
    ...
    confirmCancel: objValid => {
        ...
        if (isConfirm) {
            ...
            AlertSevice.alert({
                ...
                onConfirm: reason => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        SalApprovedPaymentCostRegisterBusiness.checkingDataWorkingForLeaveDayCancel({
                            Ids: strId,
                            reason
                        });
                    }
            ...
            //check khung giờ cho Phổ Đình
            let strId = objValid.strResultID;
            SalApprovedPaymentCostRegisterBusiness.checkingDataWorkingForLeaveDayCancel({ Ids: strId });
        }
    },
        ...
        HttpService.Post('[URI_HR]/Att_GetData/CheckingValidateEmployeeWorking', {
            ...
            try {
                if (res && Array.isArray(res)) {
                    //dữ liệu hợp lệ
                    if (res[0] === 'success') {
                        SalApprovedPaymentCostRegisterBusiness.setStatusCancel(objValid);
                    }
                    //khung giờ không hợp lệ => gọi hàm mở popup để review
                    else if (res[0] === 'error') {
                        SalApprovedPaymentCostRegisterBusiness.confirmEmpWorkingForCancel(res, objValid);
                    }
                }
                //FAIL
                else {
