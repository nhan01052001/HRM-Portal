...
//import { generateRowActionAndSelected, TaxSubmitTaxInformationRegisterBusinessFunction } from '../taxSubmitTaxInformationRegister/TaxSubmitTaxInformationRegisterBusiness';
import {
...
export default class TaxApproveTaxInformationRegisterViewDetail extends Component {
    constructor(props) {
        super(props);
    ...
    getDataItem = async (isReload = false) => {
        try {
            ...
            if (!Vnr_Function.CheckIsNullOrEmpty(dataId) || isReload) {
                ...
                if (response?.Status) {
                    let listBusinessAllowAction = [];
                    if (response.Status.toUpperCase() != 'E_REJECTED' && response.Status.toUpperCase() != 'E_REJECT') {
                        listBusinessAllowAction.push('E_MODIFY');
                    }

                    if (
                        response.Status.toUpperCase() != 'E_REJECTED' &&
                        response.Status.toUpperCase() != 'E_REJECT' &&
                        response.Status.toUpperCase() != 'E_VALID' &&
                        response.Status.toUpperCase() != 'E_TRANSFERRED'
                    ) {
                        listBusinessAllowAction.push('E_SENDMAIL');
                    }

                    if (
                        response.Status.toUpperCase() == 'E_PENDINGSEND' ||
                        response.Status.toUpperCase() == 'E_SUBMITTEMP'
                    ) {
                        listBusinessAllowAction.push('E_DELETE');
                    }

                    if (listBusinessAllowAction.length > 0) {
                        response.BusinessAllowAction = listBusinessAllowAction.join();
                    }
                }
    ...
    reload = (E_KEEP_FILTER, actionIsDelete) => {
        ...
        if (actionIsDelete) {
            DrawerServices.navigate('TaxApproveTaxInformationRegister');
        } else {
    ...
    componentDidMount() {
        TaxApproveTaxInformationRegisterBusinessFunctionisterList.setThisForBusiness(this, true);
        this.getDataItem();
    }
