...
import TaxInformationRegisterListItem from './TaxInformationRegisterListItem';
import TaxInformationRegisterListItemApprove from './TaxInformationRegisterListItemApprove';
import { ConfigListFilter } from '../../../../../assets/configProject/ConfigListFilter';
const heightActionBottom = 45;

export default class TaxInformationRegisterList extends React.Component {
    constructor(props) {
        super(props);
            ...
            getDataLocal(keyDataLocal)
                .then((resData) => {
                    ...
                    if (res && res !== EnumName.E_EMPTYDATA) {
                        ...
                        // if (detail.screenName === ScreenName.TaxApproveTaxInformationRegister) {
                        //     data.map(item => {
                        //         let arrStatus = [];
                        //         if (item.Status) {
                        ...
                        //                 status !== 'E_REJECT' &&
                        //                 status !== 'E_VALID') {
                        //                 arrStatus.push('E_MODIFY');
                        //             }

                        //             if (status !== 'E_REJECTED' &&
                        //                 status !== 'E_REJECT' &&
                        //                 status !== 'E_VALID' &&
                        //                 status !== 'E_TRANSFERRED') {
                        //                 arrStatus.push('E_SENDMAIL');
                        //             }

                        //             if (status === 'E_PENDINGSEND' ||
                        //                 status === 'E_SUBMITTEMP') {
                        //                 arrStatus.push('E_DELETE');
                        //             }

                        //             item.BusinessAllowAction = arrStatus.join();
                        //         }
                        //     });
                        // } else if (detail.screenName === ScreenName.TaxApprovedTaxInformationRegister) {
                        //     data.map(item => {
                        //         let arrStatus = [];
                        //         if (item.Status) {
                        //             if (item.Status === 'E_PENDINGSEND' ||
                        //                 item.Status === 'E_SUBMIT_TEMP') {
                        //                 arrStatus.push('E_DELETE');
                        //             }
    ...
    _renderFooterLoading = () => {
        return (
            <View style={styles.styFooterLoaading}>
                            ...
                            <View style={styles.styleViewBorderButtom}>
                                ...
                                >
                                    <View style={CustomStyleSheet.flex(1)}>
                                        {detail.screenName == ScreenName.TaxApproveTaxInformationRegister ||
                                        detail.screenName == ScreenName.TaxApprovedTaxInformationRegister ? (
                                                <TaxInformationRegisterListItemApprove
                                                ...
                                                <TaxInformationRegisterListItem
                                                    currentDetail={detail}
                                                    onClick={() => {
                                                        isOpenAction == true ? this.addItemChecked(index) : null;
                                                    }}
...
const styles = StyleSheet.create({
    styFooterLoaading: { flex: 1, paddingVertical: styleSheets.p_10, marginBottom: 30 },
    containBotton: {
        flex: 1
    },
    containerGrey: {
