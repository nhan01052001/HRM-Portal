...
export default class AttSubmitLateEarlyAllowedViewDetail extends Component {
    constructor(props) {
        super(props);
    ...
    getDataItem = async (isReload = false) => {
        try {
            ...
            if (!Vnr_Function.CheckIsNullOrEmpty(dataId) || isReload) {
                const response = await HttpService.Post('[URI_HR]/Att_GetData/GetLateEarlyAllowedById', {
    ...
    reload = (E_KEEP_FILTER, actionIsDelete) => {
        ...
        if (actionIsDelete) {
            DrawerServices.navigate('AttSubmitLateEarlyAllowed');
        } else {
    ...
    componentDidMount() {
        AttSubmitLateEarlyAllowedBusinessFunction.setThisForBusiness(this, true);
        this.getDataItem();
    }
