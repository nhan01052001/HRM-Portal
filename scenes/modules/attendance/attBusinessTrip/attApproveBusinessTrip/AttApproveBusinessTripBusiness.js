...
let enumName = EnumName,
    attApproveBusinessTrip = ScreenName.AttApproveBusinessTrip,
...
export const generateRowActionAndSelected = () => {
    ...
    if (ConfigList.value != null && ConfigList.value != undefined) {
        const _configList = ConfigList.value[attApproveBusinessTrip],
            ...
            _rowActions = [
                {
                    ...
                    onPress: (item, dataBody) =>
                        AttApproveBusinessTripBusinessFunction.businessApproveRecords([{ ...item }], dataBody),
                    ...actionApprove
                }
            ];
        ...
        if (actionRejectPer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_REJECT,
                    onPress: (item, dataBody) =>
                        AttApproveBusinessTripBusinessFunction.businessRejectRecords([{ ...item }], dataBody),
                    ...actionReject
                }
            ];
        ...
        if (actionCancelPer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_CANCEL,
                    onPress: (item, dataBody) =>
                        AttApproveBusinessTripBusinessFunction.businessCancelRecords([{ ...item }], dataBody),
                    ...actionCancel
                }
            ];
            ...
            _selected = [
                ..._selected,
                {
                    ...
                    onPress: (items, dataBody) =>
                        AttApproveBusinessTripBusinessFunction.businessCancelRecords(items, dataBody),
                    ...actionCancel
                }
            ];
        }
...
export const AttApproveBusinessTripBusinessFunction = {
    ...
    businessCancelRecords: (items, dataBody) => {
        if (items.length === 0 && !dataBody) {
            ...
            HttpService.Post('[URI_HR]/Att_GetData/BusinessAllowActionCancel', {
                selectedID,
                screenName: ScreenName.AttApprovedBusinessTrip,
                ...
                try {
                    if (res && typeof res === enumName.E_object) {
                        ...
                        if (isValid) {
                            AttApproveBusinessTripBusinessFunction.confirmCancel(res);
                        }
                        //không hợp lệ
    ...
    confirmCancel: objValid => {
        ...
        if (isConfirm) {
            ...
            AlertSevice.alert({
                ...
                onConfirm: reason => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        AttApproveBusinessTripBusinessFunction.checkingDataWorkingForOTCancel({ Ids: strId, reason });
                    }
                }
            });
        } else {
            //check khung giờ cho Phổ Đình
            let strId = objValid.strResultID;
            AttApproveBusinessTripBusinessFunction.checkingDataWorkingForOTCancel({ Ids: strId });
        }
    },
        ...
        HttpService.Post('[URI_HR]/Att_GetData/CheckingValidateEmployeeWorkingForOvertime', {
            ...
            try {
                if (res && Array.isArray(res)) {
                    //dữ liệu hợp lệ
                    if (res[0] === 'success') {
                        AttApproveBusinessTripBusinessFunction.setStatusCancel(objValid);
                    }
                    //khung giờ không hợp lệ => gọi hàm mở popup để review
                    else if (res[0] === 'error') {
                        AttApproveBusinessTripBusinessFunction.confirmEmpWorkingForCancel(res, objValid);
                    }
                }
                //FAIL
                else {
            ...
            try {
                if (res && res !== '') {
                    if (res == enumName.E_Success) {
                        ...
                        AttApproveBusinessTripBusinessFunction.checkForReLoadScreen[
            ...
            HttpService.Post('[URI_HR]/Att_GetData/BusinessAllowActionApprove', {
                ...
                try {
                    if (res && typeof res === enumName.E_object) {
                        ...
                        if (isValid) {
                            AttApproveBusinessTripBusinessFunction.confirmApprove(res);
                        }
                        //không hợp lệ
                        else if (isValid == false && res.message && typeof res.message === enumName.E_string) {
                            ToasterSevice.showWarning(res.message, 4000);
            ...
            AlertSevice.alert({
                ...
                onConfirm: reason => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        let strId = objValid.strResultID;
                        AttApproveBusinessTripBusinessFunction.setStatusApprove({ Ids: strId, reason });
                    }
                }
            });
        } else {
            let strId = objValid.strResultID;
            AttApproveBusinessTripBusinessFunction.setStatusApprove({ Ids: strId });
        }
    },
    ...
    setStatusApprove: objValid => {
        ...
        VnrLoadingSevices.show();
        HttpService.Post('[URI_HR]/Att_GetData/SetApproveBusinessTripInPortal', dataBody).then(data => {
            VnrLoadingSevices.hide();
            if (data && typeof data === 'string') {
                if (data == enumName.E_Success) {
                    ...
                    if (!_isOnScreenNotification) DrawerServices.navigate(ScreenName.AttApproveBusinessTrip);
            ...
            HttpService.Post('[URI_HR]/Att_GetData/BusinessAllowActionReject', {
                ...
                try {
                    if (res && typeof res === 'object') {
                        ...
                        //dữ liệu hợp lệ => gọi hàm confirm Hủy
                        if (isValid) {
                            AttApproveBusinessTripBusinessFunction.confirmReject(res);
                        }
                        //không hợp lệ
            ...
            AlertSevice.alert({
                ...
                onConfirm: reason => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        let strId = objValid.strResultID;
                        AttApproveBusinessTripBusinessFunction.setStatusReject({ Ids: strId, reason });
                    }
                }
            });
        } else {
            let strId = objValid.strResultID;
            AttApproveBusinessTripBusinessFunction.setStatusReject({ Ids: strId });
        }
    },
    ...
    setStatusReject: objValid => {
        ...
        VnrLoadingSevices.show();
        HttpService.Post('[URI_HR]/Att_GetData/SetRejectBusinessTripInPortal', {
            host: apiConfig.uriPor,
            selectedIds,
            userProcessID: headers.userid,
            DeclineReason
...
export default {
    ...
    AttApproveBusinessTripBusinessFunction: AttApproveBusinessTripBusinessFunction
};
