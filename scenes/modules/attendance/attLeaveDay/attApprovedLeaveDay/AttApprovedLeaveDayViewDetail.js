...
import { AttApprovedLeaveDayBusinessFunction, generateRowActionAndSelected } from './AttApprovedLeaveDayBusiness';
...
export default class AttApprovedLeaveDayViewDetail extends Component {
    constructor(props) {
        super(props);
        this.state = {
            dataItem: null,
    ...
    getDataItem = async (isReload = false) => {
        try {
            ...
            if (!Vnr_Function.CheckIsNullOrEmpty(dataId) || isReload) {
                const response = await HttpService.Post('[URI_HR]/Att_GetData/GetLeaveDayById', {
    ...
    reload = () => {
        ...
        DrawerServices.navigate('AttApprovedLeaveDay');
    };
