...
let enumName = EnumName,
    attApproveLeaveDay = ScreenName.AttApproveLeaveDay,
    apiConfig = null,
    headers = null;
...
export const generateRowActionAndSelected = () => {
    ...
    if (ConfigList.value != null && ConfigList.value != undefined) {
        const _configList = ConfigList.value[attApproveLeaveDay],
            ...
            _rowActions = [
                {
                    ...
                    onPress: (item, dataBody) =>
                        AttApproveLeaveDayBusinessFunction.businessApproveRecords([{ ...item }], dataBody),
                    ...actionApprove
                }
            ];
        ...
        if (actionRejectPer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_REJECT,
                    onPress: (item, dataBody) =>
                        AttApproveLeaveDayBusinessFunction.businessRejectRecords([{ ...item }], dataBody),
                    ...actionReject
                }
            ];
...
export const AttApproveLeaveDayBusinessFunction = {
            ...
            HttpService.Post('[URI_HR]/Att_GetData/BusinessAllowActionReject', {
                ...
                try {
                    if (res && typeof res === 'object') {
                        ...
                        if (isValid) {
                            AttApproveLeaveDayBusinessFunction.confirmReject(res);
                        }
                        //không hợp lệ
    ...
    confirmReject: objValid => {
        ...
        if (isConfirm) {
            ...
            AlertSevice.alert({
                ...
                onConfirm: reason => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        AttApproveLeaveDayBusinessFunction.checkingDataWorkingForLeaveDayReject({ Ids: strId, reason });
                    }
                }
            });
        } else {
            //check khung giờ cho Phổ Đình
            let strId = objValid.strResultID;
            AttApproveLeaveDayBusinessFunction.checkingDataWorkingForLeaveDayReject({ Ids: strId });
        }
    },

    checkingDataWorkingForLeaveDayReject: objValid => {
        ...
        HttpService.Post('[URI_HR]/Att_GetData/CheckingValidateEmployeeWorking', {
            ...
            try {
                if (res && Array.isArray(res)) {
                    //dữ liệu hợp lệ
                    if (res[0] === 'success') {
                        AttApproveLeaveDayBusinessFunction.setStatusReject(objValid);
                    }
                    //khung giờ không hợp lệ => gọi hàm mở popup để review
                    else if (res[0] === 'error') {
                        AttApproveLeaveDayBusinessFunction.confirmEmpWorkingForReject(res, objValid);
                    }
                }
                //FAIL
                else {
    ...
    setStatusReject: objValid => {
        ...
        VnrLoadingSevices.show();
        HttpService.Post('[URI_HR]/Att_GetData/SetRejectLeaveDayInPortal', {
            ...
            try {
                if (res && res !== '') {
                    if (res === enumName.E_Success) {
                        ...
                        AttApproveLeaveDayBusinessFunction.checkForReLoadScreen[ScreenName.AttApprovedLeaveDay] = true;
                        ...
                        if (!_isOnScreenNotification) DrawerServices.navigate('AttApproveLeaveDay');
            ...
            HttpService.Post('[URI_HR]/Att_GetData/BusinessAllowActionApprove', {
                ...
                try {
                    if (res && typeof res === enumName.E_object) {
                        ...
                        if (isValid) {
                            AttApproveLeaveDayBusinessFunction.confirmApprove(res);
                        }
                        //không hợp lệ
    ...
    confirmApprove: objValid => {
        ...
        if (isConfirm) {
            ...
            AlertSevice.alert({
                ...
                onConfirm: reason => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        //check khung giờ cho Phổ Đình
                        let strId = objValid.strResultID;
                        AttApproveLeaveDayBusinessFunction.checkingDataWorkingForLeaveDayApprove({
                            Ids: strId,
                            reason
                        });
                    }
            ...
            //check khung giờ cho Phổ Đình
            let strId = objValid.strResultID;
            AttApproveLeaveDayBusinessFunction.checkingDataWorkingForLeaveDayApprove({ Ids: strId });
        }
    },

    checkingDataWorkingForLeaveDayApprove: objValid => {
        let selectedID = objValid.Ids.split(',');
        ...
        HttpService.Post('[URI_HR]/Att_GetData/CheckingValidateEmployeeWorking', {
            ...
            try {
                if (res && Array.isArray(res)) {
                    //dữ liệu hợp lệ
                    if (res[0] === 'success') {
                        AttApproveLeaveDayBusinessFunction.checkApproveLeavedayDataLocked(objValid);
                    }
                    //khung giờ không hợp lệ => gọi hàm mở popup để review
                    else if (res[0] === 'error') {
                        AttApproveLeaveDayBusinessFunction.confirmEmpWorkingForApprove(res, objValid);
                    }
                }
                //FAIL
                else {
    ...
    checkApproveLeavedayDataLocked: objValid => {
        let strId = objValid.Ids.split(',');

        VnrLoadingSevices.show();
        HttpService.Post('[URI_HR]/Att_GetData/CheckApproveLeavedayDataLocked', { selectedIds: strId }).then(res => {
            VnrLoadingSevices.hide();
            try {
                if (res) {
                    ...
                    if (
                        ...
                        AttApproveLeaveDayBusinessFunction.setStatusApprove(objValid);
                    }
                    //khóa
                    else if (
                        ...
                        AttApproveLeaveDayBusinessFunction.checkAllowApproveDataLocked(objValid);
                    } else {
                        ToasterSevice.showError('HRM_Common_SendRequest_Error', 5000);
                    }
                } else {
    ...
    checkAllowApproveDataLocked: objValid => {
        ...
        HttpService.Post('[URI_HR]/Sal_GetData/GetConfigElement', {
            key: 'HRM_ATT_CONFIG_ISALLOWAPPROVEDATALOCKEDANDTRANFERCUTOFDUATIONLEAVEDAY'
        }).then(res => {
            VnrLoadingSevices.hide();
            try {
                if (res) {
    ...
    setStatusApprove: objValid => {
        ...
        VnrLoadingSevices.show();
        HttpService.Post('[URI_HR]/Att_GetData/SetApproveLeaveDayInPortal', {
            host: apiConfig.uriPor,
            selectedIds,
            userProcessID: headers.userid,
            approveComment,
...
export default {
    ...
    AttApproveLeaveDayBusinessFunction: AttApproveLeaveDayBusinessFunction
};
