...
const enumName = EnumName,
    attSubmitBusinessTravel = ScreenName.AttSubmitBusinessTravel,
    attSubmitBusinessTravelAddOrEdit = ScreenName.AttSubmitBusinessTravelAddOrEdit;
...
export const generateRowActionAndSelected = () => {
    ...
    if (ConfigList.value != null && ConfigList.value != undefined) {
        const _configList = ConfigList.value[attSubmitBusinessTravel],
            ...
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    onPress: (item, dataBody) =>
                        AttSubmitBusinessTravelBusinessFunction.businessModifyRecord({ ...item }, dataBody),
                    ...actionEdit
                }
            ];
        }
        ...
        if (actionSendMailPer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_SENDMAIL,
                    onPress: (item, dataBody) =>
                        AttSubmitBusinessTravelBusinessFunction.businessSendMailRecords([{ ...item }], dataBody),
                    ...actionSendMail
                }
            ];
            ...
            _selected = [
                ..._selected,
                {
                    ...
                    onPress: (items, dataBody) =>
                        AttSubmitBusinessTravelBusinessFunction.businessSendMailRecords(items, dataBody),
                    ...actionSendMail
                }
            ];
        }
        ...
        if (actionDeletePer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_DELETE,
                    onPress: (item, dataBody) =>
                        AttSubmitBusinessTravelBusinessFunction.businessDeleteRecords([{ ...item }], dataBody),
                    ...actionDelete
                }
            ];
            ...
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    onPress: (item, dataBody, isShowMultiRecord) =>
                        AttSubmitBusinessTravelBusinessFunction.businessCancelRecords(
                            [{ ...item }],
                            dataBody,
                            isShowMultiRecord
                        ),
        ...
        if (actionRequestCancelPer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    onPress: (item) =>
                        AttSubmitBusinessTravelBusinessFunction.businessRequestCancelRecords({ ...item }),
                    ...actionRequestCancel
                }
            ];
        }
...
export const AttSubmitBusinessTravelBusinessFunction = {
    ...
    businessDeleteRecords: (items, dataBody) => {
        if (items.length === 0 && !dataBody) {
            ...
            if (selectedID.length === 1) {
                ...
                HttpService.Post('[URI_HR]/Att_GetData/GetBusinessTravelById', {
                    id: selectedID[0],
                            ...
                            HttpService.Post('[URI_HR]/Att_GetData/BusinessAllowActionDelete', {
                                ...
                                try {
                                    if (res && typeof res === enumName.E_object) {
                                        ...
                                        if (isValid) {
                                            AttSubmitBusinessTravelBusinessFunction.confirmDelete(res);
                                        }
                                        //không hợp lệ
                                        else if (
                                            isValid == false &&
    ...
    confirmDelete: (objValid) => {
        ...
        if (isConfirm) {
            ...
            AlertSevice.alert({
                ...
                onConfirm: (reason) => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        AttSubmitBusinessTravelBusinessFunction.setIsDelete({ Ids: strId, reason });
                    }
                }
            });
        } else {
            //check khung giờ cho Phổ Đình
            let strId = objValid.strResultID;
            AttSubmitBusinessTravelBusinessFunction.setIsDelete({ Ids: strId });
        }
    },
    ...
    setIsDelete: (objValid) => {
        ...
        HttpService.Post('[URI_POR]/New_Att_BussinessTravel/RemoveSelected', {
            selectedIds,
            reason
        }).then((res) => {
            VnrLoadingSevices.hide();
                            ...
                            HttpService.Post('[URI_HR]/Att_GetData/BusinessAllowActionSendEmail', {
                                ...
                                try {
                                    if (res && typeof res === enumName.E_object) {
                                        let isValid = res.isValid;
                                        if (isValid == true) {
                                            AttSubmitBusinessTravelBusinessFunction.confirmSendMail(res);
                                        } else if (
                                            isValid == false &&
                                            res.message &&
                                            typeof res.message === enumName.E_string
            ...
            AlertSevice.alert({
                ...
                onConfirm: (reason) => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        HttpService.Post('[URI_HR]/Att_GetData/ProcessSendMailBusinessTravel', {
                            host: _uriPor,
                            selectedIds: arrId,
                            reason
                        }).then((res) => {
                            ...
                            HttpService.Post('[URI_HR]/Att_GetData/BusinessAllowActionCancel', {
                                ...
                                try {
                                    if (res && typeof res === enumName.E_object) {
                                        ...
                                        if (isValid) {
                                            AttSubmitBusinessTravelBusinessFunction.confirmCancel(res);
                                        }
                                        //không hợp lệ
                                        else if (
                                            isValid == false &&
    ...
    confirmCancel: (objValid) => {
        ...
        if (isConfirm) {
            ...
            AlertSevice.alert({
                ...
                onConfirm: (reason) => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        //check khung giờ cho Phổ Đình
                        let strId = objValid.strResultID;
                        AttSubmitBusinessTravelBusinessFunction.setStatusCancel({ Ids: strId, reason });
                    }
                }
            });
        } else {
            //check khung giờ cho Phổ Đình
            let strId = objValid.strResultID;
            AttSubmitBusinessTravelBusinessFunction.setStatusCancel({ Ids: strId });
        }
    },
    ...
    setStatusCancel: (objValid) => {
        ...
        VnrLoadingSevices.show();
        HttpService.Post('[URI_HR]/Att_GetData/ChangeStatusCancelBusinessTravel', {
            selectedIds,
            host: _uriPor,
            commentCancel,
        ...
        HttpService.Post('[URI_HR]/Att_GetData/GetBusinessTravelById', {
            ...
            try {
                if (res && res.BusinessAllowAction && res.BusinessAllowAction.indexOf('E_MODIFY') >= 0) {
                    const { reload } = _this;
                    _this.props.navigation.navigate(attSubmitBusinessTravelAddOrEdit, { record: item, reload });
                } else {
                    ToasterSevice.showWarning('StatusNotAction');
                }
            } catch (error) {
        ...
        HttpService.Post('[URI_HR]/Att_GetData/BusinessAllowActionRequestCancel', {
            ...
            try {
                if (res && typeof res === enumName.E_object) {
                    ...
                    if (isValid) {
                        const { reload } = _this;
                        DrawerServices.navigate('AttSubmitBusinessTravelCancelAddOrEdit', {
                            record: item,
                            reload,
                            screenName: attSubmitBusinessTravel
                        });
