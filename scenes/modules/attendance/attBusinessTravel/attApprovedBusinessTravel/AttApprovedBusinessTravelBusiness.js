...
let enumName = EnumName,
    attApprovedBusinessTravel = ScreenName.AttApprovedBusinessTravel;
...
export const generateRowActionAndSelected = () => {
    ...
    if (ConfigList.value != null && ConfigList.value != undefined) {
        const _configList = ConfigList.value[attApprovedBusinessTravel],
            ...
            _rowActions = [
                {
                    ...
                    onPress: (item, dataBody) =>
                        AttApprovedBusinessTravelBusinessFunction.businessCancelRecords([{ ...item }], dataBody),
                    ...actionCancel
                }
            ];
            ...
            _selected = [
                {
                    ...
                    onPress: (items, dataBody) =>
                        AttApprovedBusinessTravelBusinessFunction.businessCancelRecords(items, dataBody),
                    ...actionCancel
                }
            ];
        }
...
export const AttApprovedBusinessTravelBusinessFunction = {
                ...
                HttpService.Post('[URI_HR]/Att_GetData/BusinessAllowActionCancel', {
                    ...
                    try {
                        if (res && typeof res === enumName.E_object) {
                            ...
                            if (isValid) {
                                AttApprovedBusinessTravelBusinessFunction.confirmCancel(res);
                            }
                            //không hợp lệ
    ...
    confirmCancel: objValid => {
        ...
        if (isConfirm) {
            ...
            AlertSevice.alert({
                ...
                onConfirm: reason => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        AttApprovedBusinessTravelBusinessFunction.checkingDataWorkingForOTCancel({
                            Ids: strId,
                            reason
                        });
                    }
            ...
            //check khung giờ cho Phổ Đình
            let strId = objValid.strResultID;
            AttApprovedBusinessTravelBusinessFunction.checkingDataWorkingForOTCancel({ Ids: strId });
        }
    },
        ...
        HttpService.Post('[URI_HR]/Att_GetData/CheckingValidateEmployeeWorkingForOvertime', {
            ...
            try {
                if (res && Array.isArray(res)) {
                    //dữ liệu hợp lệ
                    if (res[0] === 'success') {
                        AttApprovedBusinessTravelBusinessFunction.setStatusCancel(objValid);
                    }
                    //khung giờ không hợp lệ => gọi hàm mở popup để review
                    else if (res[0] === 'error') {
                        AttApprovedBusinessTravelBusinessFunction.confirmEmpWorkingForCancel(res, objValid);
                    }
                }
                //FAIL
                else {
