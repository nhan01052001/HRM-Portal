...
export default class ShiftDetail extends Component {
    ...
    SendForApprove = () => {
        ...
        if (
            ...
            if (checkInOutUpdate['checkOutTimeLog'] != undefined && checkInOutUpdate['checkOutTimeLog'] != null) {
                _dataBodyCheckOut.TimeLog = moment(checkInOutUpdate['checkOutTimeLog']).format(
                    'YYYY-MM-DD HH:mm:ss'
                );
            }

            if (checkInOutUpdate['checkOutTimeLogTime']) {
                _dataBodyCheckOut.TimeLogTime = moment(checkInOutUpdate['checkOutTimeLogTime']).format('HH:mm');
            }
                        ...
                        <View style={styles.container}>
                            ...
                            <View style={styles.ShiftTotalView}>
                                ...
                                <View style={styles.viewValueTotalHour}>
                                    <Text style={[styleSheets.lable, { ...CustomStyleSheet.fontSize(Size.text + 3), ...CustomStyleSheet.paddingBottom(0) }]}>
                                        {valueHoursTotal}
                                    </Text>
                                    ...
                                    <View style={styles.styleViewHoursContentModal}>
                                        ...
                                        <View style={styles.viewValueTotalHour}>
                                            <Text
                                                style={[
                                                    styleSheets.lable,
                                                    { ...CustomStyleSheet.fontSize(Size.text + 3), ...CustomStyleSheet.paddingBottom(0) }
                                                ]}
                                            >
                                                {valueHoursTotalUpdate}
                                            </Text>
...
const styles = StyleSheet.create({
    ...
    txtMinutesTotalUpdate: { fontSize: Size.text + 3, paddingLeft: 5, paddingBottom: 0 },
    safeAreaView: {
        flex: 1,
        borderTopLeftRadius: 15,
        borderTopRightRadius: 15
