...
export const generateRowActionAndSelected = () => {
    ...
    if (ConfigList.value != null && ConfigList.value != undefined) {
        const _configList = ConfigList.value[attApproveRosterGroupByEmp],
            ...
            _rowActions = [
                {
                    ...
                    onPress: (item, dataBody) => {
                        if (Array.isArray(item) && item.length > 0) {
                            AttApproveRosterGroupByEmpBusinessFunction.businessApproveRecords(item, dataBody);
                        } else {
                            AttApproveRosterGroupByEmpBusinessFunction.businessApproveRecords([{ ...item }], dataBody);
                        }
                    },
            ...
            _selected = [
                {
                    ...
                    onPress: (items, dataBody) =>
                        AttApproveRosterGroupByEmpBusinessFunction.businessApproveRecords(items, dataBody),
                    ...actionApprove
                }
            ];
        }
        ...
        if (actionRejectPer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    onPress: (item, dataBody) => {
                        if (Array.isArray(item) && item.length > 0) {
                            AttApproveRosterGroupByEmpBusinessFunction.businessRejectRecords(item, dataBody);
                        } else {
                            AttApproveRosterGroupByEmpBusinessFunction.businessRejectRecords([{ ...item }], dataBody);
                        }
                    },
            ...
            HttpService.Post('[URI_HR]/Att_GetData/BusinessAllowActionApprove', {
                ...
                try {
                    if (res && typeof res === 'object') {
                        ...
                        if (isValid) {
                            AttApproveRosterGroupByEmpBusinessFunction.confirmApprove(res);
            ...
            AlertSevice.alert({
                ...
                onConfirm: reason => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        VnrLoadingSevices.show();
                        HttpService.Post('[URI_HR]/Att_GetData/SetApproveRosterGroupByEmpInPortal', {
                            host: _uriPor,
                            selectedIds: strId,
                            userApproveID: null,
                            ...
                            try {
                                if (res && res !== '') {
                                    if (res == 'Success') {
                                        ...
                                        // set true để refresh AttApprovedRosterGroupByEmp
                                        AttApproveRosterGroupByEmpBusinessFunction.checkForReLoadScreen[
                                        ...
                                        if (!_isOnScreenNotification)
                                            DrawerServices.navigate('AttApproveRosterGroupByEmp');
                                    } else if (res == 'Locked') {
                                        ToasterSevice.showWarning('ObjectIsLockedV2', 5000);
                                    } else if (res.indexOf('FieldLengthError') >= 0) {
                                        ToasterSevice.showWarning(res.split('|')[1]);
                                    } else if (res.indexOf('VaildateMaxLengthError|') > -1) {
                ...
                try {
                    if (res && res !== '') {
                        if (res == 'Success') {
                            ...
                            // nếu duyệt ở màn hình notify thì không chuyển hướng.
                            if (!_isOnScreenNotification) DrawerServices.navigate('AttApproveRosterGroupByEmp');
                        } else if (res == 'Locked') {
                            ToasterSevice.showWarning('ObjectIsLockedV2', 5000);
                        } else if (res.indexOf('FieldLengthError') >= 0) {
                            ToasterSevice.showWarning(res.split('|')[1]);
            ...
            HttpService.Post('[URI_HR]/Att_GetData/BusinessAllowActionReject', {
                ...
                try {
                    if (res && typeof res === enumName.E_object) {
                        let isValid = res.isValid;
                        if (isValid) {
                            AttApproveRosterGroupByEmpBusinessFunction.confirmReject(res);
            ...
            AlertSevice.alert({
                ...
                onConfirm: reason => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        VnrLoadingSevices.show();
                        HttpService.Post('[URI_HR]/Att_GetData/SetRejectRosterGroupByEmpInPortal', {
                            host: _uriPor,
                            selectedIds: strId,
                            userRejectID: null,
                            userProcessID: headers.userid,
...
export default {
    ...
    AttApproveRosterGroupByEmpBusinessFunction: AttApproveRosterGroupByEmpBusinessFunction
};
