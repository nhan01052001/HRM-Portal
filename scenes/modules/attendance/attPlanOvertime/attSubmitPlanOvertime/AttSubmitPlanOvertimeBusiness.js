...
export const generateRowActionAndSelected = () => {
    ...
    if (ConfigList.value != null && ConfigList.value != undefined) {
        const _configList = ConfigList.value[attSubmitPlanOvertime],
        ...
        if (actionEditPer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_MODIFY,
                    onPress: item => AttSubmitPlanOvertimeBusinessFunction.businessModifyRecord({ ...item }),
                    ...actionEdit
                }
            ];
        }
            ...
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    onPress: (item, dataBody) =>
                        AttSubmitPlanOvertimeBusinessFunction.businessSendMailRecords([{ ...item }], dataBody),
                    ...actionSendMail
                }
            ];
            ...
            _selected = [
                ..._selected,
                {
                    ...
                    onPress: (items, dataBody) =>
                        AttSubmitPlanOvertimeBusinessFunction.businessSendMailRecords(items, dataBody),
                    ...actionSendMail
                }
            ];
        }
        ...
        if (actionDeletePer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_DELETE,
                    onPress: (item, dataBody) =>
                        AttSubmitPlanOvertimeBusinessFunction.businessDeleteRecords([{ ...item }], dataBody),
                    ...actionDelete
                }
            ];
        ...
        if (actionCancelPer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_CANCEL,
                    onPress: (item, dataBody) =>
                        AttSubmitPlanOvertimeBusinessFunction.businessCancelRecords([{ ...item }], dataBody),
                    ...actionCancel
                }
            ];
        ...
        if (actionRequestCancelPer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    onPress: item => AttSubmitPlanOvertimeBusinessFunction.businessRequestCancelRecords({ ...item }),
                    ...actionRequestCancel
                }
            ];
        }
...
export const AttSubmitPlanOvertimeBusinessFunction = {
    ...
    businessSendMailRecords: (items, dataBody) => {
        if (items.length === 0 && !dataBody) {
            ...
            if (selectedID.length === 1) {
                ...
                HttpService.Post('[URI_HR]/Att_GetData/GetPlanOvertimeById', {
                    id: selectedID[0],
                            ...
                            HttpService.Post('[URI_HR]/Att_GetData/BusinessAllowActionSendEmail', {
                                ...
                                try {
                                    if (res && typeof res === enumName.E_object) {
                                        ...
                                        if (isValid == true) {
                                            AttSubmitPlanOvertimeBusinessFunction.confirmSendMail(res);
                                        } else if (
                                            isValid == false &&
                                            res.message &&
            ...
            AlertSevice.alert({
                ...
                onConfirm: reason => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        HttpService.Post('[URI_HR]/Att_GetData/ProcessingSendMailOvertimePlan', {
                            host: _uriPor,
                            selectedIds: arrId,
                            reason
                        }).then(res => {
                            ...
                            HttpService.Post('[URI_HR]/Att_GetData/BusinessAllowActionDelete', {
                                ...
                                try {
                                    if (res && typeof res === enumName.E_object) {
                                        ...
                                        if (isValid) {
                                            AttSubmitPlanOvertimeBusinessFunction.confirmDelete(res);
                                        }
                                        //không hợp lệ
                                        else if (
                                            isValid == false &&
            ...
            AlertSevice.alert({
                ...
                onConfirm: reason => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        AttSubmitPlanOvertimeBusinessFunction.setIsDelete({ Ids: strId, reason });
                    }
                }
            });
        } else {
            let strId = objValid.strResultID;
            AttSubmitPlanOvertimeBusinessFunction.setIsDelete({ Ids: strId });
        }
    },
        ...
        HttpService.Post('[URI_HR]/Att_GetData/GetPlanOvertimeById', {
            ...
            try {
                if (res && res.BusinessAllowAction && res.BusinessAllowAction.indexOf('E_MODIFY') >= 0) {
                    const { reload } = _this;
                    _this.props.navigation.navigate(attSubmitPlanOvertimeAddOrEdit, { record: item, reload });
                } else {
                    ToasterSevice.showWarning('StatusNotAction');
                }
            } catch (error) {
                            ...
                            HttpService.Post('[URI_HR]/Att_GetData/BusinessAllowActionCancel', {
                                ...
                                try {
                                    if (res && typeof res === enumName.E_object) {
                                        ...
                                        if (isValid) {
                                            AttSubmitPlanOvertimeBusinessFunction.confirmCancel(res);
                                        }
                                        //không hợp lệ
                                        else if (
                                            isValid == false &&
    ...
    confirmCancel: objValid => {
        ...
        if (isConfirm) {
            ...
            AlertSevice.alert({
                ...
                onConfirm: reason => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        let strId = objValid.strResultID;
                        AttSubmitPlanOvertimeBusinessFunction.setStatusCancel({ Ids: strId, reason });
                    }
                }
            });
        } else {
            //check khung giờ cho Phổ Đình
            let strId = objValid.strResultID;
            AttSubmitPlanOvertimeBusinessFunction.setStatusCancel({ Ids: strId });
        }
    },
        ...
        HttpService.Post('[URI_HR]/Att_GetData/BusinessAllowActionRequestCancel', {
            ...
            try {
                if (res && typeof res === enumName.E_object) {
                    ...
                    if (isValid) {
                        const { reload } = _this;
                        // đã fix code truyền vô screenName: 'AttSubmitPlanOvertime'
                        // 05/04/2022
                        DrawerServices.navigate('AttSubmitPlanOvertimeCancelAddOrEdit', {
                            record: item,
                            reload,
                            screenName: 'AttSubmitPlanOvertime'
                        });
