...
export default class AttOvertimeListItem extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    render() {
        ...
        if (
            dataItem.Status == EnumStatus.E_APPROVE ||
            dataItem.Status == EnumStatus.E_APPROVED ||
            dataItem.Status == EnumStatus.E_APPROVED1 ||
            dataItem.Status == EnumStatus.E_APPROVED2 ||
            dataItem.Status == EnumStatus.E_FIRST_APPROVED ||
            dataItem.Status == EnumStatus.E_APPROVED3 ||
            dataItem.Status == EnumStatus.E_CANCEL
        ) {
            timeStatus = dataItem.ApproveHours ? `${Vnr_Function.mathRoundNumber(dataItem.ApproveHours)}` : '';
        } else if (dataItem.Status == EnumStatus.E_REJECT || dataItem.Status == EnumStatus.E_REJECTED) {
            if (dataItem.ApproveHours) {
                timeStatus = dataItem.ApproveHours ? `${Vnr_Function.mathRoundNumber(dataItem.ApproveHours)}` : '';
            } else {
                timeStatus = dataItem.RegisterHours ? `${Vnr_Function.mathRoundNumber(dataItem.RegisterHours)}` : '';
            }
        } else if (dataItem.Status == EnumStatus.E_CONFIRMED || dataItem.Status == EnumStatus.E_CONFIRM) {
            timeStatus = dataItem.ConfirmHours ? `${Vnr_Function.mathRoundNumber(dataItem.ConfirmHours)}` : '';
        } else {
            timeStatus = dataItem.RegisterHours ? `${Vnr_Function.mathRoundNumber(dataItem.RegisterHours)}` : '';
        }
                ...
                <View style={styles.line_date_row}>
                    ...
                    <View style={styles.line_time_wrap}>
                        <Text style={[styleSheets.text, styles.line_value_value]} numberOfLines={1}>
                            {dataItem.OutTime != null &&
                                dataItem.InTime != null &&
                                `${moment(dataItem.InTime).format('HH:mm')} - ${moment(dataItem.OutTime).format(
                                    'HH:mm'
                                )}`}
                        </Text>
                    </View>
        ...
        viewDatetime = (
            <View style={styles.lineRight}>
                <View style={styles.line_date_row}>
                    ...
                    <View style={styles.line_time_wrap}>
                        <Text style={[styleSheets.text, styles.line_value_value]}>{timeStatus}</Text>
                    </View>
                </View>

                <View style={styles.line_date_row}>
                    <VnrText i18nKey={'CompanyRate'} style={[styleSheets.text, styles.line_value_lable]} />
                    <View style={styles.line_time_wrap}>
                        <Text style={[styleSheets.text, styles.line_value_value]}>
                            {dataItem.Rate && `${Vnr_Function.mathRoundNumber(dataItem.Rate)}`}
                        </Text>
                    </View>
                </View>
            </View>
