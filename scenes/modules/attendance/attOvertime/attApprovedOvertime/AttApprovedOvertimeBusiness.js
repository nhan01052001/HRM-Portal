...
export const generateRowActionAndSelected = () => {
    ...
    if (ConfigList.value != null && ConfigList.value != undefined) {
        const _configList = ConfigList.value[attApprovedOvertime],
            ...
            _rowActions = [
                {
                    ...
                    onPress: (item, dataBody) =>
                        AttApprovedOvertimeBusinessFunction.businessCancelRecords([{ ...item }], dataBody),
                    ...actionCancel
                }
            ];
            ...
            _selected = [
                {
                    ...
                    onPress: (items, dataBody) =>
                        AttApprovedOvertimeBusinessFunction.businessCancelRecords(items, dataBody),
                    ...actionCancel
                }
            ];
        }
...
export const AttApprovedOvertimeBusinessFunction = {
                ...
                HttpService.Post('[URI_HR]/Att_GetData/BusinessAllowActionCancel', {
                    ...
                    try {
                        if (res && typeof res === enumName.E_object) {
                            ...
                            if (isValid) {
                                AttApprovedOvertimeBusinessFunction.confirmCancel(res);
                            }
                            //không hợp lệ
    ...
    confirmCancel: objValid => {
        ...
        if (isConfirm) {
            ...
            AlertSevice.alert({
                ...
                onConfirm: reason => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        AttApprovedOvertimeBusinessFunction.checkingDataWorkingForOTCancel({ Ids: strId, reason });
                    }
                }
            });
        } else {
            //check khung giờ cho Phổ Đình
            let strId = objValid.strResultID;
            AttApprovedOvertimeBusinessFunction.checkingDataWorkingForOTCancel({ Ids: strId });
        }
    },
        ...
        HttpService.Post('[URI_HR]/Att_GetData/CheckingValidateEmployeeWorkingForOvertime', {
            ...
            try {
                if (res && Array.isArray(res)) {
                    //dữ liệu hợp lệ
                    if (res[0] === 'success') {
                        AttApprovedOvertimeBusinessFunction.setStatusCancel(objValid);
                    }
                    //khung giờ không hợp lệ => gọi hàm mở popup để review
                    else if (res[0] === 'error') {
                        AttApprovedOvertimeBusinessFunction.confirmEmpWorkingForCancel(res, objValid);
                    }
                }
                //FAIL
                else {
