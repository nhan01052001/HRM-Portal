...
export const generateRowActionAndSelected = () => {
    ...
    if (ConfigList.value != null && ConfigList.value != undefined) {
        const _configList = ConfigList.value[attApproveRegisterVehicle],
            ...
            _rowActions = [
                {
                    ...
                    onPress: (item, dataBody) =>
                        AttApproveRegisterVehicleBusinessFunction.businessApproveRecords([{ ...item }], dataBody),
                    ...actionApprove
                }
            ];
        ...
        if (actionRejectPer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_REJECT,
                    onPress: (item, dataBody) =>
                        AttApproveRegisterVehicleBusinessFunction.businessRejectRecords([{ ...item }], dataBody),
                    ...actionReject
                }
            ];
...
export const AttApproveRegisterVehicleBusinessFunction = {
    ...
    businessRejectRecords: (items, dataBody) => {
        if (items.length === 0 && !dataBody) {
            ToasterSevice.showWarning('HRM_Insurance_InsuranceRecord_youmustselect', 5000);
        } else {
            ...
            HttpService.Post('[URI_HR]/Att_GetData/BusinessAllowActionReject', {
                ...
                try {
                    if (res && typeof res === 'object') {
                        ...
                        if (isValid) {
                            AttApproveRegisterVehicleBusinessFunction.confirmReject(res);
                        }
                        //không hợp lệ
            ...
            AlertSevice.alert({
                ...
                onConfirm: reason => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        AttApproveRegisterVehicleBusinessFunction.setStatusReject({ Ids: strId, reason });
                    }
                }
            });
        } else {
    ...
    setStatusReject: objValid => {
        ...
        // need fix  SetRejectOvertimePlanInPortal [done]
        VnrLoadingSevices.show();
        HttpService.Post('[URI_HR]/Att_GetData/SetRejectRegisterVehicleInPortal', {
            ...
            try {
                if (data && typeof data === 'string') {
                    if (data == enumName.E_Success) {
                        ...
                        // set true để refresh AttApprovedRegisterVehicle
                        AttApproveRegisterVehicleBusinessFunction.checkForReLoadScreen[
                            ScreenName.AttApprovedRegisterVehicle
                        ] = true;

                        // nếu duyệt ở màn hình notify thì không chuyển hướng.
                        if (!_isOnScreenNotification) DrawerServices.navigate(ScreenName.AttApproveRegisterVehicle);
            ...
            HttpService.Post('[URI_HR]/Att_GetData/BusinessAllowActionApprove', {
                ...
                try {
                    if (res && typeof res === enumName.E_object) {
                        ...
                        if (isValid) {
                            AttApproveRegisterVehicleBusinessFunction.confirmApprove(res);
                        }
                        //không hợp lệ
            ...
            AlertSevice.alert({
                ...
                onConfirm: reason => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        let strId = objValid.strResultID;
                        AttApproveRegisterVehicleBusinessFunction.setStatusApprove({ Ids: strId, reason });
                    }
                }
            });
        } else {
            let strId = objValid.strResultID;
            AttApproveRegisterVehicleBusinessFunction.setStatusApprove({ Ids: strId });
        }
    },
    ...
    setStatusApprove: objValid => {
        ...
        // need fix   SetApproveOvertimePlanInPortal [done]
        VnrLoadingSevices.show();
        HttpService.Post('[URI_HR]/Att_GetData/SetApproveRegisterVehicleInPortal', dataBody).then(data => {
            VnrLoadingSevices.hide();
            if (data && typeof data === 'string') {
                if (data == enumName.E_Success) {
...
export default {
    ...
    AttApproveRegisterVehicleBusinessFunction: AttApproveRegisterVehicleBusinessFunction
};
