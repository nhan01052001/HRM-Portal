...
export default class AttRegisterVehicleListItemApprove extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    render() {
        ...
        if (dataItem.DepartureDate != null && dataItem.ReturnDate != null) {
            // xử lý dateStart và dateEnd
            let dateStart = dataItem.DepartureDate ? dataItem.DepartureDate : null,
                dateEnd = dataItem.ReturnDate ? dataItem.ReturnDate : null,
                TimeCouse = '';
        ...
        if (dataItem.DepartureTime && dataItem.ReturnTime)
            textFieldTime = `(${moment(dataItem.DepartureTime).format('HH:mm')} - ${moment(dataItem.ReturnTime).format(
                'HH:mm'
            )})`;
        ...
        return (
            ...
            >
                                ...
                                <View style={[styles.lineLeft, CustomStyleSheet.borderRightWidth(0)]}>
                                    <View style={[styles.line_date_row]}>
                                        <VnrText
                                            ...
                                            i18nKey={'HRM_Attendance_AppPortal_PurposeRegister'}
                                        />
                                    ...
                                    <View style={[styles.line_date_row]}>
                                        ...
                                        <View style={styles.line_date_row}>
                                            {dataItem.DepartureDate && dataItem.ReturnDate && (
                                                <Text
                                                    numberOfLines={1}
                                                    style={[styleSheets.text, styles.line_value_value]}
                                                >
                                                    {`${textFieldDate} ${textFieldTime ? textFieldTime : ''}`}
                                                </Text>
                                            )}
                                        </View>
                                    </View>
                ...
                <View
                  style={[
                    styles.lineSatusLimit,
                    {
                      backgroundColor: colorTextOTLimit,
                    },
                  ]}>
                  <VnrText
                    numberOfLines={1}
                    style={[styleSheets.text, styles.lineSatus_text]}
                    i18nKey={'HRM_Vuot_OT'}
                  />
                </View>
              )} */}
