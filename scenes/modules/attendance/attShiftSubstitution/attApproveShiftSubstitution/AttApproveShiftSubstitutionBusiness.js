...
export const generateRowActionAndSelected = () => {
    ...
    if (ConfigList.value != null && ConfigList.value != undefined) {
        const _configList = ConfigList.value[attApproveShiftSubstitution],
            ...
            _rowActions = [
                {
                    ...
                    onPress: (item, dataBody) => {
                        if (Array.isArray(item) && item.length > 0) {
                            AttApproveShiftSubstitutionBusinessFunction.businessApproveRecords(item, dataBody);
                        } else {
                            AttApproveShiftSubstitutionBusinessFunction.businessApproveRecords([{ ...item }], dataBody);
                        }
                    },
            ...
            _selected = [
                {
                    ...
                    onPress: (items, dataBody) =>
                        AttApproveShiftSubstitutionBusinessFunction.businessApproveRecords(items, dataBody),
                    ...actionApprove
                }
            ];
        }
        ...
        if (actionRejectPer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    onPress: (item, dataBody) => {
                        if (Array.isArray(item) && item.length > 0) {
                            AttApproveShiftSubstitutionBusinessFunction.businessRejectRecords(item, dataBody);
                        } else {
                            AttApproveShiftSubstitutionBusinessFunction.businessRejectRecords([{ ...item }], dataBody);
                        }
                    },
...
export const AttApproveShiftSubstitutionBusinessFunction = {
    // Đối tượng checkForReLoadScreen để check có refresh ListApprovedStopWorking không
    checkForReLoadScreen: {
        [ScreenName.AttApprovedShiftSubstitution]: false
    },
            ...
            HttpService.Post('[URI_HR]/Att_GetData/BusinessAllowActionApprove', {
                ...
                try {
                    if (res && typeof res === 'object') {
                        ...
                        if (isValid) {
                            AttApproveShiftSubstitutionBusinessFunction.confirmApprove(res);
            ...
            AlertSevice.alert({
                ...
                onConfirm: reason => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        VnrLoadingSevices.show();
                        HttpService.Post('[URI_HR]/Att_GetData/SetApproveShiftSubstitutionInPortal', {
                            host: _uriPor,
                            selectedIds: strId,
                            userApproveID: null,
                                ...
                                if (res && res !== '') {
                                    if (res == 'Success') {
                                        ...
                                        if (!_isOnScreenNotification)
                                            DrawerServices.navigate('AttApproveShiftSubstitution');
                                    } else if (res == 'Locked') {
                ...
                try {
                    if (res && res !== '') {
                        if (res == 'Success') {
                            ...
                            // nếu duyệt ở màn hình notify thì không chuyển hướng.
                            if (!_isOnScreenNotification) DrawerServices.navigate('AttApproveShiftSubstitution');
                        } else if (res == 'Locked') {
                            ToasterSevice.showWarning('Hrm_Locked', 5000);
                        } else {
                            ToasterSevice.showWarning(res, 5000);
            ...
            HttpService.Post('[URI_HR]/Att_GetData/BusinessAllowActionReject', {
                ...
                try {
                    if (res && typeof res === enumName.E_object) {
                        let isValid = res.isValid;
                        if (isValid) {
                            AttApproveShiftSubstitutionBusinessFunction.confirmReject(res);
            ...
            AlertSevice.alert({
                ...
                onConfirm: reason => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        VnrLoadingSevices.show();
                        HttpService.Post('[URI_HR]/Att_GetData/SetRejectShiftSubstitutionInPortal', {
                            host: _uriPor,
                            selectedIds: strId,
                            userRejectID: null,
                            userProcessID: headers.userid,
...
export default {
    ...
    AttApproveShiftSubstitutionBusinessFunction: AttApproveShiftSubstitutionBusinessFunction
};
