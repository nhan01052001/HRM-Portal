...
import { Colors, CustomStyleSheet, Size, stylesScreenDetailV2, stylesScreenDetailV3 } from '../../../../../constants/styleConfig';
...
import AttShiftSubstitutionListItemApprove from './AttShiftSubstitutionListItemApprove';
import AttShiftSubstitutionListItem from './AttShiftSubstitutionListItem';
import { ConfigListFilter } from '../../../../../assets/configProject/ConfigListFilter';
const heightActionBottom = 45;

export default class AttShiftSubstitutionList extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            //valueField: "ID",
    ...
    remoteData = (param = {}) => {
        const { dataSource, page } = this.state,
            { isLazyLoading } = param,
            { keyDataLocal, keyQuery, GroupField } = this.props;
        if (keyDataLocal) {
                        ...
                        if (GroupField && GroupField.length > 0) {
                            data.map(item => {
                                ...
                                GroupField.forEach(el => {
                                    newKey += `${item[el] != null ? item[el] : ''}`;
                                });

                                if (newKey && objGroup[newKey]) {
                                    objGroup[newKey]['dataGroupMaster'].push(item);
                                } else if (newKey) {
                                    objGroup[newKey] = {
                                        ...objGroup[newKey],
                                        ...item,
    ...
    _renderFooterLoading = () => {
        return (
            <View
                style={stylesScreenDetailV2.loadingFooter}
            >
    ...
    render() {
        ...
        if (isLoading) {
            let typeLoading =
                detail.screenName == ScreenName.AttApproveShiftSubstitution ||
                detail.screenName == ScreenName.AttWaitConfirmShiftSubstitution ||
                detail.screenName == ScreenName.AttWaitConfirmedShiftSubstitution ||
                detail.screenName == ScreenName.AttApprovedShiftSubstitution
            ...
            contentList = (
                <FlatList
                    ...
                    style={
                        !isHaveFilter ? CustomStyleSheet.paddingTop(Size.defineSpace) : CustomStyleSheet.paddingTop(0)
                    }
                            ...
                            <View style={styles.styleViewBorderButtom}>
                                ...
                                >
                                    <View style={CustomStyleSheet.flex(1)}>
                                        ...
                                        detail.screenName == ScreenName.AttWaitConfirmedShiftSubstitution ||
                                        detail.screenName == ScreenName.AttApprovedShiftSubstitution ? (
                                                <AttShiftSubstitutionListItemApprove
