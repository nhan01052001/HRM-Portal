...
export default class AttSubmitPlanOvertimeCancelViewDetail extends Component {
    constructor(props) {
        super(props);
    ...
    getDataItem = async (isReload = false) => {
        try {
            ...
            if (!Vnr_Function.CheckIsNullOrEmpty(dataId) || isReload) {
                const response = await HttpService.Post('[URI_HR]/Att_GetData/GetRequestCancelationOTPlanByID', {
    ...
    reload = (E_KEEP_FILTER, actionIsDelete) => {
        ...
        if (actionIsDelete) {
            DrawerServices.navigate('AttSubmitPlanOvertimeCancel');
        } else {
    ...
    componentDidMount() {
        AttSubmitPlanOvertimeCancelBusinessFunction.setThisForBusiness(this, true);
        this.getDataItem();
    }
