...
export const generateRowActionAndSelected = () => {
    ...
    if (ConfigList.value != null && ConfigList.value != undefined) {
        const _configList = ConfigList.value[attApprovePlanOvertimeCancel],
            ...
            _rowActions = [
                {
                    ...
                    onPress: (item, dataBody) =>
                        AttApprovePlanOvertimeCancelBusinessFunction.businessApproveRecords([{ ...item }], dataBody),
                    ...actionApprove
                }
            ];
        ...
        if (actionRejectPer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_REJECT,
                    onPress: (item, dataBody) =>
                        AttApprovePlanOvertimeCancelBusinessFunction.businessRejectRecords([{ ...item }], dataBody),
                    ...actionReject
                }
            ];
        ...
        //             type: E_LIMIT_OT,
        //             onPress: (item, dataBody) => AttApprovePlanOvertimeCancelBusinessFunction.businessShowLimitOT({ ...item }),
        //             ...actionReject
        //         }
        //     ],
        // ];
...
export const AttApprovePlanOvertimeCancelBusinessFunction = {
    // Đối tượng checkForReLoadScreen để check có refresh ListApprovedStopWorking không
    checkForReLoadScreen: {
        [ScreenName.AttApprovedPlanOvertimeCancel]: false
    },
            ...
            HttpService.Post('[URI_HR]/Att_GetData/BusinessAllowActionReject', {
                ...
                try {
                    if (res && typeof res === 'object') {
                        ...
                        if (isValid) {
                            AttApprovePlanOvertimeCancelBusinessFunction.confirmReject(res);
                        }
                        //không hợp lệ
            ...
            AlertSevice.alert({
                ...
                onConfirm: reason => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        AttApprovePlanOvertimeCancelBusinessFunction.setStatusReject({ Ids: strId, reason });
                    }
                }
            });
        } else {
            let strId = objValid.strResultID;
            AttApprovePlanOvertimeCancelBusinessFunction.setStatusReject({ Ids: strId });
        }
    },
            ...
            try {
                if (data && typeof data === 'string') {
                    if (data == enumName.E_Success) {
                        ...
                        if (!_isOnScreenNotification) DrawerServices.navigate(ScreenName.AttApprovePlanOvertimeCancel);
            ...
            HttpService.Post('[URI_HR]/Att_GetData/BusinessAllowActionApprove', {
                ...
                try {
                    if (res && typeof res === enumName.E_object) {
                        ...
                        if (isValid) {
                            AttApprovePlanOvertimeCancelBusinessFunction.confirmApprove(res);
                        }
                        //không hợp lệ
            ...
            AlertSevice.alert({
                ...
                onConfirm: reason => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        let strId = objValid.strResultID;
                        AttApprovePlanOvertimeCancelBusinessFunction.setStatusApprove({ Ids: strId, reason });
                    }
                }
            });
        } else {
            let strId = objValid.strResultID;
            AttApprovePlanOvertimeCancelBusinessFunction.setStatusApprove({ Ids: strId });
        }
    },
    ...
    //#region
    // businessShowLimitOT: (item, dataBody) => {
...
export default {
    ...
    AttApprovePlanOvertimeCancelBusinessFunction: AttApprovePlanOvertimeCancelBusinessFunction
};
