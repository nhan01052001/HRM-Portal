...
import {
    ...
    AttSubmitBusinessTravelCancelBusinessFunction
} from './AttSubmitBusinessTravelCancelBusiness';
...
export default class AttSubmitBusinessTravelCancelViewDetail extends Component {
    constructor(props) {
        super(props);
    ...
    getDataItem = async (isReload = false) => {
        try {
            ...
            if (!Vnr_Function.CheckIsNullOrEmpty(dataId) || isReload) {
                const response = await HttpService.Post('[URI_HR]/Att_GetData/GetBusinessTravelCancelById', {
    ...
    reload = (E_KEEP_FILTER, actionIsDelete) => {
        ...
        if (actionIsDelete) {
            DrawerServices.navigate('AttSubmitBusinessTravelCancel');
        } else {
    ...
    componentDidMount() {
        AttSubmitBusinessTravelCancelBusinessFunction.setThisForBusiness(this, true);
        this.getDataItem();
    }
