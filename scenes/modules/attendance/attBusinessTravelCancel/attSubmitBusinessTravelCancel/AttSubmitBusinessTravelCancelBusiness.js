...
const enumName = EnumName,
    attSubmitBusinessTravelCancel = ScreenName.AttSubmitBusinessTravelCancel,
    attSubmitBusinessTravelCancelAddOrEdit = ScreenName.AttSubmitBusinessTravelCancelAddOrEdit;
...
export const generateRowActionAndSelected = () => {
    ...
    if (ConfigList.value != null && ConfigList.value != undefined) {
        const _configList = ConfigList.value[attSubmitBusinessTravelCancel],
            ...
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    onPress: (item, dataBody) =>
                        AttSubmitBusinessTravelCancelBusinessFunction.businessModifyRecord({ ...item }, dataBody),
                    ...actionEdit
                }
            ];
        }
        ...
        if (actionSendMailPer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_SENDMAIL,
                    onPress: (item, dataBody) =>
                        AttSubmitBusinessTravelCancelBusinessFunction.businessSendMailRecords([{ ...item }], dataBody),
                    ...actionSendMail
                }
            ];
            ...
            _selected = [
                ..._selected,
                {
                    ...
                    onPress: (items, dataBody) =>
                        AttSubmitBusinessTravelCancelBusinessFunction.businessSendMailRecords(items, dataBody),
                    ...actionSendMail
                }
            ];
        }
        ...
        if (actionDeletePer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_DELETE,
                    onPress: (item, dataBody) =>
                        AttSubmitBusinessTravelCancelBusinessFunction.businessDeleteRecords([{ ...item }], dataBody),
                    ...actionDelete
                }
            ];
            ...
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    onPress: (item, dataBody, isShowMultiRecord) =>
                        AttSubmitBusinessTravelCancelBusinessFunction.businessCancelRecords(
                            [{ ...item }],
                            dataBody,
                            isShowMultiRecord
                        ),
...
export const AttSubmitBusinessTravelCancelBusinessFunction = {
            ...
            HttpService.Post('[URI_HR]/Att_GetData/BusinessAllowActionDelete', {
                ...
                try {
                    if (res && typeof res === enumName.E_object) {
                        ...
                        if (isValid) {
                            AttSubmitBusinessTravelCancelBusinessFunction.confirmDelete(res);
                        }
                        //không hợp lệ
    ...
    confirmDelete: (objValid) => {
        ...
        if (isConfirm) {
            ...
            AlertSevice.alert({
                ...
                onConfirm: (reason) => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        AttSubmitBusinessTravelCancelBusinessFunction.setIsDelete({ Ids: strId, reason });
                    }
                }
            });
        } else {
            //check khung giờ cho Phổ Đình
            let strId = objValid.strResultID;
            AttSubmitBusinessTravelCancelBusinessFunction.setIsDelete({ Ids: strId });
        }
    },
    ...
    setIsDelete: (objValid) => {
        ...
        HttpService.Post('[URI_POR]/New_Att_RequestCancelationBusinessTravel/RemoveSelected', {
            selectedIds,
            reason
        }).then((res) => {
            VnrLoadingSevices.hide();
            ...
            HttpService.Post('[URI_HR]/Att_GetData/BusinessAllowActionSendEmail', {
                ...
                try {
                    if (res && typeof res === enumName.E_object) {
                        ...
                        if (isValid == true) {
                            AttSubmitBusinessTravelCancelBusinessFunction.confirmSendMail(res);
                        } else if (isValid == false && res.message && typeof res.message === enumName.E_string) {
                            ToasterSevice.showWarning(res.message, 4000);
                        } else {
                            ToasterSevice.showError('HRM_Common_SendRequest_Error', 4000);
    ...
    businessCancelRecords: (items, dataBody) => {
        if (items.length === 0 && !dataBody) {
            ...
            if (selectedID.length == 1) {
                VnrLoadingSevices.show();
                HttpService.Post('[URI_HR]/Att_GetData/GetBusinessTravelCancelById', {
                    id: selectedID[0],
                    screenName: attSubmitBusinessTravelCancel
                }).then((res) => {
                    VnrLoadingSevices.hide();
                            ...
                            HttpService.Post('[URI_HR]/Att_GetData/BusinessAllowActionCancel', {
                                ...
                                try {
                                    if (res && typeof res === enumName.E_object) {
                                        ...
                                        if (isValid) {
                                            AttSubmitBusinessTravelCancelBusinessFunction.confirmCancel(res);
                                        }
                                        //không hợp lệ
                                        else if (
                                            isValid == false &&
    ...
    confirmCancel: (objValid) => {
        ...
        if (isConfirm) {
            ...
            AlertSevice.alert({
                ...
                onConfirm: (reason) => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        //check khung giờ cho Phổ Đình
                        let strId = objValid.strResultID;
                        AttSubmitBusinessTravelCancelBusinessFunction.setStatusCancel({ Ids: strId, reason });
                    }
                }
            });
        } else {
            //check khung giờ cho Phổ Đình
            let strId = objValid.strResultID;
            AttSubmitBusinessTravelCancelBusinessFunction.setStatusCancel({ Ids: strId });
        }
    },
    ...
    setStatusCancel: (objValid) => {
        ...
        VnrLoadingSevices.show();
        HttpService.Post('[URI_HR]/Att_GetData/ChangeStatusCancelBusinessTravelCancel', {
            selectedIds,
            host: _uriPor,
            commentCancel,
        ...
        HttpService.Post('[URI_HR]/Att_GetData/GetBusinessTravelCancelById', {
            ...
            try {
                if (res && res.BusinessAllowAction && res.BusinessAllowAction.indexOf('E_MODIFY') >= 0) {
                    const { reload } = _this;
                    _this.props.navigation.navigate(attSubmitBusinessTravelCancelAddOrEdit, {
                        record: item,
                        reload,
                        screenName: attSubmitBusinessTravelCancel
                    });
