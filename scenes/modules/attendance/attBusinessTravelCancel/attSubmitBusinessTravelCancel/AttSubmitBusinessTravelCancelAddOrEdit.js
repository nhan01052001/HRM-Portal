...
export default class AttSubmitBusinessTravelCancelAddOrEdit extends Component {
    constructor(props) {
        super(props);
        ...
        props.navigation.setParams({
            title:
                props.navigation.state.params.screenName == 'AttSubmitBusinessTravelCancel'
                    ? 'HRM_New_Att_RequestCancelationBusinessTravel_Edit'
                    : 'HRM_New_Att_RequestCancelationBusinessTravel_Add'
        });
    }
    ...
    refreshView = () => {
        ...
        this.setVariable();
        this.setState(initSateDefault, () => this.getConfigValid('Att_RequestCancelBusinessTravelPortal', true));
    };
        ...
        HttpService.Get('[URI_POR]/Portal/GetConfigValid?tableName=' + tblName).then((res) => {
            ...
            if (res) {
                try {
                    ...
                    const _configField =
                        ConfigField && ConfigField.value['AttSubmitBusinessTravelCancelAddOrEdit']
                            ? ConfigField.value['AttSubmitBusinessTravelCancelAddOrEdit']['Hidden']
                            : [];
    ...
    componentDidMount() {
        this.getConfigValid('Att_RequestCancelBusinessTravelPortal');
    }

    handleSetState = (screenName, resAll) => {
        let nextState = {};
        const {
                Profile,
                BusinessTripTypeID,
            ...
            readOnlyApproverControl = resAll[1],
            levelApproveBussinessTravel = resAll[3],
            item = resAll[2];

        let recordID = item.RecordID;

        if (screenName == 'AttSubmitBusinessTravel') {
            recordID = item.ID;
        }
        ...
        if (levelApproveBussinessTravel) {
            this.levelApproveRequestCancelation = levelApproveBussinessTravel;
    ...
    getRecordAndConfigByID = (record, screenName, _handleSetState) => {
        ...
        if (screenName == 'AttSubmitBusinessTravel') {
            arrRequest = [...arrRequest, HttpService.Get('[URI_HR]/Att_GetData/GetBussinessTravelByID?id=' + ID)];
        } else {
            arrRequest = [
                ...arrRequest,
                HttpService.Get('[URI_HR]/Att_GetData/New_GetRequestCancelationBusinessTravelByID?id=' + ID)
            ];
        }
        ...
        HttpService.MultiRequest(arrRequest).then((resAll) => {
            VnrLoadingSevices.hide();
            try {
                this.GetLevelApproveBussinessTravel(_handleSetState, screenName, resAll);
            } catch (error) {
                DrawerServices.navigate('ErrorScreen', { ErrorDisplay: error });
            }
        });
    ...
    GetLevelApproveBussinessTravel = (_handleSetState, screenName, resAll) => {
        const item = resAll[2],
            { ProfileID } = item;
        HttpService.Post('[URI_HR]/Att_GetData/GetLevelApproveRequestCancelationBusinessTravel', {
            ProfileID: ProfileID
        }).then((res) => {
            if (res) {
                resAll = [...resAll, res];
                _handleSetState(screenName, resAll);
            }
        });
    };

    GetHighSupervior = () => {
        const { Profile, UserApproveID, UserApproveID2, UserApproveID3, UserApproveID4, BusinessTripTypeID } =
            this.state;
        ...
        HttpService.Post('[URI_HR]//Att_GetData/GetHighSupervisor', {
            ...
            type: 'E_REQUESTCANCELATION_BUSINESSTRAVEL',
            missionPlaceType: '',
            resource: {
            ...
            <SafeAreaView {...styleSafeAreaView}>
              ...
              <View style={viewControl}>
                ...
                />
                        ...
                        {UserApproveID.visibleConfig && UserApproveID.visible && (
                            <View style={contentViewControl}>
                                ...
                                <View style={viewControl}>
                                    <VnrPicker
                                        api={{
                                            urlApi: '[URI_SYS]/Sys_GetData/GetMultiUserApproved_E_REQUESTCANCELATION_BUSINESSTRAVEL',
                                            type: 'E_GET'
                                        }}
                                        refresh={UserApproveID.refresh}
                        ...
                        {BusinessTripReasonID.visibleConfig && BusinessTripReasonID.visible && false && (
                            <View style={contentViewControl}>
                                <View style={viewLable}>
                                    <VnrText
                                        style={[styleSheets.text, textLableInfo]}
