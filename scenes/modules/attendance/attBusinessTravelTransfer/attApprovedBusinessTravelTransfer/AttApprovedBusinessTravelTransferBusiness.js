...
export const generateRowActionAndSelected = () => {
    ...
    if (ConfigList.value != null && ConfigList.value != undefined) {
        const _configList = ConfigList.value[attApprovedBusinessTravelTransfer],
            ...
            _rowActions = [
                {
                    ...
                    onPress: (item, dataBody) =>
                        AttApprovedBusinessTravelTransferBusinessFunction.businessCancelRecords(
                            [{ ...item }],
                            dataBody
                        ),
                    ...actionCancel
            ...
            _selected = [
                {
                    ...
                    onPress: (items, dataBody) =>
                        AttApprovedBusinessTravelTransferBusinessFunction.businessCancelRecords(items, dataBody),
                    ...actionCancel
                }
            ];
        }
...
export const AttApprovedBusinessTravelTransferBusinessFunction = {
                ...
                HttpService.Post('[URI_HR]/Att_GetData/BusinessAllowActionCancel', {
                    ...
                    try {
                        if (res && typeof res === enumName.E_object) {
                            ...
                            if (isValid) {
                                AttApprovedBusinessTravelTransferBusinessFunction.confirmCancel(res);
                            }
                            //không hợp lệ
    ...
    confirmCancel: (objValid) => {
        ...
        if (isConfirm) {
            ...
            AlertSevice.alert({
                ...
                onConfirm: (reason) => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        AttApprovedBusinessTravelTransferBusinessFunction.checkingDataWorkingForOTCancel({
                            Ids: strId,
                            reason
                        });
                    }
            ...
            let strId = objValid.strResultID;
            AttApprovedBusinessTravelTransferBusinessFunction.checkingDataWorkingForOTCancel({ Ids: strId });
        }
    },
        ...
        HttpService.Post('[URI_HR]/Att_GetData/CheckingValidateEmployeeWorkingForOvertime', {
            ...
            try {
                if (res && Array.isArray(res)) {
                    //dữ liệu hợp lệ
                    if (res[0] === 'success') {
                        AttApprovedBusinessTravelTransferBusinessFunction.setStatusCancel(objValid);
                    }
                    //khung giờ không hợp lệ => gọi hàm mở popup để review
                    else if (res[0] === 'error') {
                        AttApprovedBusinessTravelTransferBusinessFunction.confirmEmpWorkingForCancel(res, objValid);
                    }
                }
                //FAIL
                else {
