...
export const generateRowActionAndSelected = () => {
    ...
    if (ConfigList.value != null && ConfigList.value != undefined) {
        const _configList = ConfigList.value[attSubmitBusinessTravelTransfer],
            ...
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    onPress: (item, dataBody) =>
                        AttSubmitBusinessTravelTransferBusinessFunction.businessModifyRecord({ ...item }, dataBody),
                    ...actionEdit
                }
            ];
        }
        ...
        if (actionSendMailPer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_SENDMAIL,
                    onPress: (item, dataBody) =>
                        AttSubmitBusinessTravelTransferBusinessFunction.businessSendMailRecords(
                            [{ ...item }],
                            dataBody
                        ),
                    ...actionSendMail
            ...
            _selected = [
                ..._selected,
                {
                    ...
                    onPress: (items, dataBody) =>
                        AttSubmitBusinessTravelTransferBusinessFunction.businessSendMailRecords(items, dataBody),
                    ...actionSendMail
                }
            ];
        }
        ...
        if (actionDeletePer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_DELETE,
                    onPress: (item, dataBody) =>
                        AttSubmitBusinessTravelTransferBusinessFunction.businessDeleteRecords([{ ...item }], dataBody),
                    ...actionDelete
                }
            ];
            ...
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    onPress: (item, dataBody, isShowMultiRecord) =>
                        AttSubmitBusinessTravelTransferBusinessFunction.businessCancelRecords(
                            [{ ...item }],
                            dataBody,
                            isShowMultiRecord
                        ),
...
export const AttSubmitBusinessTravelTransferBusinessFunction = {
    ...
    businessDeleteRecords: (items, dataBody) => {
        if (items.length === 0 && !dataBody) {
            ...
            if (selectedID.length === 1) {
                ...
                HttpService.Post('[URI_HR]/Att_GetData/GetBussinessTravelTransferByID', {
                    id: selectedID[0],
                            ...
                            HttpService.Post('[URI_HR]/Att_GetData/BusinessAllowActionDelete', {
                                ...
                                try {
                                    if (res && typeof res === enumName.E_object) {
                                        ...
                                        if (isValid) {
                                            AttSubmitBusinessTravelTransferBusinessFunction.confirmDelete(res);
                                        }
                                        //không hợp lệ
                                        else if (
                                            isValid == false &&
            ...
            AlertSevice.alert({
                ...
                onConfirm: reason => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        AttSubmitBusinessTravelTransferBusinessFunction.setIsDelete({ Ids: strId, reason });
                    }
                }
            });
        } else {
            let strId = objValid.strResultID;
            AttSubmitBusinessTravelTransferBusinessFunction.setIsDelete({ Ids: strId });
        }
    },
    ...
    setIsDelete: objValid => {
        ...
        HttpService.Post('[URI_POR]/New_Att_BusinessTravelTransfer/RemoveSelected', {
            selectedIds,
            reason
        }).then(res => {
            VnrLoadingSevices.hide();
                            ...
                            HttpService.Post('[URI_HR]/Att_GetData/BusinessAllowActionSendEmail', {
                                ...
                                try {
                                    if (res && typeof res === enumName.E_object) {
                                        let isValid = res.isValid;
                                        if (isValid == true) {
                                            AttSubmitBusinessTravelTransferBusinessFunction.confirmSendMail(res);
                                        } else if (
                                            isValid == false &&
                                            res.message &&
                                            typeof res.message === enumName.E_string
            ...
            AlertSevice.alert({
                ...
                onConfirm: reason => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        HttpService.Post('[URI_HR]/Att_GetData/ProcessSendMailBusinessTravelTransfer', {
                            host: _uriPor,
                            selectedIds: arrId,
                            reason
                        }).then(res => {
                            ...
                            HttpService.Post('[URI_HR]/Att_GetData/BusinessAllowActionCancel', {
                                ...
                                try {
                                    if (res && typeof res === enumName.E_object) {
                                        ...
                                        if (isValid) {
                                            AttSubmitBusinessTravelTransferBusinessFunction.confirmCancel(res);
                                        }
                                        //không hợp lệ
                                        else if (
                                            isValid == false &&
            ...
            AlertSevice.alert({
                ...
                onConfirm: reason => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        let strId = objValid.strResultID;
                        AttSubmitBusinessTravelTransferBusinessFunction.setStatusCancel({ Ids: strId, reason });
                    }
                }
            });
        } else {
            let strId = objValid.strResultID;
            AttSubmitBusinessTravelTransferBusinessFunction.setStatusCancel({ Ids: strId });
        }
    },
    ...
    setStatusCancel: objValid => {
        ...
        VnrLoadingSevices.show();
        HttpService.Post('[URI_HR]/Att_GetData/ChangeStatusCancelBusinessTravelTransfer', {
            selectedIds,
            host: _uriPor,
            commentCancel,
        ...
        HttpService.Post('[URI_HR]/Att_GetData/GetBussinessTravelTransferByID', {
            ...
            try {
                if (res && res.BusinessAllowAction && res.BusinessAllowAction.indexOf('E_MODIFY') >= 0) {
                    const { reload } = _this;
                    _this.props.navigation.navigate(attSubmitBusinessTravelTransferAddOrEdit, { record: item, reload });
                } else {
                    ToasterSevice.showWarning('StatusNotAction');
                }
            } catch (error) {
