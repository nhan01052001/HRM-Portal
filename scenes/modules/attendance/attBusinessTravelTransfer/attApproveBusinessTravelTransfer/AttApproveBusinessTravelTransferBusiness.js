...
export const generateRowActionAndSelected = () => {
    ...
    if (ConfigList.value != null && ConfigList.value != undefined) {
        const _configList = ConfigList.value[attApproveBusinessTravelTransfer],
            ...
            _rowActions = [
                {
                    ...
                    onPress: (item, dataBody) =>
                        AttApproveBusinessTravelTransferBusinessFunction.businessApproveRecords(
                            [{ ...item }],
                            dataBody
                        ),
                    ...actionApprove
            ...
            _selected = [
                {
                    ...
                    type: E_APPROVE,
                    onPress: (item, dataBody) =>
                        AttApproveBusinessTravelTransferBusinessFunction.businessApproveRecords(item, dataBody),
                    ...actionApprove
                }
            ];
        }
        ...
        if (actionRejectPer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_REJECT,
                    onPress: (item, dataBody) =>
                        AttApproveBusinessTravelTransferBusinessFunction.businessRejectRecords([{ ...item }], dataBody),
                    ...actionReject
                }
            ];
        ...
        if (actionCancelPer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_CANCEL,
                    onPress: (item, dataBody) =>
                        AttApproveBusinessTravelTransferBusinessFunction.businessCancelRecords([{ ...item }], dataBody),
                    ...actionCancel
                }
            ];
            ...
            _selected = [
                ..._selected,
                {
                    ...
                    onPress: (items, dataBody) =>
                        AttApproveBusinessTravelTransferBusinessFunction.businessCancelRecords(items, dataBody),
                    ...actionCancel
                }
            ];
        }
...
export const AttApproveBusinessTravelTransferBusinessFunction = {
    // Đối tượng checkForReLoadScreen để check có refresh ListApprovedStopWorking không
    checkForReLoadScreen: {
        [ScreenName.AttApprovedBusinessTravelTransfer]: false
    },
            ...
            HttpService.Post('[URI_HR]/Att_GetData/BusinessAllowActionCancel', {
                ...
                try {
                    if (res && typeof res === enumName.E_object) {
                        ...
                        if (isValid) {
                            AttApproveBusinessTravelTransferBusinessFunction.confirmCancel(res);
                        }
                        //không hợp lệ
    ...
    confirmCancel: (objValid) => {
        ...
        if (isConfirm) {
            ...
            AlertSevice.alert({
                ...
                onConfirm: (reason) => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        AttApproveBusinessTravelTransferBusinessFunction.checkingDataWorkingForOTCancel({
                            Ids: strId,
                            reason
                        });
                    }
            ...
            //check khung giờ cho Phổ Đình
            let strId = objValid.strResultID;
            AttApproveBusinessTravelTransferBusinessFunction.checkingDataWorkingForOTCancel({ Ids: strId });
        }
    },
        ...
        HttpService.Post('[URI_HR]/Att_GetData/CheckingValidateEmployeeWorkingForOvertime', {
            ...
            try {
                if (res && Array.isArray(res)) {
                    //dữ liệu hợp lệ
                    if (res[0] === 'success') {
                        AttApproveBusinessTravelTransferBusinessFunction.setStatusCancel(objValid);
                    }
                    //khung giờ không hợp lệ => gọi hàm mở popup để review
                    else if (res[0] === 'error') {
                        AttApproveBusinessTravelTransferBusinessFunction.confirmEmpWorkingForCancel(res, objValid);
                    }
                }
                //FAIL
                else {
            ...
            HttpService.Post('[URI_HR]/Att_GetData/BusinessAllowActionApprove', {
                ...
                try {
                    if (res && typeof res === enumName.E_object) {
                        ...
                        if (isValid) {
                            AttApproveBusinessTravelTransferBusinessFunction.confirmApprove(res);
                        }
                        //không hợp lệ
                        else if (isValid == false && res.message && typeof res.message === enumName.E_string) {
                            ToasterSevice.showWarning(res.message, 4000);
            ...
            AlertSevice.alert({
                ...
                onConfirm: (reason) => {
                    ...
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        let strId = objValid.strResultID;
                        AttApproveBusinessTravelTransferBusinessFunction.setStatusApprove({ Ids: strId, reason });
                    }
                }
            });
        } else {
            let strId = objValid.strResultID;
            AttApproveBusinessTravelTransferBusinessFunction.setStatusApprove({ Ids: strId });
        }
    },
    ...
    setStatusApprove: (objValid) => {
        ...
        VnrLoadingSevices.show();
        HttpService.Post('[URI_HR]/Att_GetData/SetApproveBusinessTravelTransferInPortal', dataBody).then((data) => {
            VnrLoadingSevices.hide();
            if (data && typeof data === 'string') {
                if (data == enumName.E_Success) {
                    ...
                    if (!_isOnScreenNotification) DrawerServices.navigate(ScreenName.AttApproveBusinessTravelTransfer);
            ...
            HttpService.Post('[URI_HR]/Att_GetData/BusinessAllowActionReject', {
                ...
                try {
                    if (res && typeof res === 'object') {
                        ...
                        //dữ liệu hợp lệ => gọi hàm confirm Hủy
                        if (isValid) {
                            AttApproveBusinessTravelTransferBusinessFunction.confirmReject(res);
                        }
                        //không hợp lệ
            ...
            AlertSevice.alert({
                ...
                onConfirm: (reason) => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        let strId = objValid.strResultID;
                        AttApproveBusinessTravelTransferBusinessFunction.setStatusReject({ Ids: strId, reason });
                    }
                }
            });
        } else {
            let strId = objValid.strResultID;
            AttApproveBusinessTravelTransferBusinessFunction.setStatusReject({ Ids: strId });
        }
    },
    ...
    setStatusReject: (objValid) => {
        ...
        VnrLoadingSevices.show();
        HttpService.Post('[URI_HR]/Att_GetData/SetRejectBusinessTravelTransferInPortal', {
            host: apiConfig.uriPor,
            selectedIds,
            userProcessID: headers.userid,
            DeclineReason
            ...
            try {
                if (data && typeof data === 'string') {
                    if (data == enumName.E_Success) {
                        ...
                        if (!_isOnScreenNotification)
                            DrawerServices.navigate(ScreenName.AttApproveBusinessTravelTransfer);
                ...
                //                 onCancel: () => { },
                //                 onConfirm: () => {
                //                     AttApproveBusinessTravelTransferBusinessFunction.setStatusReject(objValid);
                //                 }
                //             })
                //         }
                //         else {
...
export default {
    ...
    AttApproveBusinessTravelTransferBusinessFunction: AttApproveBusinessTravelTransferBusinessFunction
};
