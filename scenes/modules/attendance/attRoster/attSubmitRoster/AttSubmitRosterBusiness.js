...
export const generateRowActionAndSelected = () => {
    ...
    if (ConfigList.value != null && ConfigList.value != undefined) {
        const _configList = ConfigList.value[attSubmitRoster],
                ...
                ..._rowActions,
                {
                    ...
                    onPress: (item, dataBody) => {
                        if (Array.isArray(item) && item.length > 0) {
                            // AttSubmitRosterBusinessFunction.businessSendMailRecords(item, dataBody)
                        } else {
                            AttSubmitRosterBusinessFunction.businessModifyRecord({ ...item }, dataBody);
                        }
                    },
                    ...actionEdit
                }
            ...
            _selected = [
                ..._selected,
                {
                    ...
                    onPress: (items, dataBody) =>
                        AttSubmitRosterBusinessFunction.businessSendMailRecords(items, dataBody),
                    ...actionSendMail
                }
            ];
        }
        ...
        if (actionDeletePer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    onPress: (item, dataBody) => {
                        if (Array.isArray(item) && item.length > 0) {
                            AttSubmitRosterBusinessFunction.businessDeleteRecords(item, dataBody);
                        } else {
                            AttSubmitRosterBusinessFunction.businessDeleteRecords([{ ...item }], dataBody);
                        }
                    },
            ...
            _selected = [
                ..._selected,
                {
                    title: translate('HRM_Common_Delete'),
                    type: E_DELETE,
                    onPress: (item, dataBody) => AttSubmitRosterBusinessFunction.businessDeleteRecords(item, dataBody),
                    ...actionDelete
                }
            ];
        }
        ...
        if (actionCancelPer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    onPress: (item, dataBody) => {
                        if (Array.isArray(item) && item.length > 0) {
                            AttSubmitRosterBusinessFunction.businessCancelRecords(item, dataBody);
                        } else {
                            AttSubmitRosterBusinessFunction.businessCancelRecords([{ ...item }], dataBody);
                        }
                    },
            ...
            _selected = [
                ..._selected,
                {
                    title: translate('HRM_Common_Cancel'),
                    type: E_CANCEL,
                    onPress: (item, dataBody) => AttSubmitRosterBusinessFunction.businessCancelRecords(item, dataBody),
                    ...actionCancel
                }
            ];
        }
...
export const AttSubmitRosterBusinessFunction = {
                ...
                .then(res => {
                    try {
                        if (res && typeof res === 'object') {
                            ...
                            if (isValid) {
                                AttSubmitRosterBusinessFunction.confirmDelete(res);
                ...
                .then(res => {
                    try {
                        if (res && typeof res === 'object') {
                            let isValid = res.isValid;
                            if (isValid == true) {
                                AttSubmitRosterBusinessFunction.confirmSendMail(res);
                                VnrLoadingSevices.hide();
                            } else if (isValid == false && res.message && typeof res.message === 'string') {
                                ToasterSevice.showWarning(res.message, 4000);
                                VnrLoadingSevices.hide();
                ...
                .then(res => {
                    try {
                        if (res && typeof res === 'object') {
                            let isValid = res.isValid;
                            if (isValid) {
                                AttSubmitRosterBusinessFunction.confirmCancel(res);
                                VnrLoadingSevices.hide();
                            } else if (isValid == false && res.message && typeof res.message === 'string') {
                                ToasterSevice.showWarning(res.message, 4000);
                                VnrLoadingSevices.hide();
    ...
    businessModifyRecord: item => {
        const { reload } = _this;
        _this.props.navigation.navigate('AttSubmitRosterAddOrEdit', { record: item, reload });
    }
    //#endregion
};
