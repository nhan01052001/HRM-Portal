...
let configList = null,
    enumName = null,
    attRejectSubmitShiftChange = null,
    attSubmitShiftChange = null,
    attSubmitShiftChangeViewDetail = null,
    attRejectSubmitShiftChangeKeyTask = null,
    pageSizeList = 20;

class AttRejectSubmitShiftChange extends Component {
    constructor(props) {
        super(props);
        ...
        this.willFocusScreen = this.props.navigation.addListener('willFocus', () => {
            ...
            if (AttSubmitShiftChangeBusinessFunction.checkForReLoadScreen[attRejectSubmitShiftChange]) {
                this.reload();
            }
        });
    }
    ...
    reload = (paramsFilter) => {
        ...
        AttSubmitShiftChangeBusinessFunction.checkForReLoadScreen[attRejectSubmitShiftChange] = false;

        // Gửi yêu cầu lọc dữ liệu
        this.setState(_paramsDefault, () => {
            const { dataBody, keyQuery } = this.state;
            startTask({
                keyTask: attRejectSubmitShiftChangeKeyTask,
                payload: {
                    ...dataBody,
                    keyQuery: keyQuery,
                    isCompare: false,
    ...
    paramsDefault = () => {
        if (!attRejectSubmitShiftChange) {
            attRejectSubmitShiftChange = ScreenName.AttRejectSubmitShiftChange;
        }
        if (!configList[attRejectSubmitShiftChange]) {
            configList[attRejectSubmitShiftChange] = {
                Api: {
    ...
    UNSAFE_componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.reloadScreenName == attRejectSubmitShiftChangeKeyTask) {
    ...
    componentDidMount() {
        ...
        attRejectSubmitShiftChangeKeyTask = EnumTask.KT_AttRejectSubmitShiftChange;
        AttSubmitShiftChangeBusinessFunction.checkForReLoadScreen[attRejectSubmitShiftChange] = false;
        //set by config
    ...
    render() {
        ...
        return (
            <SafeAreaViewDetail style={styleSafeAreaView.style}>
                {attRejectSubmitShiftChange && attSubmitShiftChangeViewDetail && enumName && (
                            ...
                            {keyQuery && (
                                <AttShiftChangeList
                                    detail={{
                                        ...
                                        screenName: attSubmitShiftChange,
                                        screenNameRender: attRejectSubmitShiftChange
                                    }}
                                    scrollYAnimatedValue={this.scrollYAnimatedValue}
                                    rowActions={rowActions}
                                    selected={selected}
                                    reloadScreenList={this.reload.bind(this)}
                                    keyDataLocal={attRejectSubmitShiftChangeKeyTask}
