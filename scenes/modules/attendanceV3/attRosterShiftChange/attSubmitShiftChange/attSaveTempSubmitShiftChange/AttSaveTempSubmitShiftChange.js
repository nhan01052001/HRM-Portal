...
let configList = null,
    enumName = null,
    attSaveTempSubmitShiftChange = null,
    attSubmitShiftChange = null,
    attSubmitShiftChangeViewDetail = null,
    attSaveTempSubmitShiftChangeKeyTask = null,
    pageSizeList = 20;

class AttSaveTempSubmitShiftChange extends Component {
    constructor(props) {
        super(props);
        ...
        this.willFocusScreen = this.props.navigation.addListener('willFocus', () => {
            ...
            if (AttSubmitShiftChangeBusinessFunction.checkForReLoadScreen[attSaveTempSubmitShiftChange]) {
                this.reload();
            }
        });
    }
    ...
    reload = (paramsFilter) => {
        ...
        AttSubmitShiftChangeBusinessFunction.checkForReLoadScreen[attSaveTempSubmitShiftChange] = false;

        // Gửi yêu cầu lọc dữ liệu
        this.setState(_paramsDefault, () => {
            const { dataBody, keyQuery } = this.state;
            startTask({
                keyTask: attSaveTempSubmitShiftChangeKeyTask,
                payload: {
                    ...dataBody,
                    keyQuery: keyQuery,
                    isCompare: false,
    ...
    paramsDefault = () => {
        if (!attSaveTempSubmitShiftChange) {
            attSaveTempSubmitShiftChange = ScreenName.AttSaveTempSubmitShiftChange;
        }
        if (!configList[attSaveTempSubmitShiftChange]) {
            configList[attSaveTempSubmitShiftChange] = {
                Api: {
        ...
        const _configList = configList[attSaveTempSubmitShiftChange],
            renderRow = _configList[enumName.E_Row],
            filter = _configList[enumName.E_Filter],
            dataFromParams = this.checkDataFormNotify();

        const dataRowActionAndSelected = generateRowActionAndSelected(attSaveTempSubmitShiftChange);
        let _params = {
            ...dataFromParams,
            IsPortalNew: true,
            filter: filter,
    ...
    UNSAFE_componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.reloadScreenName == attSaveTempSubmitShiftChangeKeyTask) {
    ...
    componentDidMount() {
        ...
        attSaveTempSubmitShiftChangeKeyTask = EnumTask.KT_AttSaveTempSubmitShiftChange;
        AttSubmitShiftChangeBusinessFunction.checkForReLoadScreen[attSaveTempSubmitShiftChange] = false;
    ...
    render() {
        ...
        return (
            <SafeAreaViewDetail style={styleSafeAreaView.style}>
                {attSaveTempSubmitShiftChange && attSubmitShiftChangeViewDetail && enumName && (
                            ...
                            {keyQuery && (
                                <AttShiftChangeList
                                    detail={{
                                        ...
                                        screenName: attSubmitShiftChange,
                                        screenNameRender: attSaveTempSubmitShiftChange
                                    }}
                                    scrollYAnimatedValue={this.scrollYAnimatedValue}
                                    rowActions={rowActions}
                                    selected={selected}
                                    reloadScreenList={this.reload.bind(this)}
                                    keyDataLocal={attSaveTempSubmitShiftChangeKeyTask}
...
export default connect(mapStateToProps, null)(AttSaveTempSubmitShiftChange);
