...
let configList = null,
    enumName = null,
    attApproveSubmitShiftChange = null,
    attSubmitShiftChange = null,
    attSubmitShiftChangeViewDetail = null,
    attApproveSubmitShiftChangeTask = null,
    pageSizeList = 20;

class AttApproveSubmitShiftChange extends Component {
    constructor(props) {
        super(props);
        ...
        this.willFocusScreen = this.props.navigation.addListener('willFocus', () => {
            ...
            if (AttSubmitShiftChangeBusinessFunction.checkForReLoadScreen[attApproveSubmitShiftChange]) {
                this.reload();
            }
        });
    }
    ...
    reload = (paramsFilter) => {
        ...
        AttSubmitShiftChangeBusinessFunction.checkForReLoadScreen[attApproveSubmitShiftChange] = false;

        // Gửi yêu cầu lọc dữ liệu
        this.setState(_paramsDefault, () => {
            const { dataBody, keyQuery } = this.state;
            startTask({
                keyTask: attApproveSubmitShiftChangeTask,
                payload: {
                    ...dataBody,
                    keyQuery: keyQuery,
                    isCompare: false,
    ...
    paramsDefault = () => {
        if (!attApproveSubmitShiftChange) {
            attApproveSubmitShiftChange = ScreenName.AttApproveSubmitShiftChange;
        }
        if (!configList[attApproveSubmitShiftChange]) {
            configList[attApproveSubmitShiftChange] = {
                Api: {
        ...
        const _configList = configList[attApproveSubmitShiftChange],
            renderRow = _configList[enumName.E_Row],
            filter = _configList[enumName.E_Filter],
            dataFromParams = this.checkDataFormNotify();

        const dataRowActionAndSelected = generateRowActionAndSelected(attApproveSubmitShiftChange);
        let _params = {
            ...dataFromParams,
            IsPortalNew: true,
            filter: filter,
    ...
    UNSAFE_componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.reloadScreenName == attApproveSubmitShiftChangeTask) {
    ...
    componentDidMount() {
        ...
        attApproveSubmitShiftChangeTask = EnumTask.KT_AttApproveSubmitShiftChange;
        AttSubmitShiftChangeBusinessFunction.checkForReLoadScreen[attApproveSubmitShiftChange] = false;
        //set by config
    ...
    render() {
        ...
        return (
            <SafeAreaViewDetail style={styleSafeAreaView.style}>
                {attApproveSubmitShiftChange && attSubmitShiftChangeViewDetail && enumName && (
                            ...
                            {keyQuery && (
                                <AttShiftChangeList
                                    detail={{
                                        ...
                                        screenName: attSubmitShiftChange,
                                        screenNameRender: attApproveSubmitShiftChange
                                    }}
                                    scrollYAnimatedValue={this.scrollYAnimatedValue}
                                    rowActions={rowActions}
                                    selected={selected}
                                    reloadScreenList={this.reload.bind(this)}
                                    keyDataLocal={attApproveSubmitShiftChangeTask}
...
export default connect(mapStateToProps, null)(AttApproveSubmitShiftChange);
