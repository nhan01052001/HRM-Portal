...
let configList = null,
    enumName = null,
    attCanceledDelegationApproval = null,
    attConfirmDelegationApprovalViewDetail = null,
    attConfirmDelegationApproval = null,
    keyListTask = null,
    pageSizeList = 20;

class AttCanceledDelegationApproval extends Component {
    constructor(props) {
        super(props);
        ...
        this.willFocusScreen = this.props.navigation.addListener('willFocus', () => {
            // reload danh sách khi có Confirm hoặc reject dữ liệu
            AttConfirmDelegationApprovalBusiness.setThisForBusiness(this);
            if (AttConfirmDelegationApprovalBusiness.checkForReLoadScreen[attCanceledDelegationApproval]) {
                this.reload();
            }
        });
    }
    ...
    reload = paramsFilter => {
        ...
        AttConfirmDelegationApprovalBusiness.checkForReLoadScreen[attCanceledDelegationApproval] = false;
    ...
    paramsDefault = () => {
        if (!attCanceledDelegationApproval) {
            attCanceledDelegationApproval = ScreenName.AttCanceledDelegationApproval;
        }
        if (!configList[attCanceledDelegationApproval]) {
            configList[attCanceledDelegationApproval] = {
                Api: {
        ...
        const _configList = configList[attCanceledDelegationApproval],
            renderRow = _configList[enumName.E_Row],
            filter = _configList[enumName.E_Filter],
            dataFromParams = this.checkDataFormNotify();

        const dataRowActionAndSelected = generateRowActionAndSelected(attCanceledDelegationApproval);
        let _params = {
            ...dataFromParams,
            IsPortalNew: true,
            filter: filter,
    ...
    componentDidMount() {
        ...
        keyListTask = EnumTask.KT_AttCanceledDelegationApproval;
        AttConfirmDelegationApprovalBusiness.checkForReLoadScreen[attCanceledDelegationApproval] = false;
        //set by config
    ...
    render() {
        ...
        return (
            <SafeAreaViewDetail style={styleSafeAreaView.style}>
                {attCanceledDelegationApproval && attConfirmDelegationApprovalViewDetail && enumName && (
                        ...
                        <View style={[styleSheets.container]}>
                            {keyQuery && (
                                <AttDelegationApprovalList
                                    ...
                                    api={{
                                        urlApi:
                                            '[URI_CENTER]/api/Att_OvertimePlan/New_PlanGetOvertimeConfirmdByFilterHandle_App',
