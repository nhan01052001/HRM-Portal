...
import AttDelegationApprovalListItem from './AttDelegationApprovalListItem';
import AttDelegationApprovalListItemApprove from './AttDelegationApprovalListItemApprove';
...
export default class AttDelegationApprovalList extends VnrRenderList {
    remoteData = (param = {}) => {
                ...
                .then(resData => {
                    ...
                    if (res && res !== EnumName.E_EMPTYDATA) {
                        ...
                        data.map(item => {
                            ...
                            if (detail?.screenName === ScreenName.AttSubmitDelegationApproval && item.Status === 'E_WAITING_CONFIRM') {
                                item.BusinessAllowAction = 'E_CANCEL';
                            } else if (detail?.screenName === ScreenName.AttConfirmedDelegationApproval) {
                                item.BusinessAllowAction = Vnr_Services.handleStatus(
                                    item.Status,
                                    item?.SendEmailStatus ? item?.SendEmailStatus : false
                                );
                            } else if (detail?.screenName === ScreenName.AttConfirmDelegationApproval) {
                            ...
                            <View style={styles.styleViewBorderButtom}>
                                <TouchableWithoutFeedback>
                                    <View style={CustomStyleSheet.flex(1)}>
                                        {detail?.screenName === ScreenName.AttConfirmDelegationApproval ? (
                                            <AttDelegationApprovalListItemApprove
    ...
    render() {
        const {
            ...
            permissionBtnCreate =
                PermissionForAppMobile &&
                    PermissionForAppMobile.value['New_Sys_DelegateApprove_New_Index_Portal'] &&
                    PermissionForAppMobile.value['New_Sys_DelegateApprove_New_Index_Portal']['Create']
                    ? true
                    : false;
                        ...
                        <View style={[styles.containBotton]}>
                            ...
                            {item?.isGroup ? (
                                ...
                                >
                                    <View style={styles.styleViewBorderButtom}>
                                        <View style={CustomStyleSheet.flex(1)}>
                                            {detail.screenName !== ScreenName.AttSubmitDelegationApproval ? (
                                                <AttDelegationApprovalListItemApprove
                                                    key={index}
                                                    renderConfig={renderConfig}
        ...
        let isShowBtn =
            permissionBtnCreate &&
            (detail.screenNameRender === ScreenName.AttSubmitDelegationApproval);
        return (
            <View
                style={[
