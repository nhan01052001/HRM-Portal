...
let configList = null,
    enumName = null,
    attCanceledSubmitDelegationApproval = null,
    attSubmitDelegationApproval = null,
    attSubmitDelegationApprovalViewDetail = null,
    attCanceledDelegationApprovalKeyTask = null,
    dataRowActionAndSelected = null,
    pageSizeList = 20;

class AttCanceledSubmitDelegationApproval extends Component {
    constructor(props) {
        super(props);
        ...
        this.willFocusScreen = this.props.navigation.addListener('willFocus', () => {
            ...
            if (AttSubmitDelegationApprovalBusiness.checkForReLoadScreen[attCanceledSubmitDelegationApproval]) {
                this.reload();
            }
        });
    }
    ...
    reload = paramsFilter => {
        ...
        AttSubmitDelegationApprovalBusiness.checkForReLoadScreen[attCanceledSubmitDelegationApproval] = false;

        // Gửi yêu cầu lọc dữ liệu
        this.setState(_paramsDefault, () => {
            const { dataBody, keyQuery } = this.state;
            startTask({
                keyTask: attCanceledDelegationApprovalKeyTask,
                payload: {
                    ...dataBody,
                    keyQuery: keyQuery,
                    isCompare: false,
    ...
    paramsDefault = () => {
        if (!attCanceledSubmitDelegationApproval) {
            attCanceledSubmitDelegationApproval = ScreenName.AttCanceledSubmitDelegationApproval;
        }
        if (!configList[attCanceledSubmitDelegationApproval]) {
            configList[attCanceledSubmitDelegationApproval] = {
                Api: {
        ...
        const _configList = configList[attCanceledSubmitDelegationApproval],
            renderRow = _configList[enumName.E_Row],
            filter = _configList[enumName.E_Filter],
            dataFromParams = this.checkDataFormNotify();

        dataRowActionAndSelected = generateRowActionAndSelected(attCanceledSubmitDelegationApproval);
        let _params = {
            ...dataFromParams,
            IsPortalNew: true,
            filter: filter,
    ...
    UNSAFE_componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.reloadScreenName == attCanceledDelegationApprovalKeyTask) {
    ...
    componentDidMount() {
        ...
        attCanceledDelegationApprovalKeyTask = EnumTask.KT_AttCanceledSubmitDelegationApproval;
        AttSubmitDelegationApprovalBusiness.checkForReLoadScreen[attCanceledSubmitDelegationApproval] = false;
        //set by config
    ...
    render() {
        ...
        return (
            <SafeAreaViewDetail style={styleSafeAreaView.style}>
                {attCanceledSubmitDelegationApproval && attSubmitDelegationApprovalViewDetail && enumName && (
                            ...
                            {keyQuery && (
                                <AttDelegationApprovalList
                                    detail={{
                                        ...
                                        screenName: attSubmitDelegationApproval,
                                        screenNameRender: attCanceledSubmitDelegationApproval
                                    }}
                                    scrollYAnimatedValue={this.scrollYAnimatedValue}
                                    rowActions={rowActions}
                                    selected={selected}
                                    reloadScreenList={this.reload.bind(this)}
                                    keyDataLocal={attCanceledDelegationApprovalKeyTask}
