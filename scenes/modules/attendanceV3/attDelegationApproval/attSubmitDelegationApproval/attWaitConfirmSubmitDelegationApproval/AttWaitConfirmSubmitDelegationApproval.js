...
let configList = null,
    enumName = null,
    attWaitConfirmSubmitDelegationApproval = null,
    attSubmitDelegationApproval = null,
    attSubmitDelegationApprovalViewDetail = null,
    attWaitConfirmDelegationApprovalTask = null,
    dataRowActionAndSelected = null,
    pageSizeList = 20;

class AttWaitConfirmSubmitDelegationApproval extends Component {
    constructor(props) {
        super(props);
        ...
        this.willFocusScreen = this.props.navigation.addListener('willFocus', () => {
            ...
            if (AttSubmitDelegationApprovalBusiness.checkForReLoadScreen[attWaitConfirmSubmitDelegationApproval]) {
                this.reload();
            }
        });
    }
    ...
    reload = paramsFilter => {
        ...
        AttSubmitDelegationApprovalBusiness.checkForReLoadScreen[attWaitConfirmSubmitDelegationApproval] = false;

        // Gửi yêu cầu lọc dữ liệu
        this.setState(_paramsDefault, () => {
            const { dataBody, keyQuery } = this.state;
            startTask({
                keyTask: attWaitConfirmDelegationApprovalTask,
                payload: {
                    ...dataBody,
                    keyQuery: keyQuery,
                    isCompare: false,
    ...
    paramsDefault = () => {
        if (!attWaitConfirmSubmitDelegationApproval) {
            attWaitConfirmSubmitDelegationApproval = ScreenName.AttWaitConfirmSubmitDelegationApproval;
        }
        if (!configList[attWaitConfirmSubmitDelegationApproval]) {
            configList[attWaitConfirmSubmitDelegationApproval] = {
                Api: {
                ...
                BusinessAction: [
                    {
                        Type: 'E_CANCEL',
                        Resource: {
                            Name: 'New_Sys_DelegateApprove_New_Index_btnCancel',
                            Rule: 'View'
                        },
                        Confirm: {
                            isInputText: true,
        ...
        const _configList = configList[attWaitConfirmSubmitDelegationApproval],
            renderRow = _configList[enumName.E_Row],
            filter = _configList[enumName.E_Filter],
            dataFromParams = this.checkDataFormNotify();

        dataRowActionAndSelected = generateRowActionAndSelected(attWaitConfirmSubmitDelegationApproval);
        let _params = {
            ...dataFromParams,
            IsPortalNew: true,
            filter: filter,
    ...
    UNSAFE_componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.reloadScreenName == attWaitConfirmDelegationApprovalTask) {
    ...
    componentDidMount() {
        ...
        attWaitConfirmDelegationApprovalTask = EnumTask.KT_AttWaitConfirmSubmitDelegationApproval;
        AttSubmitDelegationApprovalBusiness.checkForReLoadScreen[attWaitConfirmSubmitDelegationApproval] = false;
        //set by config
    ...
    render() {
        ...
        return (
            <SafeAreaViewDetail style={styleSafeAreaView.style}>
                {attWaitConfirmSubmitDelegationApproval && attSubmitDelegationApprovalViewDetail && enumName && (
                            ...
                            {keyQuery && (
                                <AttDelegationApprovalList
                                    detail={{
                                        ...
                                        screenName: attSubmitDelegationApproval,
                                        screenNameRender: attWaitConfirmSubmitDelegationApproval
                                    }}
                                    scrollYAnimatedValue={this.scrollYAnimatedValue}
                                    rowActions={rowActions}
                                    selected={selected}
                                    reloadScreenList={this.reload.bind(this)}
                                    keyDataLocal={attWaitConfirmDelegationApprovalTask}
