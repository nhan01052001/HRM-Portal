...
let configList = null,
    ...
    attSubmitDelegationApprovalKeyTask = null,
    dataRowActionAndSelected = null,
    pageSizeList = 20;

class AttSubmitDelegationApproval extends Component {
    constructor(props) {
        super(props);
        ...
        this.willFocusScreen = this.props.navigation.addListener('willFocus', () => {
            ...
            if (AttSubmitDelegationApprovalBusiness.checkForReLoadScreen[attSubmitDelegationApproval]) {
                this.reload();
            }
        });
    }
    ...
    reload = paramsFilter => {
        if (typeof paramsFilter === 'object')
            Object.keys(paramsFilter).map((item) => {
                if (!item)
                    delete paramsFilter[item];
            });
        ...
        AttSubmitDelegationApprovalBusiness.checkForReLoadScreen[attSubmitDelegationApproval] = false;

        // Gửi yêu cầu lọc dữ liệu
        this.setState(_paramsDefault, () => {
            const { dataBody, keyQuery } = this.state;
            startTask({
                keyTask: attSubmitDelegationApprovalKeyTask,
                payload: {
                    ...dataBody,
                    keyQuery: keyQuery,
                    isCompare: false,
    ...
    paramsDefault = () => {
        if (!attSubmitDelegationApproval) {
            attSubmitDelegationApproval = ScreenName.AttSubmitDelegationApproval;
        }
        if (!configList[attSubmitDelegationApproval]) {
            configList[attSubmitDelegationApproval] = {
                Api: {
        ...
        const _configList = configList[attSubmitDelegationApproval],
            renderRow = _configList[enumName.E_Row],
            filter = _configList[enumName.E_Filter],
            dataFromParams = this.checkDataFormNotify();


        dataRowActionAndSelected = generateRowActionAndSelected(attSubmitDelegationApproval);
    ...
    UNSAFE_componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.reloadScreenName == attSubmitDelegationApprovalKeyTask) {
    ...
    componentDidMount() {
        ...
        attSubmitDelegationApprovalKeyTask = EnumTask.KT_AttSubmitDelegationApproval;
        AttSubmitDelegationApprovalBusiness.checkForReLoadScreen[attSubmitDelegationApproval] = false;
        //set by config
    ...
    render() {
        ...
        return (
            <SafeAreaViewDetail style={styleSafeAreaView.style}>
                {attSubmitDelegationApproval && attSubmitDelegationApprovalViewDetail && enumName && (
                            ...
                            {keyQuery && (
                                <AttDelegationApprovalList
                                    detail={{
                                        ...
                                        screenName: attSubmitDelegationApproval,
                                        screenNameRender: ScreenName.AttSubmitDelegationApproval
                                    }}
                                    scrollYAnimatedValue={this.scrollYAnimatedValue}
                                    rowActions={rowActions}
                                    selected={selected}
                                    reloadScreenList={this.reload.bind(this)}
                                    keyDataLocal={attSubmitDelegationApprovalKeyTask}
