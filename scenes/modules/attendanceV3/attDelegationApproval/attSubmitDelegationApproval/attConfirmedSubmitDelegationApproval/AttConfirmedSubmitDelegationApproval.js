...
let configList = null,
    enumName = null,
    attConfirmedSubmitDelegationApproval = null,
    AttSubmitDelegationApproval = null,
    attSubmitDelegationApprovalViewDetail = null,
    attConfirmedDelegationApprovalKeyTask = null,
    dataRowActionAndSelected = null,
    pageSizeList = 20;

class AttConfirmedSubmitDelegationApproval extends Component {
    constructor(props) {
        super(props);
        ...
        this.willFocusScreen = this.props.navigation.addListener('willFocus', () => {
            ...
            if (AttSubmitDelegationApprovalBusiness.checkForReLoadScreen[attConfirmedSubmitDelegationApproval]) {
                this.reload();
            }
        });
    }
    ...
    reload = (paramsFilter) => {
        ...
        AttSubmitDelegationApprovalBusiness.checkForReLoadScreen[attConfirmedSubmitDelegationApproval] = false;

        // Gửi yêu cầu lọc dữ liệu
        this.setState(_paramsDefault, () => {
            const { dataBody, keyQuery } = this.state;
            startTask({
                keyTask: attConfirmedDelegationApprovalKeyTask,
                payload: {
                    ...dataBody,
                    keyQuery: keyQuery,
                    isCompare: false,
    ...
    paramsDefault = () => {
        if (!attConfirmedSubmitDelegationApproval) {
            attConfirmedSubmitDelegationApproval = ScreenName.AttConfirmedSubmitDelegationApproval;
        }
        if (!configList[attConfirmedSubmitDelegationApproval] || 1 === 1) {
            configList[attConfirmedSubmitDelegationApproval] = {
                Api: {
        ...
        const _configList = configList[attConfirmedSubmitDelegationApproval],
            renderRow = _configList[enumName.E_Row],
            filter = _configList[enumName.E_Filter],
            dataFromParams = this.checkDataFormNotify();

        dataRowActionAndSelected = generateRowActionAndSelected(attConfirmedSubmitDelegationApproval);
        let _params = {
            ...dataFromParams,
            IsPortalNew: true,
            filter: filter,
    ...
    UNSAFE_componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.reloadScreenName == attConfirmedDelegationApprovalKeyTask) {
    ...
    componentDidMount() {
        ...
        attConfirmedDelegationApprovalKeyTask = EnumTask.KT_AttConfirmedSubmitDelegationApproval;
        AttSubmitDelegationApprovalBusiness.checkForReLoadScreen[attConfirmedSubmitDelegationApproval] = false;
    ...
    render() {
        ...
        return (
            <SafeAreaViewDetail style={styleSafeAreaView.style}>
                {attConfirmedSubmitDelegationApproval && attSubmitDelegationApprovalViewDetail && enumName && (
                            ...
                            {keyQuery && (
                                <AttDelegationApprovalList
                                    detail={{
                                        ...
                                        screenName: AttSubmitDelegationApproval,
                                        screenNameRender: attConfirmedSubmitDelegationApproval
                                    }}
                                    scrollYAnimatedValue={this.scrollYAnimatedValue}
                                    rowActions={rowActions}
                                    selected={selected}
                                    reloadScreenList={this.reload.bind(this)}
                                    keyDataLocal={attConfirmedDelegationApprovalKeyTask}
...
export default connect(mapStateToProps, null)(AttConfirmedSubmitDelegationApproval);
