...
let configList = null,
    enumName = null,
    attCanceledSubmitWorkingOvertime = null,
    attSubmitWorkingOvertime = null,
    attSubmitWorkingOvertimeViewDetail = null,
    attCanceledSubmitWorkingOvertimeKeyTask = null,
    pageSizeList = 20;

class AttCanceledSubmitWorkingOvertime extends Component {
    constructor(props) {
        super(props);
        ...
        this.willFocusScreen = this.props.navigation.addListener('willFocus', () => {
            ...
            if (AttSubmitWorkingOvertimeBusinessFunction.checkForReLoadScreen[attCanceledSubmitWorkingOvertime]) {
                this.reload();
            }
        });
    }
    ...
    reload = paramsFilter => {
        ...
        AttSubmitWorkingOvertimeBusinessFunction.checkForReLoadScreen[attCanceledSubmitWorkingOvertime] = false;

        // Gửi yêu cầu lọc dữ liệu
        this.setState(_paramsDefault, () => {
            const { dataBody, keyQuery } = this.state;
            startTask({
                keyTask: attCanceledSubmitWorkingOvertimeKeyTask,
                payload: {
                    ...dataBody,
                    keyQuery: keyQuery,
                    isCompare: false,
    ...
    paramsDefault = () => {
        if (!attCanceledSubmitWorkingOvertime) {
            attCanceledSubmitWorkingOvertime = ScreenName.AttCanceledSubmitWorkingOvertime;
        }
        if (!configList[attCanceledSubmitWorkingOvertime]) {
            configList[attCanceledSubmitWorkingOvertime] = {
                Api: {
        ...
        const _configList = configList[attCanceledSubmitWorkingOvertime],
            filter = _configList[enumName.E_Filter],
            dataFromParams = this.checkDataFormNotify();

        const dataRowActionAndSelected = generateRowActionAndSelected(attCanceledSubmitWorkingOvertime);
        let _params = {
            ...dataFromParams,
            IsPortalNew: true,
            filter: filter,
    ...
    UNSAFE_componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.reloadScreenName == attCanceledSubmitWorkingOvertimeKeyTask) {
    ...
    componentDidMount() {
        ...
        attCanceledSubmitWorkingOvertimeKeyTask = EnumTask.KT_AttCanceledSubmitWorkingOvertime;
        AttSubmitWorkingOvertimeBusinessFunction.checkForReLoadScreen[attCanceledSubmitWorkingOvertime] = false;
        //set by config
    ...
    render() {
        ...
        return (
            <SafeAreaViewDetail style={styleSafeAreaView.style}>
                {attCanceledSubmitWorkingOvertime && attSubmitWorkingOvertimeViewDetail && enumName && (
                            ...
                            {keyQuery && (
                                <AttWorkingOvertimeList
                                    detail={{
                                        ...
                                        screenName: attSubmitWorkingOvertime,
                                        screenNameRender: attCanceledSubmitWorkingOvertime
                                    }}
                                    scrollYAnimatedValue={this.scrollYAnimatedValue}
                                    rowActions={rowActions}
                                    selected={selected}
                                    reloadScreenList={this.reload.bind(this)}
                                    keyDataLocal={attCanceledSubmitWorkingOvertimeKeyTask}
