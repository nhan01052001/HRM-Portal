...
let configList = null,
    enumName = null,
    attSaveTempSubmitWorkingOvertime = null,
    attSubmitWorkingOvertime = null,
    attSubmitWorkingOvertimeViewDetail = null,
    attSaveTempSubmitWorkingOvertimeKeyTask = null,
    pageSizeList = 20;

class AttSaveTempSubmitWorkingOvertime extends Component {
    constructor(props) {
        super(props);
        ...
        this.willFocusScreen = this.props.navigation.addListener('willFocus', () => {
            ...
            if (AttSubmitWorkingOvertimeBusinessFunction.checkForReLoadScreen[attSaveTempSubmitWorkingOvertime]) {
                this.reload();
            }
        });
    }
    ...
    reload = paramsFilter => {
        ...
        AttSubmitWorkingOvertimeBusinessFunction.checkForReLoadScreen[attSaveTempSubmitWorkingOvertime] = false;

        // Gửi yêu cầu lọc dữ liệu
        this.setState(_paramsDefault, () => {
            const { dataBody, keyQuery } = this.state;
            startTask({
                keyTask: attSaveTempSubmitWorkingOvertimeKeyTask,
                payload: {
                    ...dataBody,
                    keyQuery: keyQuery,
                    isCompare: false,
    ...
    paramsDefault = () => {
        if (!attSaveTempSubmitWorkingOvertime) {
            attSaveTempSubmitWorkingOvertime = ScreenName.AttSaveTempSubmitWorkingOvertime;
        }
        if (!configList[attSaveTempSubmitWorkingOvertime]) {
            configList[attSaveTempSubmitWorkingOvertime] = {
                Api: {
        ...
        const _configList = configList[attSaveTempSubmitWorkingOvertime],
            filter = _configList[enumName.E_Filter],
            dataFromParams = this.checkDataFormNotify();

        const dataRowActionAndSelected = generateRowActionAndSelected(attSaveTempSubmitWorkingOvertime);
        let _params = {
            ...dataFromParams,
            IsPortalNew: true,
            filter: filter,
    ...
    UNSAFE_componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.reloadScreenName == attSaveTempSubmitWorkingOvertimeKeyTask) {
    ...
    componentDidMount() {
        ...
        attSaveTempSubmitWorkingOvertimeKeyTask = EnumTask.KT_AttSaveTempSubmitWorkingOvertime;
        AttSubmitWorkingOvertimeBusinessFunction.checkForReLoadScreen[attSaveTempSubmitWorkingOvertime] = false;
    ...
    render() {
        ...
        return (
            <SafeAreaViewDetail style={styleSafeAreaView.style}>
                {attSaveTempSubmitWorkingOvertime && attSubmitWorkingOvertimeViewDetail && enumName && (
                            ...
                            {keyQuery && (
                                <AttWorkingOvertimeList
                                    detail={{
                                        ...
                                        screenNameRender: attSaveTempSubmitWorkingOvertime
                                    }}
                                    scrollYAnimatedValue={this.scrollYAnimatedValue}
                                    rowActions={rowActions}
                                    selected={selected}
                                    reloadScreenList={this.reload.bind(this)}
                                    keyDataLocal={attSaveTempSubmitWorkingOvertimeKeyTask}
