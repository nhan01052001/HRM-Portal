...
export default class AttWorkDayCalendarListItem extends React.Component {
    constructor(props) {
        super(props);
        this.state = {};
    }
    ...
    renderListDataOvertime = (_ListDataOvertime, dataItem) => {
        return _ListDataOvertime.map((item, index) => {
            ...
            let timehour = null,
                startTime = item?.StartOvertime ? moment(item?.StartOvertime).format('HH:mm') : null,
                endTIme = item?.EndOvertime ? moment(item?.EndOvertime).format('HH:mm') : null;

            if (startTime || endTIme) {
                timehour = `${startTime ?? ''} ${startTime && endTIme ? '-' : ''} ${endTIme ?? ''}`;
            }
                ...
                >
                    <View style={styles.viewStatus}>
                        ...
                        >
                            ...
                            <Text style={[styleSheets.text, styles.txtRegister]}>
                                {item.DuratationTypeView ? `${item.DuratationTypeView}, ${item.OvertimeHours} ${translate('HRM_PortalApp_Hour_Lowercase')} (${timehour ?? ''})` : ''}
                            </Text>

                            {
                                dataItem.OvertimeConfirmMasterByDate ? (
                                    <Text style={[styleSheets.text, styles.txtRegister]}>
                                        {translate('HRM_PortalApp_Actual')}: {dataItem.OvertimeConfirmMasterByDate} {translate('HRM_PortalApp_Hour_Lowercase')}
                                    </Text>
                                ) : null
                            }
                        </View>
    ...
    renderProfileTimeSheet = _ListDataProfileTimeSheet => {
        return _ListDataProfileTimeSheet.map((item, index) => {
            let colorStatusView = null;

            if (item.Status) {
                const { colorStatus } = Vnr_Services.formatStyleStatusApp(item.Status);
                colorStatusView = colorStatus ? colorStatus : null;
            }
            ...
            return (
                <TouchableWithoutFeedback
                    key={index}
                    onPress={() => this.onPressToDetail(ScreenName.AttSubmitTakeDailyTask, item.ID)}
                >
                    <View style={styles.viewStatus}>
                        <View
                            style={[
                                styles.styViewTextType,
                                {
                                    borderLeftColor: colorStatusView ?? Colors.white
                                }
                            ]}
                        >
                            <VnrText
                                style={[styleSheets.lable, styles.txtLableTpye]}
                                i18nKey={'HRM_PortalApp_TimeSheet'}
                            />
    ...
    renderContent = () => {
        ...
        let { isMissIn, isMissOut, isLate, isEarly, InTime1, Outtime1 } = dataItem;
        ...
        if (dataItem.TamScanLogStatus && dataItem.Type != 'E_NORMAL') {
        ...
        return (
            <TouchableWithoutFeedback onPress={() => this.onPressContent()}>
                <View style={styles.styContent}>
                    ...
                    <View style={[styles.swipeable, CustomStyleSheet.flexDirection('column')]} key={index}>
    ...
    renderTimeLine = () => {
        ...
        if (dataItem.isFuture || dataItem.DayType == 'E_HOLIDAY' || dataItem.DayType == 'OFF') {
            ...
            (dataItem.Type != 'E_NORMAL' && dataItem.TamScanLogStatus !== EnumStatus.E_APPROVED)
        ) {
        ...
        } else if (dataItem.Type == 'E_NORMAL') {
            viewIconLine = (
        ...
        return (
            <View style={styles.containerItem}>
                ...
                {dataItem.ListDataOvertime &&
                    ...
                    this.renderListDataOvertime(dataItem.ListDataOvertime, dataItem)}
                ...
                {dataItem.ListDataProfileTimeSheet &&
                    dataItem.ListDataProfileTimeSheet.length > 0 &&
                    this.renderProfileTimeSheet(dataItem.ListDataProfileTimeSheet)}
            </View>
        );
    }
}
