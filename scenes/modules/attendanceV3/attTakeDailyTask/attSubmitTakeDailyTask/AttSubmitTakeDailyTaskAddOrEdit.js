...
import { ToasterInModal, ToasterSevice } from '../../../../../components/Toaster/Toaster.js';
import moment from 'moment';
import { translate } from '../../../../../i18n/translate.js';
import { AlertInModal } from '../../../../../components/Alert/Alert.js';
import { EnumName, EnumIcon } from '../../../../../assets/constant.js';
...
import AttSubmitTakeDailyTaskComponent from './AttSubmitTakeDailyTaskComponent.js';
...
export default class AttSubmitTakeDailyTaskAddOrEdit extends Component {
    constructor(props) {
        super(props);
    ...
    getConfigValid = () => {
        ...
        const tblName = 'Att__RegisterDailyWork_Form';
    ...
    handleSetState = (response) => {
        ...
        this.levelApprove = response.LevelApproved
            ? response.LevelApproved
            : Array.isArray(response?.ProcessApproval) ? response?.ProcessApproval.length : 4;
        ...
        this.setState(nextState, () => {
            ...
            if (params && params.listItem && params.listItem.length > 0) {
                ...
                if (Array.isArray(listday)) {
                    this.onChangeDateFromTo(listday)
                }
        ...
        this.AlertSevice.alert({
            ...
            onConfirm: () => {
                ...
                if (WorkDate.value) {
                    ...
                    this.listRefGetDataSave[`${WorkDate.value}`].unduData();
                }
            }
        });
    };
    ...
    onSave = (isSend) => {
        ...
        const {
                ...
                params } = this.state,
            { record } = params;
        if (!Array.isArray(WorkDate.value) || WorkDate.value.length === 0)
            return;
        ...
        WorkDate.value.map(item => {
            if (typeof this.listRefGetDataSave[`${item}`].getAllData !== 'function' || !this.listRefGetDataSave[`${item}`].getAllData())
                return;
            ListTimeSheetRegisterItem = [
                ...ListTimeSheetRegisterItem,
                {
                    ...
                    ProfileID: Profile.ID,
                    Status: isSend ? 'E_SUBMIT' : 'E_SUBMIT_TEMP'
                }
            ]
        });
        ...
        let payload = {
            ...
            UserSubmitID: dataVnrStorage.currentUser ? dataVnrStorage.currentUser.info.userid : null
        };

        if (ListTimeSheetRegisterItem.length > 0 && isDataComplete) {
            payload = {
                ...payload,
            ...
            this.showLoading(true);
            HttpService.Post('[URI_CENTER]/api/Att_Roster/CreateOrUpdateProfileTimeSheetRegister', payload).then((res) => {
                ...
                if (res && typeof res === EnumName.E_object) {
                    if (res.Status == EnumName.E_SUCCESS) {
                        ...
                        if (res.Data) {
                            if (res.Data.IsBlock || res?.Message === 'Locked') {
    ...
    onChangeDateFromTo = async (range) => {
        const { WorkDate, isRefresh, isShowModal } = this.state;
        this.listRefGetDataSave = {};
    ...
    onDeleteItemDay = (index) => {
        this.AlertSevice.alert({
            ...
            onConfirm: () => {
                const { WorkDate } = this.state;
                if (WorkDate.value && Array.isArray(WorkDate.value) && WorkDate.value.length > 1) {
                    if (this.listRefGetDataSave[WorkDate.value[index]]) {
                        delete this.listRefGetDataSave[WorkDate.value[index]];
                    }
                    // Xóa
                    WorkDate.value.splice(index, 1);
                    ...
                    renderItem={({ item, index }) => (
                        ...
                        >
                            <AttSubmitTakeDailyTaskComponent
                                ...
                                onChangeDate={(days) => this.onChangeDateFromTo(days)}
                            />
                        </View>
                    )}
        ...
        return (
            <View style={styles.container}>
                <VnrDateFromTo
                    ...
                    refresh={WorkDate.refresh}
                    value={WorkDate.value ? WorkDate.value : {}}
                        ...
                        <SafeAreaView style={styleComonAddOrEdit.wrapInsideModal}>
                            ...
                            <View style={styleComonAddOrEdit.flRowSpaceBetween}>
                                <VnrText
                                    ...
                                    i18nKey={
                                        this.isModify
                                            ? 'HRM_PortalApp_DailyTasksEdit'
                                            : 'HRM_PortalApp_DailyTasksSubmission'
                                    }
                                />
