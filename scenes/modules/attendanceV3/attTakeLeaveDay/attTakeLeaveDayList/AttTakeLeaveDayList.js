...
import AttTakeLeaveDayListItemApprove from './AttTakeLeaveDayListItemApprove';
import AttTakeLeaveDayListItem from './AttTakeLeaveDayListItem';
...
export default class AttTakeLeaveDayList extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            //valueField: "ID",
            ...
            getDataLocal(keyDataLocal)
                .then(resData => {
                    ...
                    if (res && res !== EnumName.E_EMPTYDATA) {
                        ...
                        if (detail?.screenName === ScreenName.AttApproveTakeLeaveDay) {
                            data.map(item => {
                                item.isSelect = false;
                        ...
                        } else if (detail?.screenName == ScreenName.AttApprovedTakeLeaveDay) {
                            data.map(item => {
                                item.isSelect = false;
        ...
        if (itemChecked) {
            ...
            this.setState(
                ...
                () => {
                    ...
                    //     let allData = [];
                    //     dataSource.map((item) => {
                    //        if(Array.isArray(item?.dataGroupMaster)) {
                    //         allData = [...allData, ...item?.dataGroupMaster]
                    //        }
                    //     });
                    //     const TEMP = allData.find((item) => item?.isSelect === false);
                    //     if(TEMP) {
                    //         this.setState({
                    //             isCheckAll: false,
                    //         })
                        ...
                        >
                            <View
                                style={[
                                    ...
                                    isCheckAll &&
                                    isCheckAll === true && styles.borderRadius
                                ]}
                            >
                            ...
                            <View style={styles.styleViewBorderButtom}>
                                <TouchableWithoutFeedback>
                                    <View style={{ ...CustomStyleSheet.flex(1) }}>
                                        {detail?.screenName === ScreenName.AttApproveTakeLeaveDay ? (
                                            <AttTakeLeaveDayListItemApprove
                                                key={index}
    ...
    render() {
        ...
        const hiddenFiled = {
            DurationTypeView: Vnr_Function.checkIsHaveConfigListDetail(detail?.screenName, 'DurationTypeView'),
            LeaveDayTypeName: Vnr_Function.checkIsHaveConfigListDetail(detail?.screenName, 'LeaveDayTypeName'),
            WarningViolation: Vnr_Function.checkIsHaveConfigListDetail(detail?.screenName, 'WarningViolation'),
            Comment: Vnr_Function.checkIsHaveConfigListDetail(detail?.screenName, 'Comment')
        };
        ...
        if (isLoading) {
            ...
            contentList = (
                <FlatList
                    ...
                    style={this.isHaveFilter ? styles.searchIsHaveFilter : styles.searchIsNotHaveFilter}
                        ...
                        <View style={[styles.containBotton]}>
                            ...
                            {item?.isGroup ? (
                                ...
                                >
                                    <View style={styles.styleViewBorderButtom}>
                                        <View style={CustomStyleSheet.flex(1)}>
                                            {detail.screenName !== ScreenName.AttSubmitTakeLeaveDay ? (
                                                <AttTakeLeaveDayListItemApprove
                                                    key={index}
                                                    currentDetail={detail}
        ...
        let isShowBtn =
            permissionBtnCreate &&
            (detail.screenNameRender === ScreenName.AttSubmitTakeLeaveDay ||
                detail.screenNameRender === ScreenName.AttSaveTempSubmitTakeLeaveDay);
        return (
            <View
                style={[
