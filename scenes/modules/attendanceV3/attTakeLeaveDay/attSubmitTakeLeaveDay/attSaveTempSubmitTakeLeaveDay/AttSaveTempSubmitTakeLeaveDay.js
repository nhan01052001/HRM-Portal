...
let configList = null,
    enumName = null,
    attSaveTempSubmitTakeLeaveDay = null,
    attSubmitTakeLeaveDay = null,
    attSubmitTakeLeaveDayViewDetail = null,
    attSaveTempSubmitTakeLeaveDayKeyTask = null,
    pageSizeList = 20;

class AttSaveTempSubmitTakeLeaveDay extends Component {
    constructor(props) {
        super(props);
        ...
        this.willFocusScreen = this.props.navigation.addListener('willFocus', () => {
            ...
            if (AttSubmitTakeLeaveDayBusinessFunction.checkForReLoadScreen[attSaveTempSubmitTakeLeaveDay]) {
                this.reload();
            }
        });
    }
    ...
    reload = paramsFilter => {
        ...
        AttSubmitTakeLeaveDayBusinessFunction.checkForReLoadScreen[attSaveTempSubmitTakeLeaveDay] = false;

        // Gửi yêu cầu lọc dữ liệu
        this.setState(_paramsDefault, () => {
            const { dataBody, keyQuery } = this.state;
            startTask({
                keyTask: attSaveTempSubmitTakeLeaveDayKeyTask,
                payload: {
                    ...dataBody,
                    keyQuery: keyQuery,
                    isCompare: false,
    ...
    paramsDefault = () => {
        if (!attSaveTempSubmitTakeLeaveDay) {
            attSaveTempSubmitTakeLeaveDay = ScreenName.AttSaveTempSubmitTakeLeaveDay;
        }
        if (!configList[attSaveTempSubmitTakeLeaveDay]) {
            configList[attSaveTempSubmitTakeLeaveDay] = {
                Api: {
        ...
        const _configList = configList[attSaveTempSubmitTakeLeaveDay],
            filter = _configList[enumName.E_Filter],
            dataFromParams = this.checkDataFormNotify();

        const dataRowActionAndSelected = generateRowActionAndSelected(attSaveTempSubmitTakeLeaveDay);
        let _params = {
            ...dataFromParams,
            IsPortalNew: true,
            filter: filter,
    ...
    UNSAFE_componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.reloadScreenName == attSaveTempSubmitTakeLeaveDayKeyTask) {
    ...
    componentDidMount() {
        ...
        attSaveTempSubmitTakeLeaveDayKeyTask = EnumTask.KT_AttSaveTempSubmitTakeLeaveDay;
        AttSubmitTakeLeaveDayBusinessFunction.checkForReLoadScreen[attSaveTempSubmitTakeLeaveDay] = false;
    ...
    render() {
        ...
        return (
            <SafeAreaViewDetail style={styleSafeAreaView.style}>
                {attSaveTempSubmitTakeLeaveDay && attSubmitTakeLeaveDayViewDetail && enumName && (
                            ...
                            {keyQuery && (
                                <AttTakeLeaveDayList
                                    detail={{
                                        ...
                                        screenName: attSubmitTakeLeaveDay,
                                        screenNameRender: attSaveTempSubmitTakeLeaveDay
                                    }}
                                    scrollYAnimatedValue={this.scrollYAnimatedValue}
                                    rowActions={rowActions}
                                    selected={selected}
                                    reloadScreenList={this.reload.bind(this)}
                                    keyDataLocal={attSaveTempSubmitTakeLeaveDayKeyTask}
