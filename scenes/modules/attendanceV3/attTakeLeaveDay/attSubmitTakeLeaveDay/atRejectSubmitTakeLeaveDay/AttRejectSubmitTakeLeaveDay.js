...
let configList = null,
    enumName = null,
    attRejectSubmitTakeLeaveDay = null,
    attSubmitTakeLeaveDay = null,
    attSubmitTakeLeaveDayViewDetail = null,
    attRejectSubmitTakeLeaveDayKeyTask = null,
    pageSizeList = 20;

class AttRejectSubmitTakeLeaveDay extends Component {
    constructor(props) {
        super(props);
        ...
        this.willFocusScreen = this.props.navigation.addListener('willFocus', () => {
            ...
            if (AttSubmitTakeLeaveDayBusinessFunction.checkForReLoadScreen[attRejectSubmitTakeLeaveDay]) {
                this.reload();
            }
        });
    }
    ...
    reload = paramsFilter => {
        ...
        AttSubmitTakeLeaveDayBusinessFunction.checkForReLoadScreen[attRejectSubmitTakeLeaveDay] = false;

        // Gửi yêu cầu lọc dữ liệu
        this.setState(_paramsDefault, () => {
            const { dataBody, keyQuery } = this.state;
            startTask({
                keyTask: attRejectSubmitTakeLeaveDayKeyTask,
                payload: {
                    ...dataBody,
                    keyQuery: keyQuery,
                    isCompare: false,
    ...
    paramsDefault = () => {
        if (!attRejectSubmitTakeLeaveDay) {
            attRejectSubmitTakeLeaveDay = ScreenName.AttRejectSubmitTakeLeaveDay;
        }
        if (!configList[attRejectSubmitTakeLeaveDay]) {
            configList[attRejectSubmitTakeLeaveDay] = {
                Api: {
        ...
        const _configList = configList[attRejectSubmitTakeLeaveDay],
            filter = _configList[enumName.E_Filter],
            dataFromParams = this.checkDataFormNotify();

        const dataRowActionAndSelected = generateRowActionAndSelected(attRejectSubmitTakeLeaveDay);
        let _params = {
            ...dataFromParams,
            IsPortalNew: true,
            filter: filter,
    ...
    UNSAFE_componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.reloadScreenName == attRejectSubmitTakeLeaveDayKeyTask) {
    ...
    componentDidMount() {
        ...
        attRejectSubmitTakeLeaveDayKeyTask = EnumTask.KT_AttRejectSubmitTakeLeaveDay;
        AttSubmitTakeLeaveDayBusinessFunction.checkForReLoadScreen[attRejectSubmitTakeLeaveDay] = false;
        //set by config
    ...
    render() {
        ...
        return (
            <SafeAreaViewDetail style={styleSafeAreaView.style}>
                {attRejectSubmitTakeLeaveDay && attSubmitTakeLeaveDayViewDetail && enumName && (
                            ...
                            {keyQuery && (
                                <AttTakeLeaveDayList
                                    detail={{
                                        ...
                                        screenName: attSubmitTakeLeaveDay,
                                        screenNameRender: attRejectSubmitTakeLeaveDay
                                    }}
                                    scrollYAnimatedValue={this.scrollYAnimatedValue}
                                    rowActions={rowActions}
                                    selected={selected}
                                    reloadScreenList={this.reload.bind(this)}
                                    keyDataLocal={attRejectSubmitTakeLeaveDayKeyTask}
