...
import AttLeaveDayReplacementListItem from './AttLeaveDayReplacementListItem';

const heightActionBottom = 45;
export default class AttLeaveDayReplacementlist extends VnrRenderList {
    remoteData = (param = {}) => {
                ...
                .then(resData => {
                    ...
                    if (res && res !== EnumName.E_EMPTYDATA) {
                        ...
                        data.map(item => {
                            ...
                            if (item.Status === 'E_CONFIRM' || item.Status === 'E_APPROVED') {
    ...
    moveToDetail = item => {
        ...
        if (!Vnr_Function.CheckIsNullOrEmpty(rowTouch) && typeof rowTouch == 'function') {
            ...
            DrawerServices.navigate(detail.screenDetail, {
                ...
                screenNameRender: detail.screenNameRender,
    ...
    renderDataOfGroup = (data, isGroup, indexInDataSource) => {
        const { dataSource, isPullToRefresh, isOpenAction, isDisableSelectItem } = this.state,
            { detail, rowActions, renderConfig, keyDataLocal } = this.props;
                            ...
                            <View style={styles.styleViewBorderButtom}>
                                <TouchableWithoutFeedback>
                                    <View style={CustomStyleSheet.flex(1)}>
                                        <AttLeaveDayReplacementListItem
                                            renderConfig={renderConfig}
                                            key={index}
                                            keyDataLocal={keyDataLocal}
    ...
    render() {
        const {
            ...
            { api, detail, rowActions, onCreate, scrollYAnimatedValue, renderConfig, keyDataLocal } = this.props,
            permissionBtnCreate =
        ...
        if (isLoading) {
            let typeLoading = detail.screenName == ScreenName.HreDoneRecruitmentProposalProcessing
        ...
        let isShowBtn =
            permissionBtnCreate &&
            (detail.screenNameRender === ScreenName.HreDoneRecruitmentProposalProcessing ||
                detail.screenNameRender === ScreenName.HreWaitRecruitmentProposalProcessing);
        return (
            <View
                style={[
