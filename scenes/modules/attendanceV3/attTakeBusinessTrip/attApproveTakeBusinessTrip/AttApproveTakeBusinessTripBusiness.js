...
let enumName = EnumName,
    ...
    attApproveTakeBusinessTrip = ScreenName.AttApproveTakeBusinessTrip,
    apiConfig = null,
    headers = null;
            ...
            _rowActions = [
                {
                    ...
                    onPress: (item, dataBody) =>
                        AttApproveTakeBusinessTripBusinessFunction.businessApproveRecords(
            ...
            _selected = [
                {
                    ...
                    type: E_APPROVE,
                    onPress: (item, dataBody) =>
                        AttApproveTakeBusinessTripBusinessFunction.businessApproveRecords(item, dataBody),
                    ...actionApprove
                }
            ];
        }
        ...
        if (actionRejectPer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_REJECT,
                    onPress: (item, dataBody) =>
                        AttApproveTakeBusinessTripBusinessFunction.businessRejectRecords(
                            Array.isArray(item) ? item : [{ ...item }],
                            dataBody
                        ),
                    ...actionReject
            ...
            _selected = [
                ..._selected,
                {
                    ...
                    type: E_REJECT,
                    onPress: (item, dataBody) =>
                        AttApproveTakeBusinessTripBusinessFunction.businessRejectRecords(item, dataBody),
                    ...actionReject
                }
            ];
        }
        ...
        if (actionCancelPer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_CANCEL,
                    onPress: (item, dataBody) =>
                        AttApproveTakeBusinessTripBusinessFunction.businessCancelRecords(
                            Array.isArray(item) ? item : [{ ...item }],
                            dataBody
                        ),
                    ...actionCancel
            ...
            _selected = [
                ..._selected,
                {
                    ...
                    onPress: (items, dataBody) =>
                        AttApproveTakeBusinessTripBusinessFunction.businessCancelRecords(items, dataBody),
                    ...actionCancel
                }
            ];
        }
...
export const AttApproveTakeBusinessTripBusinessFunction = {
    // Đối tượng checkForReLoadScreen để check có refresh AttApprovedTakeBusinessTrip không
    checkForReLoadScreen: {
        [ScreenName.AttApprovedTakeBusinessTrip]: false,
        AttRejectTakeBusinessTrip: false,
    ...
    checkRequireNote: async (field) => {
        try {
            const res = await HttpService.Get(
                '[URI_CENTER]/api/Att_GetData/GetConfigNoteValidate?business=E_BUSINESSTRAVEL'
            );
    ...
    businessApproveRecords: (items, dataBody) => {
        if (items.length === 0 && !dataBody) {
            ...
            if (arrValid.length > 0) {
                ...
                AttApproveTakeBusinessTripBusinessFunction.validateApproveBusinessTrip({
                    ListRecord: arrValid,
                    message,
    ...
    validateApproveBusinessTrip: objValid => {
        ...
        if (objValid && Array.isArray(objValid?.ListRecord) && objValid?.ListRecord.length > 0) {
            ...
            HttpService.Post('[URI_CENTER]/api/Att_BussinessTravel/ValidateApproveBusinessTrip', {
                ...
                .then(res => {
                    ...
                    if (res) {
                        if (res.Status === 'SUCCESS' && res.Data && Array.isArray(res.Data) && res.Data.length > 0) {
                            AttApproveTakeBusinessTripBusinessFunction.confirmApprove({
                                ListRecord: {
                                    RecordID: res.Data,
                                    Comment: '',
                                    Type: 'E_APPROVED'
            ...
            AlertSevice.alert({
                ...
                onConfirm: reason => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        //     return item;
                        // });
                        HttpService.Post('[URI_CENTER]/api/Att_BussinessTravel/SetApproveBusinessTripInPortal', {
                            Host: apiConfig.uriPor,
                            ...
                            try {
                                if (res) {
                                    if (res.Status === 'SUCCESS') {
                                        ...
                                        AttApproveTakeBusinessTripBusinessFunction.checkForReLoadScreen[
                                        ...
                                        if (!_isOnScreenNotification) DrawerServices.navigate('AttApproveTakeBusinessTrip');
                                    } else if (res.Status === 'Locked') {
                                        ToasterSevice.showWarning('Hrm_Locked', 5000);
                                    } else {
                                        ToasterSevice.showWarning(res.Message, 5000);
    ...
    businessRejectRecords: (items, dataBody) => {
        if (items.length === 0 && !dataBody) {
            ...
            if (arrValid.length > 0) {
                ...
                AttApproveTakeBusinessTripBusinessFunction.validateRejectLeaveDay({
                    ListRecord: arrValid,
                    message,
                    allItems: Array.isArray(items) ? items.length : 0
                });
    ...
    validateRejectLeaveDay: objValid => {
        if (objValid && Array.isArray(objValid?.ListRecord) && objValid?.ListRecord.length > 0) {
            ...
            VnrLoadingSevices.show();
            HttpService.Post('[URI_CENTER]/api/Att_BussinessTravel/ValidateRejectBusinessTrip', {
                Host: apiConfig.uriPor,
                ListRecordID: listID,
                UserLogin: headers.userlogin,
                UserProcessID: headers.userid
                ...
                .then(res => {
                    ...
                    if (res) {
                        if (res.Status === 'SUCCESS' && res.Data && Array.isArray(res.Data) && res.Data.length > 0) {
                            AttApproveTakeBusinessTripBusinessFunction.confirmReject({
                                ListRecord: {
                                    RecordID: res.Data,
                                    Comment: '',
                                    Type: 'E_APPROVED'
            ...
            AlertSevice.alert({
                ...
                onConfirm: reason => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        HttpService.Post('[URI_CENTER]/api/Att_BussinessTravel/SetRejectBusinessTripInPortal', {
                            Host: apiConfig.uriPor,
                            ListRecordID: objValid.ListRecord.RecordID,
                            UserLogin: headers.userlogin,
                            UserProcessID: headers.userid,
                            ...
                            try {
                                if (res) {
                                    if (res.Status === 'SUCCESS') {
                                        ...
                                        NotificationsService.getListUserPushNotify();
                                        _this.reload('E_KEEP_FILTER');
                                        // set true để refresh AttApprovedTakeBusinessTrip
                                        AttApproveTakeBusinessTripBusinessFunction.checkForReLoadScreen[
                                            ScreenName.AttApproveTakeBusinessTrip
                                        ] = true;
                                        AttApproveTakeBusinessTripBusinessFunction.checkForReLoadScreen[
                ...
                try {
                    if (res) {
                        if (res.Status === 'SUCCESS') {
                            ...
                            store.dispatch(badgesNotification.actions.fetchCountNumberApproveInfo());
                            // nếu (duyệt/từ chối) ở màn hình notify thì không chuyển hướng.
                            if (!_isOnScreenNotification) DrawerServices.navigate('AttApprovedTakeBusinessTrip');
                        } else if (res.Status === 'Locked') {
                            ToasterSevice.showWarning('Hrm_Locked', 5000);
                        } else {
                            ToasterSevice.showWarning(res.Message, 5000);
    ...
    businessCancelRecords: (items, dataBody) => {
        if (items.length === 0 && !dataBody) {
            ...
            if (arrValid.length > 0 && ListRecordID.length > 0 && totalList.length > 0) {
                ...
                AttApproveTakeBusinessTripBusinessFunction.validateCancleTakeBusinessTrip({ ListRecordID, message, totalList });
            } else {
    ...
    validateCancleTakeBusinessTrip: objValid => {
        if (objValid && Array.isArray(objValid?.ListRecordID) && objValid?.ListRecordID.length > 0) {
            ...
            HttpService.Post('[URI_CENTER]/api/Att_BussinessTravel/ValidateCancelBusinessTravel', {
                ...
                .then(res => {
                    ...
                    if (res) {
                        if (res.Status === 'SUCCESS' && res.Data && Array.isArray(res.Data) && res.Data.length > 0) {
                            AttApproveTakeBusinessTripBusinessFunction.confirmCancel({
                                ListRecord: {
                                    RecordID: res.Data,
                                    Comment: '',
                                    Type: 'E_CANCEL'
            ...
            AlertSevice.alert({
                ...
                onConfirm: reason => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        VnrLoadingSevices.show();
                        HttpService.Post('[URI_CENTER]/api/Att_BussinessTravel/SetCancelBusinessTripInPortalForApproved', {
                            Host: apiConfig.uriPor,
                            ListRecordID: objValid.ListRecord.RecordID,
                            UserLogin: headers.userlogin,
                            UserProcessID: headers.userid,
...
export default {
    ...
    AttApproveTakeBusinessTripBusinessFunction: AttApproveTakeBusinessTripBusinessFunction
};
