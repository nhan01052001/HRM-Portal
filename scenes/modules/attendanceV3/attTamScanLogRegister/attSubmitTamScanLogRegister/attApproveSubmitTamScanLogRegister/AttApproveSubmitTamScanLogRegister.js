...
let configList = null,
    enumName = null,
    attApproveSubmitTamScanLogRegister = null,
    attSubmitTamScanLogRegister = null,
    attSubmitTamScanLogRegisterViewDetail = null,
    attApproveSubmitTamScanLogRegisterKeyTask = null,
    pageSizeList = 20;

class AttApproveSubmitTamScanLogRegister extends Component {
    constructor(props) {
        super(props);
        ...
        this.willFocusScreen = this.props.navigation.addListener('willFocus', () => {
            ...
            if (AttSubmitTamScanLogRegisterBusinessFunction.checkForReLoadScreen[attApproveSubmitTamScanLogRegister]) {
                this.reload();
            }
        });
    }
    ...
    reload = paramsFilter => {
        ...
        AttSubmitTamScanLogRegisterBusinessFunction.checkForReLoadScreen[attApproveSubmitTamScanLogRegister] = false;

        // Gửi yêu cầu lọc dữ liệu
        this.setState(_paramsDefault, () => {
            const { dataBody, keyQuery } = this.state;
            startTask({
                keyTask: attApproveSubmitTamScanLogRegisterKeyTask,
                payload: {
                    ...dataBody,
                    keyQuery: keyQuery,
                    isCompare: false,
    ...
    paramsDefault = () => {
        if (!attApproveSubmitTamScanLogRegister) {
            attApproveSubmitTamScanLogRegister = ScreenName.AttApproveSubmitTamScanLogRegister;
        }
        if (!configList[attApproveSubmitTamScanLogRegister]) {
            configList[attApproveSubmitTamScanLogRegister] = {
                Api: {
                ...
                BusinessAction: [
                    {
                        Type: 'E_CANCEL',
                        Resource: {
                            Name: 'NewPortal_Att_TamScanLogRegister_Index_btnCancel',
                            Rule: 'View'
                        },
                        Confirm: {
                            isInputText: true,
        ...
        const _configList = configList[attApproveSubmitTamScanLogRegister],
            filter = _configList[enumName.E_Filter],
            dataFromParams = this.checkDataFormNotify();

        const dataRowActionAndSelected = generateRowActionAndSelected(attApproveSubmitTamScanLogRegister);
        let _params = {
            ...dataFromParams,
            IsPortalNew: true,
            filter: filter,
            pageSize: pageSizeList,
            Status: 'E_APPROVED1,E_APPROVED2,E_APPROVED3,E_SUBMIT'
        };
    ...
    UNSAFE_componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.reloadScreenName == attApproveSubmitTamScanLogRegisterKeyTask) {
    ...
    componentDidMount() {
        ...
        attApproveSubmitTamScanLogRegisterKeyTask = EnumTask.KT_AttApproveSubmitTamScanLogRegister;
        AttSubmitTamScanLogRegisterBusinessFunction.checkForReLoadScreen[attApproveSubmitTamScanLogRegister] = false;
    ...
    render() {
        ...
        return (
            <SafeAreaViewDetail style={styleSafeAreaView.style}>
                {attApproveSubmitTamScanLogRegister && attSubmitTamScanLogRegisterViewDetail && enumName && (
                            ...
                            {keyQuery && (
                                <AttTamScanLogRegisterList
                                    detail={{
                                        ...
                                        screenName: attSubmitTamScanLogRegister,
                                        screenNameRender: attApproveSubmitTamScanLogRegister
                                    }}
                                    scrollYAnimatedValue={this.scrollYAnimatedValue}
                                    rowActions={rowActions}
                                    selected={selected}
                                    reloadScreenList={this.reload.bind(this)}
                                    keyDataLocal={attApproveSubmitTamScanLogRegisterKeyTask}
