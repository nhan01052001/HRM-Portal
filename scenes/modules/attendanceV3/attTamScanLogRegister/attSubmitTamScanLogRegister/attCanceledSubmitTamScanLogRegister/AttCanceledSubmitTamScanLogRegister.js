...
let configList = null,
    enumName = null,
    attCanceledSubmitTamScanLogRegister = null,
    attSubmitTamScanLogRegister = null,
    attSubmitTamScanLogRegisterViewDetail = null,
    attCanceledSubmitTamScanLogRegisterKeyTask = null,
    pageSizeList = 20;

class AttCanceledSubmitTamScanLogRegister extends Component {
    constructor(props) {
        super(props);
        ...
        this.willFocusScreen = this.props.navigation.addListener('willFocus', () => {
            ...
            if (AttSubmitTamScanLogRegisterBusinessFunction.checkForReLoadScreen[attCanceledSubmitTamScanLogRegister]) {
                this.reload();
            }
        });
    }
    ...
    reload = paramsFilter => {
        ...
        AttSubmitTamScanLogRegisterBusinessFunction.checkForReLoadScreen[attCanceledSubmitTamScanLogRegister] = false;

        // Gửi yêu cầu lọc dữ liệu
        this.setState(_paramsDefault, () => {
            const { dataBody, keyQuery } = this.state;
            startTask({
                keyTask: attCanceledSubmitTamScanLogRegisterKeyTask,
                payload: {
                    ...dataBody,
                    keyQuery: keyQuery,
                    isCompare: false,
    ...
    paramsDefault = () => {
        if (!attCanceledSubmitTamScanLogRegister) {
            attCanceledSubmitTamScanLogRegister = ScreenName.AttCanceledSubmitTamScanLogRegister;
        }
        if (!configList[attCanceledSubmitTamScanLogRegister]) {
            configList[attCanceledSubmitTamScanLogRegister] = {
                Api: {
        ...
        const _configList = configList[attCanceledSubmitTamScanLogRegister],
            filter = _configList[enumName.E_Filter],
            dataFromParams = this.checkDataFormNotify();

        const dataRowActionAndSelected = generateRowActionAndSelected(attCanceledSubmitTamScanLogRegister);
        let _params = {
            ...dataFromParams,
            IsPortalNew: true,
            filter: filter,
    ...
    UNSAFE_componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.reloadScreenName == attCanceledSubmitTamScanLogRegisterKeyTask) {
    ...
    componentDidMount() {
        ...
        attCanceledSubmitTamScanLogRegisterKeyTask = EnumTask.KT_AttCanceledSubmitTamScanLogRegister;
        AttSubmitTamScanLogRegisterBusinessFunction.checkForReLoadScreen[attCanceledSubmitTamScanLogRegister] = false;
    ...
    render() {
        ...
        return (
            <SafeAreaViewDetail style={styleSafeAreaView.style}>
                {attCanceledSubmitTamScanLogRegister && attSubmitTamScanLogRegisterViewDetail && enumName && (
                            ...
                            {keyQuery && (
                                <AttTamScanLogRegisterList
                                    detail={{
                                        ...
                                        screenName: attSubmitTamScanLogRegister,
                                        screenNameRender: attCanceledSubmitTamScanLogRegister
                                    }}
                                    scrollYAnimatedValue={this.scrollYAnimatedValue}
                                    rowActions={rowActions}
                                    selected={selected}
                                    reloadScreenList={this.reload.bind(this)}
                                    keyDataLocal={attCanceledSubmitTamScanLogRegisterKeyTask}
