...
let configList = null,
    enumName = null,
    attApprovedSubmitTamScanLogRegister = null,
    attSubmitTamScanLogRegister = null,
    attSubmitTamScanLogRegisterViewDetail = null,
    attApprovedSubmitTamScanLogRegisterKeyTask = null,
    pageSizeList = 20;

class AttApprovedSubmitTamScanLogRegister extends Component {
    constructor(props) {
        super(props);
        ...
        this.willFocusScreen = this.props.navigation.addListener('willFocus', () => {
            ...
            if (AttSubmitTamScanLogRegisterBusinessFunction.checkForReLoadScreen[attApprovedSubmitTamScanLogRegister]) {
                this.reload();
            }
        });
    }
    ...
    reload = paramsFilter => {
        ...
        AttSubmitTamScanLogRegisterBusinessFunction.checkForReLoadScreen[attApprovedSubmitTamScanLogRegister] = false;

        // Gửi yêu cầu lọc dữ liệu
        this.setState(_paramsDefault, () => {
            const { dataBody, keyQuery } = this.state;
            startTask({
                keyTask: attApprovedSubmitTamScanLogRegisterKeyTask,
                payload: {
                    ...dataBody,
                    keyQuery: keyQuery,
                    isCompare: false,
    ...
    paramsDefault = () => {
        if (!attApprovedSubmitTamScanLogRegister) {
            attApprovedSubmitTamScanLogRegister = ScreenName.AttApprovedSubmitTamScanLogRegister;
        }
        if (!configList[attApprovedSubmitTamScanLogRegister]) {
            configList[attApprovedSubmitTamScanLogRegister] = {
                Api: {
        ...
        const _configList = configList[attApprovedSubmitTamScanLogRegister],
            filter = _configList[enumName.E_Filter],
            dataFromParams = this.checkDataFormNotify();

        const dataRowActionAndSelected = generateRowActionAndSelected(attApprovedSubmitTamScanLogRegister);
        let _params = {
            ...dataFromParams,
            IsPortalNew: true,
            filter: filter,
    ...
    UNSAFE_componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.reloadScreenName == attApprovedSubmitTamScanLogRegisterKeyTask) {
    ...
    componentDidMount() {
        ...
        attApprovedSubmitTamScanLogRegisterKeyTask = EnumTask.KT_AttApprovedSubmitTamScanLogRegister;
        AttSubmitTamScanLogRegisterBusinessFunction.checkForReLoadScreen[attApprovedSubmitTamScanLogRegister] = false;
    ...
    render() {
        ...
        return (
            <SafeAreaViewDetail style={styleSafeAreaView.style}>
                {attApprovedSubmitTamScanLogRegister && attSubmitTamScanLogRegisterViewDetail && enumName && (
                            ...
                            {keyQuery && (
                                <AttTamScanLogRegisterList
                                    detail={{
                                        ...
                                        screenName: attSubmitTamScanLogRegister,
                                        screenNameRender: attApprovedSubmitTamScanLogRegister
                                    }}
                                    scrollYAnimatedValue={this.scrollYAnimatedValue}
                                    rowActions={rowActions}
                                    selected={selected}
                                    reloadScreenList={this.reload.bind(this)}
                                    keyDataLocal={attApprovedSubmitTamScanLogRegisterKeyTask}
