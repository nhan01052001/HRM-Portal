...
import AttTakeLateEarlyAllowedListItem from './AttTakeLateEarlyAllowedListItem';
import AttTakeLateEarlyAllowedListItemApprove from './AttTakeLateEarlyAllowedListItemApprove';
...
export default class AttTakeLateEarlyAllowedList extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            //valueField: "ID",
    ...
    renderSeparator = () => {
        return (
            <View
                style={
                    stylesScreenDetailV3.separator
                }
            />
        );
    };
            ...
            getDataLocal(keyDataLocal)
                .then(resData => {
                    ...
                    if (res && res !== EnumName.E_EMPTYDATA) {
                        ...
                        if (detail?.screenName === ScreenName.AttApproveTakeLateEarlyAllowed) {
                            data.map(item => {
                                item.isSelect = false;
                        ...
                        } else if (detail?.screenName == ScreenName.AttApprovedTakeLateEarlyAllowed) {
                            data.map(item => {
                                item.isSelect = false;
    ...
    _renderFooterLoading = () => {
        return (
            <View
                style={
                    stylesScreenDetailV3.loadingFooter
                }
            >
                        ...
                        >
                            <View
                                style={[
                                    ...
                                    isCheckAll &&
                                        isCheckAll === true && styles.checkAll
                                ]}
                            >
        ...
        return (
            <ScrollView>
                {data.map((value, index) => {
                    return (
                        <View key={index} style={[styles.containBotton, index === data.length - 1 && CustomStyleSheet.marginBottom(22)]}>
                            <View style={styles.styleViewBorderButtom}>
                                <TouchableWithoutFeedback>
                                    <View style={CustomStyleSheet.flex(1)}>
                                        {detail?.screenName === ScreenName.AttApproveTakeLateEarlyAllowed ||
                                        detail.screenName === ScreenName.AttApprovedTakeLateEarlyAllowed ? (
                                                <AttTakeLateEarlyAllowedListItemApprove
                                                    key={index}
                                                ...
                                                <AttTakeLateEarlyAllowedListItem
                                                    key={index}
                                                    currentDetail={detail}
                                                    onClick={() => {
                                                        this.addItemChecked(index, isGroup, indexInDataSource);
    ...
    render() {
        const {
            ...
            permissionBtnCreate =
                ...
                PermissionForAppMobile.value['New_Att_LateEarlyAllowed_New_Index_V2'] &&
        ...
        let isShowBtn =
            permissionBtnCreate &&
            (detail.screenNameRender === ScreenName.AttSubmitTakeLateEarlyAllowed ||
                detail.screenNameRender === ScreenName.AttSaveTempSubmitTakeLateEarlyAllowed);
        return (
            <View
                style={[
