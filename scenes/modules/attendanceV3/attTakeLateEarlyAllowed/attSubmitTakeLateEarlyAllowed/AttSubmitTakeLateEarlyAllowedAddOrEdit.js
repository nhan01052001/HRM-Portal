...
import { IconCloseCircle, IconCancel } from '../../../../../constants/Icons';
...
export default class AttSubmitTakeLateEarlyAllowedAddOrEdit extends Component {
    ...
    getConfigValid = () => {
        ...
        const tblName = 'Attendance_FormRegisterLateEarly';
    ...
    onSave = isSend => {
        ...
        if (lstLateEarlyItem.length > 0) {
            ...
            this.showLoading(true);
            HttpService.Post('[URI_CENTER]/api/Att_LateEarlyAllowed/CreateOrUpdateLateEarlyAllowed', payload).then(
                res => {
    ...
    handleUpdateDateLateEarly = (value, index) => {
        const { DateFromTo } = this.state;
            ...
            return (
                <FlatList
                    ...
                    renderItem={({ item, index }) => (
                        <AttSubmitTakeLateEarlyAllowedComponent
                            key={item}
                            ref={ref => {
                            ...
                            isShowDelete={DateFromTo.value.length > 1 ? true : false}
                            value={[item]}
                            showLoading={this.showLoading}
                            ToasterSevice={() => this.ToasterSeviceCallBack()}
                            onUpdateDateRegister={(value, index) => {
                                this.handleUpdateDateLateEarly(value, index);
                            }}
                        />
                    )}
    ...
    onChangeWorkDate = item => {
        const { DateFromTo, isRefresh, isConfigMultiShift } = this.state;
            ...
            this.setState(
                {
                    DateFromTo: {
                        ...DateFromTo,
                        value: [moment(item[0]).format('YYYY-MM-DD HH:mm:ss')],
                        refresh: !DateFromTo.refresh
                    },
                    isRefresh: !isRefresh,
                    isShowModal: true
    ...
    getShiftByWorkDayLateEarly = () => {
        const { DateFromTo, Profile, ShiftID, LateEarlyType } = this.state;

        this.showLoading(true);

        HttpService.Post('[URI_HR]/Att_GetData/GetShiftByWorkDayLateEarly', {
            profileID: Profile.ID,
            dateTime: Vnr_Function.formatDateAPI(DateFromTo.value[0])
        }).then(dataReult => {
            this.showLoading(false);
    ...
    render() {
        ...
        if (
            ...
            PermissionForAppMobile.value['HRM_PortalV3_Att_LateEarly_BtnSaveTemp'] &&
            PermissionForAppMobile.value['HRM_PortalV3_Att_LateEarly_BtnSaveTemp']['View']
        ) {
            listActions.push({
                        ...
                        <SafeAreaView style={styleComonAddOrEdit.wrapInsideModal}>
                            ...
                            <View style={styleComonAddOrEdit.flRowSpaceBetween}>
                                <VnrText
                                    ...
                                    i18nKey={
                                        this.isModify
                                            ? 'HRM_PortalApp_EditLateEarly'
                                            : 'HRM_PortalApp_RegistrationLateEarly'
                                    }
                                />
                            ...
                            <Modal animationType="slide" transparent={true} isVisible={true}>
                                <View style={styleComonAddOrEdit.wrapModalError}>
                                    ...
                                    <View style={styleComonAddOrEdit.wrapContentModalError}>
                                        ...
                                        <ScrollView
                                            style={[styleComonAddOrEdit.wrapLevelError, CustomStyleSheet.paddingHorizontal(8)]}
                                        >
