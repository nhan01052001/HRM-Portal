...
let configList = null,
    enumName = null,
    attApprovedSubmitTakeLateEarlyAllowed = null,
    attSubmitTakeLateEarlyAllowed = null,
    attSubmitTakeLateEarlyAllowedViewDetail = null,
    attApprovedSubmitTakeLateEarlyAllowedKeyTask = null,
    pageSizeList = 20;

class AttApprovedSubmitTakeLateEarlyAllowed extends Component {
    constructor(props) {
        super(props);
    ...
    reload = paramsFilter => {
        ...
        this.setState(_paramsDefault, () => {
            ...
            startTask({
                keyTask: attApprovedSubmitTakeLateEarlyAllowedKeyTask,
                payload: {
                    ...dataBody,
                    keyQuery: keyQuery,
                    isCompare: false,
    ...
    paramsDefault = () => {
        if (!attApprovedSubmitTakeLateEarlyAllowed) {
            attApprovedSubmitTakeLateEarlyAllowed = ScreenName.AttApprovedSubmitTakeLateEarlyAllowed;
        }

        if (!configList[attApprovedSubmitTakeLateEarlyAllowed]) {
            configList[attApprovedSubmitTakeLateEarlyAllowed] = {
                Api: {
        ...
        const _configList = configList[attApprovedSubmitTakeLateEarlyAllowed],
            filter = _configList[enumName.E_Filter],
            dataFromParams = this.checkDataFormNotify();

        const dataRowActionAndSelected = generateRowActionAndSelected(attApprovedSubmitTakeLateEarlyAllowed);
        let _params = {
            ...dataFromParams,
            IsPortalNew: true,
            filter: filter,
    ...
    UNSAFE_componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.reloadScreenName == attApprovedSubmitTakeLateEarlyAllowedKeyTask) {
    ...
    componentDidMount() {
        ...
        attApprovedSubmitTakeLateEarlyAllowedKeyTask = EnumTask.KT_AttApprovedSubmitTakeLateEarlyAllowed;
        AttSubmitTakeLateEarlyAllowedBusinessFunction.checkForReLoadScreen[
            attApprovedSubmitTakeLateEarlyAllowed
        ] = false;
        //set by config
    ...
    render() {
        ...
        return (
            <SafeAreaViewDetail style={styleSafeAreaView.style}>
                {attApprovedSubmitTakeLateEarlyAllowed && attSubmitTakeLateEarlyAllowedViewDetail && enumName && (
                            ...
                            {keyQuery && (
                                <AttTakeLateEarlyAllowedList
                                    detail={{
                                        ...
                                        screenName: attSubmitTakeLateEarlyAllowed,
                                        screenNameRender: attApprovedSubmitTakeLateEarlyAllowed
                                    }}
                                    scrollYAnimatedValue={this.scrollYAnimatedValue}
                                    rowActions={rowActions}
                                    selected={selected}
                                    reloadScreenList={this.reload.bind(this)}
                                    keyDataLocal={attApprovedSubmitTakeLateEarlyAllowedKeyTask}
