...
let configList = null,
    enumName = null,
    attRejectSubmitTakeLateEarlyAllowed = null,
    attSubmitTakeLateEarlyAllowed = null,
    attSubmitTakeLateEarlyAllowedViewDetail = null,
    attRejectSubmitTakeLateEarlyAllowedKeyTask = null,
    pageSizeList = 20;

class AttRejectSubmitTakeLateEarlyAllowed extends Component {
    constructor(props) {
        super(props);
        ...
        this.willFocusScreen = this.props.navigation.addListener('willFocus', () => {
            ...
            if (
                AttSubmitTakeLateEarlyAllowedBusinessFunction.checkForReLoadScreen[attRejectSubmitTakeLateEarlyAllowed]
            ) {
                this.reload();
            }
        });
    ...
    reload = paramsFilter => {
        ...
        AttSubmitTakeLateEarlyAllowedBusinessFunction.checkForReLoadScreen[attRejectSubmitTakeLateEarlyAllowed] = false;

        // Gửi yêu cầu lọc dữ liệu
        this.setState(_paramsDefault, () => {
            const { dataBody, keyQuery } = this.state;
            startTask({
                keyTask: attRejectSubmitTakeLateEarlyAllowedKeyTask,
                payload: {
                    ...dataBody,
                    keyQuery: keyQuery,
                    isCompare: false,
    ...
    paramsDefault = () => {
        if (!attRejectSubmitTakeLateEarlyAllowed) {
            attRejectSubmitTakeLateEarlyAllowed = ScreenName.AttRejectSubmitTakeLateEarlyAllowed;
        }
        if (!configList[attRejectSubmitTakeLateEarlyAllowed]) {
            configList[attRejectSubmitTakeLateEarlyAllowed] = {
                Api: {
        ...
        const _configList = configList[attRejectSubmitTakeLateEarlyAllowed],
            filter = _configList[enumName.E_Filter],
            dataFromParams = this.checkDataFormNotify();

        const dataRowActionAndSelected = generateRowActionAndSelected(attRejectSubmitTakeLateEarlyAllowed);
        let _params = {
            ...dataFromParams,
            IsPortalNew: true,
            filter: filter,
    ...
    UNSAFE_componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.reloadScreenName == attRejectSubmitTakeLateEarlyAllowedKeyTask) {
    ...
    componentDidMount() {
        ...
        attRejectSubmitTakeLateEarlyAllowedKeyTask = EnumTask.KT_AttRejectSubmitTakeLateEarlyAllowed;
        AttSubmitTakeLateEarlyAllowedBusinessFunction.checkForReLoadScreen[attRejectSubmitTakeLateEarlyAllowed] = false;
        //set by config
    ...
    render() {
        ...
        return (
            <SafeAreaViewDetail style={styleSafeAreaView.style}>
                {attRejectSubmitTakeLateEarlyAllowed && attSubmitTakeLateEarlyAllowedViewDetail && enumName && (
                            ...
                            {keyQuery && (
                                <AttTakeLateEarlyAllowedList
                                    detail={{
                                        ...
                                        screenName: attSubmitTakeLateEarlyAllowed,
                                        screenNameRender: attRejectSubmitTakeLateEarlyAllowed
                                    }}
                                    scrollYAnimatedValue={this.scrollYAnimatedValue}
                                    rowActions={rowActions}
                                    selected={selected}
                                    reloadScreenList={this.reload.bind(this)}
                                    keyDataLocal={attRejectSubmitTakeLateEarlyAllowedKeyTask}
