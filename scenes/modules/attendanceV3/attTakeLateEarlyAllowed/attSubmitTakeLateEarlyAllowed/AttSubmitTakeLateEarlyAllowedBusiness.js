...
export const generateRowActionAndSelected = (screenName = attSubmitTakeLateEarlyAllowed) => {
    _rowActions = [];
    _selected = [];
            ...
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    onPress: (item, dataBody) =>
                        AttSubmitTakeLateEarlyAllowedBusinessFunction.businessModifyRecord(
        ...
        if (actionSendMailPer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_SENDMAIL,
                    onPress: (item, dataBody) =>
                        AttSubmitTakeLateEarlyAllowedBusinessFunction.businessSendMailRecords(
                            Array.isArray(item) ? item : [{ ...item }],
                            dataBody
                        ),
                    ...actionSendMail
            ...
            _selected = [
                ..._selected,
                {
                    ...
                    onPress: (items, dataBody) =>
                        AttSubmitTakeLateEarlyAllowedBusinessFunction.businessSendMailRecords(items, dataBody),
                    ...actionSendMail
                }
            ];
        }
        ...
        if (actionDeletePer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_DELETE,
                    onPress: (item, dataBody) =>
                        AttSubmitTakeLateEarlyAllowedBusinessFunction.businessDeleteRecords(
                            Array.isArray(item) ? item : [{ ...item }],
                            dataBody
                        ),
                    ...actionDelete
            ...
            _selected = [
                ..._selected,
                {
                    ...
                    type: E_DELETE,
                    onPress: (item, dataBody) =>
                        AttSubmitTakeLateEarlyAllowedBusinessFunction.businessDeleteRecords(item, dataBody),
                    ...actionDelete
                }
            ];
        }
        ...
        if (actionCancelPer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_CANCEL,
                    onPress: (item, dataBody) =>
                        AttSubmitTakeLateEarlyAllowedBusinessFunction.businessCancelRecords(
                            Array.isArray(item) ? item : [{ ...item }],
                            dataBody
                        ),
                    ...actionCancel
            ...
            _selected = [
                ..._selected,
                {
                    ...
                    type: E_CANCEL,
                    onPress: (item, dataBody) =>
                        AttSubmitTakeLateEarlyAllowedBusinessFunction.businessCancelRecords(item, dataBody),
                    ...actionCancel
                }
            ];
        }
...
export const AttSubmitTakeLateEarlyAllowedBusinessFunction = {
    checkForReLoadScreen: {
        AttSubmitTakeLateEarlyAllowed: false,
        AttSaveTempSubmitTakeLateEarlyAllowed: false,
        AttCanceledSubmitTakeLateEarlyAllowed: false,
        AttApproveSubmitTakeLateEarlyAllowed: false,
    ...
    businessDeleteRecords: (items, dataBody) => {
        if (items.length === 0 && !dataBody) {
            ...
            if (selectedID.length == 0) {
                ...
                AttSubmitTakeLateEarlyAllowedBusinessFunction.confirmDelete({
            ...
            AlertSevice.alert({
                ...
                onConfirm: reason => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        AttSubmitTakeLateEarlyAllowedBusinessFunction.setDelete(objValid);
                    }
                }
            });
        } else {
    ...
    setDelete: objValid => {
        ...
        HttpService.Post('[URI_CENTER]/api/Att_LateEarlyAllowed/DeleteLeaveEarlyAllowed', {
    ...
    businessSendMailRecords: (items, dataBody) => {
        if (items.length === 0 && !dataBody) {
            ...
            if (selectedID.length == 0) {
                ToasterSevice.showWarning('Hrm_DataCannotMailed', 4000);
                return;
            } else {
                AttSubmitTakeLateEarlyAllowedBusinessFunction.setSendMail({
                    strResultID: selectedID,
                    message: ''
                });
            }
    ...
    setSendMail: objValid => {
        ...
        VnrLoadingSevices.show();
        HttpService.Post('[URI_CENTER]/api/Att_LateEarlyAllowed/ProcessingSendMailLateEarlyAllowed', {
            ListRecordID: objValid.strResultID,
            UserLogin: dataVnrStorage.currentUser.headers.userlogin,
            UserProcessID: dataVnrStorage.currentUser.headers.userid,
            Host: _uriPor
    ...
    businessCancelRecords: (items, dataBody) => {
        if (items.length === 0 && !dataBody) {
            ...
            HttpService.MultiRequest([
                HttpService.Post('[URI_CENTER]/api/Att_LateEarlyAllowed/ValidateLateEarlyAllowed', {
                    ListRecordID: selectedID,
                    UserLogin: dataVnrStorage.currentUser.headers.userlogin,
                    UserProcessID: dataVnrStorage.currentUser.headers.userid,
                    Host: _uriPor
                }),
                HttpService.Get('[URI_CENTER]/api/Att_GetData/GetConfigNoteValidate?business=E_LATEEARLYALLOWED')
            ]).then(resAll => {
                VnrLoadingSevices.hide();
                ...
                if (res && res.Status == EnumName.E_SUCCESS) {
                    if (res.Data && res.Data.length > 0) {
                        ...
                        AttSubmitTakeLateEarlyAllowedBusinessFunction.confirmCancel({
                            strResultID: res.Data,
                            message: res.Message //keyTrans.replace('[E_NUMBER]', numberRow)
                        }, isNote);
                    } else {
                        ToasterSevice.showError('HRM_Common_SendRequest_Error', 4000);
                    }
            ...
            AlertSevice.alert({
                ...
                onConfirm: reason => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        AttSubmitTakeLateEarlyAllowedBusinessFunction.setCancel({ ...objValid, Comment: reason });
                    }
                }
            });
        } else {
            AttSubmitTakeLateEarlyAllowedBusinessFunction.setCancel(objValid);
        }
    },
    ...
    businessModifyRecord: (item, dataBody, itemRoot) => {
        ...
        if (itemRoot) {
            if (
                ...
                if (ID) {
                    VnrLoadingSevices.show();
                    HttpService.Get(`[URI_CENTER]/api/Att_LateEarlyAllowed/GetLateEarlyAllowedById?ID=${ID}`)
                        .then(res => {
                            VnrLoadingSevices.hide();
