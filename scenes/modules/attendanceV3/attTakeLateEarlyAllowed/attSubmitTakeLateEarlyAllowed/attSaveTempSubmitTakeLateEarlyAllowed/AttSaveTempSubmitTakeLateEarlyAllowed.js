...
let configList = null,
    enumName = null,
    attSaveTempSubmitTakeLateEarlyAllowed = null,
    attSubmitTakeLateEarlyAllowed = null,
    attSubmitTakeLateEarlyAllowedViewDetail = null,
    attSaveTempSubmitTakeLateEarlyAllowedKeyTask = null,
    pageSizeList = 20;

class AttSaveTempSubmitTakeLateEarlyAllowed extends Component {
    constructor(props) {
        super(props);
    ...
    reload = paramsFilter => {
        ...
        this.setState(_paramsDefault, () => {
            ...
            startTask({
                keyTask: attSaveTempSubmitTakeLateEarlyAllowedKeyTask,
                payload: {
                    ...dataBody,
                    keyQuery: keyQuery,
                    isCompare: false,
    ...
    paramsDefault = () => {
        if (!attSaveTempSubmitTakeLateEarlyAllowed) {
            attSaveTempSubmitTakeLateEarlyAllowed = ScreenName.AttSaveTempSubmitTakeLateEarlyAllowed;
        }
        if (!configList[attSaveTempSubmitTakeLateEarlyAllowed]) {
            configList[attSaveTempSubmitTakeLateEarlyAllowed] = {
                Api: {
        ...
        const _configList = configList[attSaveTempSubmitTakeLateEarlyAllowed],
            filter = _configList[enumName.E_Filter],
            dataFromParams = this.checkDataFormNotify();

        const dataRowActionAndSelected = generateRowActionAndSelected(attSaveTempSubmitTakeLateEarlyAllowed);
        let _params = {
            ...dataFromParams,
            IsPortalNew: true,
            filter: filter,
    ...
    UNSAFE_componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.reloadScreenName == attSaveTempSubmitTakeLateEarlyAllowedKeyTask) {
    ...
    componentDidMount() {
        ...
        attSaveTempSubmitTakeLateEarlyAllowedKeyTask = EnumTask.KT_AttSaveTempSubmitTakeLateEarlyAllowed;
        AttSubmitTakeLateEarlyAllowedBusinessFunction.checkForReLoadScreen[
            attSaveTempSubmitTakeLateEarlyAllowed
        ] = false;
    ...
    render() {
        ...
        return (
            <SafeAreaViewDetail style={styleSafeAreaView.style}>
                {attSaveTempSubmitTakeLateEarlyAllowed && attSubmitTakeLateEarlyAllowedViewDetail && enumName && (
                            ...
                            {keyQuery && (
                                <AttTakeLateEarlyAllowedList
                                    detail={{
                                        ...
                                        screenName: attSubmitTakeLateEarlyAllowed,
                                        screenNameRender: attSaveTempSubmitTakeLateEarlyAllowed
                                    }}
                                    scrollYAnimatedValue={this.scrollYAnimatedValue}
                                    rowActions={rowActions}
                                    selected={selected}
                                    reloadScreenList={this.reload.bind(this)}
                                    keyDataLocal={attSaveTempSubmitTakeLateEarlyAllowedKeyTask}
