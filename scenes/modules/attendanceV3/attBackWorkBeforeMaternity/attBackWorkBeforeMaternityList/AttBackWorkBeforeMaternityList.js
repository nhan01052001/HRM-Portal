...
import AttBackWorkBeforeMaternityListItemApprove from './AttBackWorkBeforeMaternityListItemApprove';
import AttBackWorkBeforeMaternityListItem from './AttBackWorkBeforeMaternityListItem';
...
export default class AttBackWorkBeforeMaternityList extends VnrRenderList {
    remoteData = (param = {}) => {
            ...
            getDataLocal(keyDataLocal)
                .then((resData) => {
                    ...
                    if (res && res !== EnumName.E_EMPTYDATA) {
                        ...
                        if (detail?.screenName === ScreenName.AttApproveBackWorkBeforeMaternity) {
                            data.map((item) => {
                                item.isSelect = false;
                        ...
                        } else if (detail?.screenName == ScreenName.AttApprovedBackWorkBeforeMaternity) {
                            data.map((item) => {
                                item.isSelect = false;
                            ...
                            <View style={styles.styleViewBorderButtom}>
                                <TouchableWithoutFeedback>
                                    <View style={CustomStyleSheet.flex(1)}>
                                        {detail?.screenName === ScreenName.AttApproveBackWorkBeforeMaternity ||
                                        detail.screenName === ScreenName.AttApprovedBackWorkBeforeMaternity ? (
                                                <AttBackWorkBeforeMaternityListItemApprove
                                                    key={index}
                                                ...
                                                <AttBackWorkBeforeMaternityListItem
                                                    key={index}
                                                    currentDetail={detail}
                                                    onClick={() => {
                                                        this.addItemChecked(index, isGroup, indexInDataSource);
    ...
    render() {
        const {
            ...
            permissionBtnCreate =
                ...
                PermissionForAppMobile.value['New_Att_BackWorkBeforeMaternity_New_Index_V2'] &&
                PermissionForAppMobile.value['New_Att_BackWorkBeforeMaternity_New_Index_V2']['Create']
                    ? true
                    : false;
        ...
        const hiddenFiled = {
            BackWorkReasonName: Vnr_Function.checkIsHaveConfigListDetail(detail?.screenName, 'BackWorkReasonName')
        };
        if (dataBody) {
            dataBody = {
                ...dataBody,
        ...
        let isShowBtn =
            permissionBtnCreate &&
            (detail.screenNameRender === ScreenName.AttSubmitBackWorkBeforeMaternity ||
                detail.screenNameRender === ScreenName.AttSaveTempSubmitBackWorkBeforeMaternity) && itemSelected.length === 0;
