...
export default class HreCandidateProfileListItem extends VnrRenderListItem {
                ...
                return (
                    <View
                        style={[
                            styles.lineSatus,
                            {
                                backgroundColor: dataItem?.ColorRGB
                                    ? this.convertTextToColor(dataItem?.ColorRGB)
                                    : Colors.white
                            }
                        ]}
                    >
                        <Text
                            ...
                            style={[
                                ...
                                {
                                    color: dataItem?.Color ? dataItem?.Color : Colors.gray_10
                                }
                            ]}
                        >
                            {dataItem?.TagName ? dataItem?.TagName : ''}
                        </Text>
                    </View>
                );
            }
    ...
    renderProfileView = (dataItem, itemConfig, renderConfig) => {
        ...
        if (
            ...
            typeof itemConfig?.Name === 'string' &&
            itemConfig?.Name.split(',').length > 0
        ) {
                    ...
                    <View style={[styles.wh69]}>
                        ...
                        <View
                            style={[
                                ...
                                CustomStyleSheet.maxWidth('80%')
                            ]}
                        >
                            <Text numberOfLines={2} style={[styleSheets.lable, CustomStyleSheet.fontSize(16)]}>
                                {dataItem?.CandidateName}
                            </Text>
                            {subProfile.length > 0 && (
                                <Text numberOfLines={2} style={styleSheets.text}>
                                    {subProfile.join(' - ')}
                                </Text>
                            )}
                        </View>
                ...
                {itemConfig?.TypeView === 'E_HEADER' && (
                    <View style={[styles.wh79]}>
                        <View style={[CustomStyleSheet.justifyContent('center')]}>
                            {itemConfig?.Name && dataItem[itemConfig?.Name] && (
                                <Text numberOfLines={1} style={[styleSheets.lable, CustomStyleSheet.fontSize(16)]}>
                                    {dataItem[itemConfig?.Name]}
                                </Text>
                            )}
                            {itemConfig?.SubName && dataItem[itemConfig?.SubName] && (
                                <Text numberOfLines={2} style={styleSheets.text}>
                                    {dataItem[itemConfig?.SubName]}
                                </Text>
                            )}
                        </View>
                    </View>
                ...
                <View style={[styles.viewContentTopRight, CustomStyleSheet.justifyContent('flex-start')]}>
                    ...
                    {dataItem?.Percent ? (
                        <View style={[styles.subTopRight, CustomStyleSheet.marginBottom(6)]}>
                            <Text
                                numberOfLines={1}
                                style={[styleSheets.text, styles.lineSatus_text, CustomStyleSheet.textAlign('center')]}
                            >
                                {dataItem?.Percent + '%'}
                            </Text>
                        </View>
    ...
    render() {
        ...
        if (this.props?.value?.props) this.props = this.props?.value?.props;
                                ...
                                <View style={[styles.contentMain]} key={index}>
                                    {_renderConfig.length > 0 &&
                                        _renderConfig.map((item, index) => {
                                            if (
                                                (item?.TypeView === 'E_PROFILE' || item?.TypeView === 'E_HEADER') &&
                                                dataItem
                                            )
                                                return this.renderProfileView(dataItem, item, renderConfig);
                                            ...
                                            else if (item?.TypeView === 'E_COMMON_SPECIAL') // custom
                                                return (
                                                    <View key={index} style={[styles.styItemContent, CustomStyleSheet.borderTopWidth(0)]}>
                                                        <View style={[styles.viewLable, { justifyContent: 'flex-start' }]}>
                                                            <VnrText
                                                                style={[styleSheets.text, { ...styleSheets.text, ...styles.styTextLableInfo }]}
                                                        ...
                                                        </View>
                                                        {
                                                            dataItem[item?.Name] && (
                                                                <View style={styles.styViewValue}>
                                                                    <Text
                                                                        numberOfLines={2}
                                                                        style={[styleSheets.lable, styles.styTextLableInfo, CustomStyleSheet.fontWeight('400')]}
                                                                    >
                                                                        {dataItem[item?.Name]}
                                                                    </Text>
                                                                </View>
                                            ...
                                            return (
                                                <View key={index} style={[CustomStyleSheet.paddingRight(16)]}>
