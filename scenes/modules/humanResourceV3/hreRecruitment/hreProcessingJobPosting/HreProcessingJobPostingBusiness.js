            ...
            _rowActions = [
                {
                    ...
                    onPress: (item, dataBody) =>
                        HreProcessingJobPostingBusiness.businessApproveRecords(
            ...
            _selected = [
                {
                    ...
                    type: E_APPROVE,
                    onPress: (item, dataBody) =>
                        HreProcessingJobPostingBusiness.businessApproveRecords(item, dataBody),
                    ...actionApprove
                }
            ];
        }
        ...
        if (actionRejectPer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_REJECT,
                    onPress: (item, dataBody) =>
                        HreProcessingJobPostingBusiness.businessRejectRecords(
                            Array.isArray(item) ? item : [{ ...item }],
                            dataBody
                        ),
                    ...actionReject
            ...
            _selected = [
                ..._selected,
                {
                    ...
                    type: E_REJECT,
                    onPress: (item, dataBody) =>
                        HreProcessingJobPostingBusiness.businessRejectRecords(item, dataBody),
                    ...actionReject
                }
            ];
        }
        ...
        if (actionRequestChangePer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    onPress: (item, dataBody) =>
                        HreProcessingJobPostingBusiness.businessRequestChangelRecords(
                            Array.isArray(item) ? item : [{ ...item }],
                            dataBody
                        ),
                    ...actionRequestChange
            ...
            _selected = [
                ..._selected,
                {
                    ...
                    onPress: (items, dataBody) =>
                        HreProcessingJobPostingBusiness.businessRequestChangelRecords(items, dataBody),
                    ...actionRequestChange
                }
            ];
        }
...
export const HreProcessingJobPostingBusiness = {
    // Đối tượng checkForReLoadScreen để check có refresh HreWaitProcessingJobPosting không
    checkForReLoadScreen: {
        HreDoneProcessingJobPosting: false,
        HreWaitProcessingJobPosting: false
    },
    ...
    businessApproveRecords: (items, dataBody) => {
        if (items.length === 0 && !dataBody) {
            ...
            if (arrValid.length > 0) {
                ...
                HreProcessingJobPostingBusiness.confirmApprove({
                    ListRecord: arrValid,
                    message,
            ...
            AlertSevice.alert({
                ...
                onConfirm: reason => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        HttpService.Post('[URI_CENTER]/api/Rec_ComposePosting/SetApproveComposePosting', {
                            ...
                            try {
                                if (res) {
                                    if (res.Status === 'SUCCESS') {
                                        ...
                                        // set true để refresh HreDoneProcessingJobPosting
                                        HreProcessingJobPostingBusiness.checkForReLoadScreen[
                                            ScreenName.HreWaitProcessingJobPosting
                                        ] = true;
                                        HreProcessingJobPostingBusiness.checkForReLoadScreen[
                                            ScreenName.HreDoneProcessingJobPosting
                                        ] = true;
                                        ...
                                        if (!_isOnScreenNotification)
                                            DrawerServices.navigate('HreDoneProcessingJobPosting');
                ...
                try {
                    if (res) {
                        if (res.Status === 'SUCCESS') {
                            ...
                            // nếu (duyệt/từ chối) ở màn hình notify thì không chuyển hướng.
                            if (!_isOnScreenNotification) DrawerServices.navigate('HreDoneProcessingJobPosting');
                        } else if (res.Status === 'Locked') {
                            ToasterSevice.showWarning('Hrm_Locked', 5000);
                        } else {
                            ToasterSevice.showWarning(res.Message, 5000);
        ...
        if (items.length === 0 && !dataBody) {
            ...
            if (arrValid.length > 0) {
                let message = translate('HRM_PortalApp_DecriptionReject').replace('[E_NUMBER]',
                    arrValid.length === items.length ? arrValid.length : `${arrValid.length}/${items.length}`);
                HreProcessingJobPostingBusiness.confirmReject({
                    ListRecord: arrValid,
                    message,
                    allItems: Array.isArray(items) ? items.length : 0
                });
            ...
            AlertSevice.alert({
                ...
                onConfirm: reason => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        HttpService.Post('[URI_CENTER]/api/Rec_ComposePosting/SetRejectComposePosting', {
                            ListRecordID: listRecord,
        ...
        if (items.length === 0 && !dataBody) {
            ...
            if (arrValid.length > 0) {
                let message = translate('HRM_PortalApp_HreRecruitment_YouAreRequesting').replace('[E_NUMBER]',
                    arrValid.length === items.length ? arrValid.length : `${arrValid.length}/${items.length}`);
                HreProcessingJobPostingBusiness.confirmRequestChange({
                    ListRecord: arrValid,
                    message,
                    allItems: Array.isArray(items) ? items.length : 0
                });
            ...
            AlertSevice.alert({
                ...
                onConfirm: reason => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        HttpService.Post('[URI_CENTER]/api/Rec_ComposePosting/SetFeedbackComposePostingProcess', {
                            ListRecordID: listRecord,
                            Comment: reason ? reason : null
                        }).then(res => {
                            VnrLoadingSevices.hide();
