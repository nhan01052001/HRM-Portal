...
import { IconCancel, IconCheck } from '../../../../../../constants/Icons';
import { VnrBtnCreate } from '../../../../../../componentsV3/VnrBtnCreate/VnrBtnCreate';
import HreProcessingCandidateApplicationsListItem from './HreProcessingCandidateApplicationsListItem';

const heightActionBottom = 45;
export default class HreProcessingCandidateApplicationsList extends VnrRenderList {
    remoteData = (param = {}) => {
                            ...
                            <View style={styles.styleViewBorderButtom}>
                                <TouchableWithoutFeedback>
                                    <View style={CustomStyleSheet.flex(1)}>
                                        <HreProcessingCandidateApplicationsListItem
    ...
    renderTitleGroup = (data, index, isCheckAll, quantityData) => {
        const { groupField, rowActions } = this.props;

        if (!Array.isArray(groupField) || !groupField.length > 0)
            return <View />;

        let listTileGroup = [];

        groupField.map((item) => data[item?.ValueField] && listTileGroup.push(data[item?.ValueField]));
            ...
            >
                {
                    ...
                    Array.isArray(rowActions) && rowActions.length > 0 && (
                        <View style={[
                            ...
                            isCheckAll &&
                            isCheckAll && stylesScreenDetailV3.checkAll
                        ]}
                        >
                            {isCheckAll && (
                ...
                <View style={CustomStyleSheet.maxWidth('90%')}>
                    <Text style={[styleSheets.lable, styles.textTitleGroup]} numberOfLines={2}>
                        {translate(groupField[0]?.DisplayKey)}: {listTileGroup.join(' - ')} {`(${quantityData})`}
                    </Text>
                </View>
            </TouchableOpacity>
        );
    ...
    render() {
        const {
            ...
            { api, detail, rowActions, onCreate, scrollYAnimatedValue, renderConfig } = this.props,
            permissionBtnCreate = false;
        ...
        if (isLoading) {
            let typeLoading = detail.screenName == ScreenName.HreProcesedProcessingCandidateApplications
                    ...
                    renderItem={({ item, index }) => (
                        <View style={[styles.containBotton]}>
                            ...
                            {item?.isGroup ? (
                                ...
                                >
                                    <View style={[styles.styleViewBorderButtom, CustomStyleSheet.borderBottomWidth(6), CustomStyleSheet.borderBottomColor(Colors.gray_3)]}>
                                        <View style={CustomStyleSheet.flex(1)}>
                                            <HreProcessingCandidateApplicationsListItem
                                                renderConfig={renderConfig}
                                                key={index}
        ...
        let isShowBtn =
            permissionBtnCreate &&
            (detail.screenNameRender === ScreenName.HreProcesedProcessingCandidateApplications ||
                detail.screenNameRender === ScreenName.HrePendingProcessingCandidateApplications);
        return (
            <View
                style={[
