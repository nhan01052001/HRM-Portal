...
import HreJobPostingListItem from './HreJobPostingListItem';

const heightActionBottom = 45;
export default class HreJobPostingList extends VnrRenderList {
    remoteData = (param = {}) => {
                ...
                .then(resData => {
                    ...
                    if (res && res !== EnumName.E_EMPTYDATA) {
                        ...
                        data.map(item => {
                            ...
                            if (item.Status === 'E_EXPIRE_POSTING') {
                                let actions = ['E_REPOST'];
                                if (item.IsShowStopAndExtendBtn) {
                                    actions.push('E_STOP_POSTING', 'E_EXPIRE');
                                }
                                item.BusinessAllowAction = actions.join(',');
                            }

                            if (item.Status === 'E_POSTING') {
                                let actions = ['E_POSTPONE'];
                                if (item.IsShowStopAndExtendBtn) {
                                    actions.push('E_STOP_POSTING');
                                }
                                item.BusinessAllowAction = actions.join(',');
                            }

                            if (item.Status === 'E_STOP_POSTING')
                                item.BusinessAllowAction = 'E_REPOST'; //arr.join(',');
                        });
    ...
    render() {
        ...
        if (isLoading) {
            let typeLoading = detail.screenName == ScreenName.HreProcesedApproveRecruitmentProposal
        ...
        let isShowBtn =
            permissionBtnCreate &&
            (detail.screenNameRender === ScreenName.HreProcesedApproveRecruitmentProposal ||
                detail.screenNameRender === ScreenName.HrePendingApproveRecruitmentProposal);
