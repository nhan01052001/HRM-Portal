            ...
            _rowActions = [
                {
                    ...
                    onPress: (item, dataBody) =>
                        HreProcessingPostingPlanBusiness.businessApproveRecords(
            ...
            _selected = [
                {
                    ...
                    type: E_APPROVE,
                    onPress: (item, dataBody) =>
                        HreProcessingPostingPlanBusiness.businessApproveRecords(item, dataBody),
                    ...actionApprove
                }
            ];
        }
        ...
        if (actionRejectPer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_REJECT,
                    onPress: (item, dataBody) =>
                        HreProcessingPostingPlanBusiness.businessRejectRecords(
                            Array.isArray(item) ? item : [{ ...item }],
                            dataBody
                        ),
                    ...actionReject
            ...
            _selected = [
                ..._selected,
                {
                    ...
                    type: E_REJECT,
                    onPress: (item, dataBody) =>
                        HreProcessingPostingPlanBusiness.businessRejectRecords(item, dataBody),
                    ...actionReject
                }
            ];
        }
        ...
        if (actionRequestChangePer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    onPress: (item, dataBody) =>
                        HreProcessingPostingPlanBusiness.businessRequestChangelRecords(
                            Array.isArray(item) ? item : [{ ...item }],
                            dataBody
                        ),
                    ...actionRequestChange
            ...
            _selected = [
                ..._selected,
                {
                    ...
                    onPress: (items, dataBody) =>
                        HreProcessingPostingPlanBusiness.businessRequestChangelRecords(items, dataBody),
                    ...actionRequestChange
                }
            ];
        }
...
export const HreProcessingPostingPlanBusiness = {
    // Đối tượng checkForReLoadScreen để check có refresh HreWaitProcessingPostingPlan không
    checkForReLoadScreen: {
        HreDoneProcessingPostingPlan: false,
        HreWaitProcessingPostingPlan: false
    },
    ...
    businessApproveRecords: (items, dataBody) => {
        ...
        if (items.length === 0 && !dataBody) {
            ...
            if (arrValid.length > 0) {
                ...
                HreProcessingPostingPlanBusiness.confirmApprove({
                    ListRecord: arrValid,
                    message,
                            ...
                            try {
                                if (res) {
                                    if (res.Status === 'SUCCESS') {
                                        ...
                                        // set true để refresh HreDoneProcessingPostingPlan
                                        HreProcessingPostingPlanBusiness.checkForReLoadScreen[
                ...
                try {
                    if (res) {
                        if (res.Status === 'SUCCESS') {
                            ...
                            // nếu (duyệt/từ chối) ở màn hình notify thì không chuyển hướng.
                            if (!_isOnScreenNotification) DrawerServices.navigate('HreDoneProcessingPostingPlan');
                        } else if (res.Status === 'Locked') {
                            ToasterSevice.showWarning('Hrm_Locked', 5000);
                        } else {
                            ToasterSevice.showWarning(res.Message, 5000);
        ...
        if (items.length === 0 && !dataBody) {
            ...
            if (arrValid.length > 0) {
                let message = translate('HRM_PortalApp_DecriptionReject').replace('[E_NUMBER]',
                    arrValid.length === items.length ? arrValid.length : `${arrValid.length}/${items.length}`);
                HreProcessingPostingPlanBusiness.confirmReject({
                    ListRecord: arrValid,
                    message,
                    allItems: Array.isArray(items) ? items.length : 0
                });
            ...
            AlertSevice.alert({
                ...
                onConfirm: reason => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        HttpService.Post('[URI_CENTER]/api/Rec_JobPostingPlan/SetRejectJobPostingPlan', {
                            Host: apiConfig.uriPor,
                            ListRecordID: listRecord,
                            UserLogin: headers.userlogin,
                            UserProcessID: headers.userid,
        ...
        if (items.length === 0 && !dataBody) {
            ...
            if (arrValid.length > 0) {
                let message = translate('HRM_PortalApp_HreRecruitment_YouAreRequesting').replace('[E_NUMBER]',
                    arrValid.length === items.length ? arrValid.length : `${arrValid.length}/${items.length}`);
                HreProcessingPostingPlanBusiness.confirmRequestChange({
                    ListRecord: arrValid,
                    message,
                    allItems: Array.isArray(items) ? items.length : 0
                });
            ...
            AlertSevice.alert({
                ...
                onConfirm: reason => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        HttpService.Post('[URI_CENTER]/api/Rec_JobPostingPlan/SetFeedbackPostingPlanProcess', {
                            Host: apiConfig.uriPor,
                            ListRecordID: listRecord,
                            UserLogin: headers.userlogin,
                            UserProcessID: headers.userid,
