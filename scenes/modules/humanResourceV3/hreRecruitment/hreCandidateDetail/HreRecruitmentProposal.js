...
import { styleSheets, styleScreenDetail, styleSafeAreaView, CustomStyleSheet, Colors, Size } from '../../../../../constants/styleConfig';
import { ConfigListDetail } from '../../../../../assets/configProject/ConfigListDetail';
import Vnr_Function from '../../../../../utils/Vnr_Function';
import { generateRowActionAndSelected, HrePendingApproveRecruitmentProposalBusiness } from '../hreApproveRecruitmentProposal/hrePendingApproveRecruitmentProposal/HrePendingApproveRecruitmentProposalBusiness';
...
import ViewMovementHistoryInfo from '../hreApproveRecruitmentProposal/ViewMovementHistoryInfo';
...
export default class HreRecruitmentProposal extends Component {
    ...
    getDataItem = async () => {
        try {
            ...
            if (id && Vnr_Services.checkPermissions('New_PortalV3_Rec_CandidateProfileDetail_HiringProposalTab', 'View')) {
                let listConfig = {},
                    listConfigAndData = [];
                ...
                ConfigListDetail.value = {
                    ...
                    'HreApproveRecruitmentProposalViewDetail': [
                        ...
                        },
                        {
                            'TypeView': 'E_COMMON',
                            'Name': 'UnitName',
                            'DisplayKey': 'HRM_PortalApp_HreRecruitment_Unit_Company',
                            'DataType': 'string'
                        },
                        {
                            'TypeView': 'E_COMMON',
                        ...
                        },
                        {
                            'TypeView': 'E_COMMON',
                            'Name': 'SalaryView',
                            'DisplayKey': 'HRM_PortalApp_Earnings',
                            'DataType': 'string'
                        },
                        {
                            'TypeView': 'E_COMMON',
                ...
                ListScreen.map((item) => {
                    ...
                    if (screenDetail && screenData)
                        if (screenData === 'listExtendAttachments' && Array.isArray(response?.Data.listExtendAttachments) && response?.Data.listExtendAttachments.length > 0) {
                            ...
                            listConfigAndData = [
                                ...listConfigAndData,
                                {
                                    ...
                                    data: screenData === 'CandidateInfo' && Array.isArray(response?.Data[screenData]) && response?.Data[screenData].length > 0
                ...
                this.setState({ configListDetail: listConfig, dataItem: dataItem, listActions: _listActions, listData: listConfigAndData });
            } else {
                this.setState({ dataItem: 'EmptyData' });
            }
        } catch (error) {
                        ...
                        <View style={[containerItemDetail, CustomStyleSheet.paddingHorizontal(0)]}>
                            {
                                listData.map((item, index) => {
                                    ...
                                    if (!Array.isArray(config) || !Array.isArray(data) || !data.length > 0)
                                        return null;
                                    ...
                                    if (screenNameConfig === 'HreApproveRecruitmentProposalProcessApproval')
                                        return (
                                            <View style={CustomStyleSheet.paddingHorizontal(styleSheets.p_10)}>
                                                <ItemViewDetail props={{ config, data: { ProcessApproval: data } }} key={index} />
                                            </View>
                                        );
