...
class Discipline extends Component {
    ...
    remoteData = (param = {}) => {
        const { keyQuery } = this.state,
            { isLazyLoading } = param;
        getDataLocal(EnumTask.KT_Discipline)
            .then(resData => {
    ...
    UNSAFE_componentWillReceiveProps(nextProps) {
        const { keyQuery } = this.state;
        if (nextProps.reloadScreenName == EnumTask.KT_Discipline) {
    ...
    reload = () => {
        ...
        this.remoteData();
        startTask({
            keyTask: EnumTask.KT_Discipline,
            payload: {
                keyQuery: EnumName.E_PRIMARY_DATA,
                reload: this.reload,
                isCompare: true
    ...
    //     enumName = EnumName;
    //     screenName = ScreenName.GeneralInfoDiscipline;
    //     screenViewDetail = ScreenName.DisciplineViewDetail;
    //     const _configList = configList && configList['GeneralInfoDiscipline'],
            ...
            contentList = (
                <View style={CustomStyleSheet.flex(1)}>
                    <DisciplineList
                        detail={{
                            dataLocal: false,
                            screenDetail: ScreenName.DisciplineViewDetail,
                            screenName: ScreenName.GeneralInfoDiscipline
                        }}
                    ...
                    renderRow && (
                        <View style={[styleSheets.container]}>
                            <DisciplineList
                                ...
                                api={{
                                    urlApi: `[URI_HR]/Att_GetData/GetListDisciplineProfileID`,
                                    type: "E_POST",
                                    dataBody: {
