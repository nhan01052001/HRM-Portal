...
const enumName = EnumName,
    medAnnualHealth = ScreenName.MedAnnualHealth,
...
export const generateRowActionAndSelected = () => {
    ...
    if (ConfigList.value != null && ConfigList.value != undefined) {
        ...
        const _configList = ConfigList.value[medAnnualHealth],
            ...
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    onPress: (item, dataBody) =>
                        MedAnnualHealthBusinessFunction.businessModifyRecord({ ...item }, dataBody),
                    ...actionEdit
                }
            ];
        }
        ...
        if (actionDeletePer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_DELETE,
                    onPress: (item, dataBody) =>
                        MedAnnualHealthBusinessFunction.businessDeleteRecords([{ ...item }], dataBody),
                    ...actionDelete
                }
            ];
            ...
            _selected = [
                ..._selected,
                {
                    title: translate('HRM_Common_Delete'),
                    type: E_DELETE,
                    onPress: (item, dataBody) => MedAnnualHealthBusinessFunction.businessDeleteRecords(item, dataBody),
                    ...actionDelete
                }
            ];
        }
        ...
        if (actionCancelPer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    onPress: (item, dataBody, isShowMultiRecord) =>
                        MedAnnualHealthBusinessFunction.businessCancelRecords(
                            [{ ...item }],
                            dataBody,
                            isShowMultiRecord
                        ),
            ...
            _selected = [
                ..._selected,
                {
                    ...
                    type: E_CANCEL,
                    onPress: (item, dataBody, isShowMultiRecord) =>
                        MedAnnualHealthBusinessFunction.businessCancelRecords(item, dataBody, isShowMultiRecord),
                    ...actionCancel
                }
            ];
        }
...
export const MedAnnualHealthBusinessFunction = {
    ...
    businessDeleteRecords: (items, dataBody) => {
        ...
        if (items.length === 0 && !dataBody) {
            ...
            MedAnnualHealthBusinessFunction.setIsDelete({
                Ids: selectedID,
                reason: ''
            });
        }
    ...
    setIsDelete: objValid => {
        ...
        HttpService.Post('[URI_HR]/Hre_GetDataV2/RemoveSelectedMedAnnualHealth', {
            selectedIds,
    ...
    businessCancelRecords: (items, dataBody) => {
        if (items.length === 0 && !dataBody) {
            ...
            MedAnnualHealthBusinessFunction.confirmCancel({
            ...
            AlertSevice.alert({
                ...
                onConfirm: reason => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        HttpService.Post('[URI_HR]/Med_GetData/CancelAnnualHealth', {
                            selectedIds: strId
                        })
                            .then(res => {
                                try {
    ...
    businessModifyRecord: item => {
        ...
        VnrLoadingSevices.show();
        HttpService.Get(`[URI_HR]/api/Med_AnnualHealth?ID=${item.ID}`).then(res => {
            VnrLoadingSevices.hide();
            try {
                if (res && res.BusinessAllowAction && res.BusinessAllowAction.indexOf('E_MODIFY') >= 0) {
                    const { reload } = _this;

                    _this.props.navigation.navigate(medAnnualHealthAddOrEdit, { record: res, reload });
                } else {
