...
const configDefault = [
    {
        Name: 'CodeEmp',
        DisplayKey: 'HRM_Medical_ImmunizationRecord_CodeEmp',
        DataType: 'string'
    },
    {
        Name: 'ProfileName',
    ...
    {
        Name: 'StatusView',
        DisplayKey: 'HRM_Medical_ImmunizationRecord_Status',
        DataType: 'string'
    }
];

export default class MedImmunizationViewDetail extends Component {
    constructor(props) {
        super(props);
    ...
    getDataItem = async (isReload = false) => {
        try {
            ...
            if (id) {
                const response = await HttpService.Get(`[URI_HR]/api/Med_ImmunizationRecord?ID=${id}`);
    ...
    reload = (E_KEEP_FILTER, actionIsDelete) => {
        ...
        if (actionIsDelete) {
            DrawerServices.navigate('MedImmunization');
        } else {
    ...
    componentDidMount() {
        MedImmunizationBusinessFunction.setThisForBusiness(this, true);
        this.getDataItem();
    }
