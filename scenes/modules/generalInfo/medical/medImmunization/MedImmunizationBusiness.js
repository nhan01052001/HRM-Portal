...
const enumName = EnumName,
    medImmunization = ScreenName.MedImmunization,
...
export const generateRowActionAndSelected = () => {
    ...
    if (ConfigList.value != null && ConfigList.value != undefined) {
        ...
        const _configList = ConfigList.value[medImmunization],
            businessAction = _configList[enumName.E_BusinessAction],
            { E_MODIFY, E_ResourceName, E_Name, E_Rule, E_DELETE, E_CANCEL } = enumName;
            ...
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    onPress: (item, dataBody) =>
                        MedImmunizationBusinessFunction.businessModifyRecord({ ...item }, dataBody),
                    ...actionEdit
                }
            ];
        }
        ...
        if (actionDeletePer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_DELETE,
                    onPress: (item, dataBody) =>
                        MedImmunizationBusinessFunction.businessDeleteRecords([{ ...item }], dataBody),
                    ...actionDelete
                }
            ];
            ...
            _selected = [
                ..._selected,
                {
                    title: translate('HRM_Common_Delete'),
                    type: E_DELETE,
                    onPress: (item, dataBody) => MedImmunizationBusinessFunction.businessDeleteRecords(item, dataBody),
                    ...actionDelete
                }
            ];
        }
        ...
        if (actionCancelPer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    onPress: (item, dataBody, isShowMultiRecord) =>
                        MedImmunizationBusinessFunction.businessCancelRecords(
                            [{ ...item }],
                            dataBody,
                            isShowMultiRecord
                        ),
            ...
            _selected = [
                ..._selected,
                {
                    ...
                    type: E_CANCEL,
                    onPress: (item, dataBody, isShowMultiRecord) =>
                        MedImmunizationBusinessFunction.businessCancelRecords(item, dataBody, isShowMultiRecord),
                    ...actionCancel
                }
            ];
        }
...
export const MedImmunizationBusinessFunction = {
    ...
    businessDeleteRecords: (items, dataBody) => {
        ...
        if (items.length === 0 && !dataBody) {
            ...
            MedImmunizationBusinessFunction.setIsDelete({
                Ids: selectedID,
                reason: ''
            });
        }
    ...
    setIsDelete: objValid => {
        ...
        HttpService.Post('[URI_HR]/Hre_GetDataV2/RemoveSelectedMedImmunizationRecord', {
            selectedIds,
    ...
    businessCancelRecords: (items, dataBody) => {
        if (items.length === 0 && !dataBody) {
            ...
            MedImmunizationBusinessFunction.confirmCancel({
            ...
            AlertSevice.alert({
                ...
                onConfirm: reason => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        HttpService.Post('[URI_HR]/Med_GetData/CancelImmunizationRecord', {
                            selectedIds: strId
                        })
                            .then(res => {
                                try {
    ...
    businessModifyRecord: item => {
        ...
        HttpService.Get(`[URI_HR]/api/Med_ImmunizationRecord?ID=${item.ID}`).then(res => {
            VnrLoadingSevices.hide();
            try {
                if (res && res.BusinessAllowAction && res.BusinessAllowAction.indexOf('E_MODIFY') >= 0) {
                    const { reload } = _this;
                    console.log(res, 'resresres');
                    _this.props.navigation.navigate(medImmunizationAddOrEdit, { record: res, reload });
                } else {
