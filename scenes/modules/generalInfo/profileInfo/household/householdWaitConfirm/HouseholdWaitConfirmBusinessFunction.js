...
export const generateRowActionAndSelected = () => {
    ...
    if (ConfigList.value != null && ConfigList.value != undefined) {
        const _configList = ConfigList.value[HouseholdWaitConfirm],
            ...
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    onPress: (item, dataBody) =>
                        HouseholdWaitConfirmBusinessFunction.businessModifyRecord({ ...item }, dataBody),
                    ...actionEdit
                }
            ];
        }
        ...
        if (actionSendMailPer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_SENDMAIL,
                    onPress: (item, dataBody) =>
                        HouseholdWaitConfirmBusinessFunction.businessSendMailRecords([{ ...item }], dataBody),
                    ...actionSendMail
                }
            ];
            ...
            _selected = [
                ..._selected,
                {
                    ...
                    onPress: (items, dataBody) =>
                        HouseholdWaitConfirmBusinessFunction.businessSendMailRecords(items, dataBody),
                    ...actionSendMail
                }
            ];
        }
        ...
        if (actionDeletePer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_DELETE,
                    onPress: (item, dataBody) =>
                        HouseholdWaitConfirmBusinessFunction.businessDeleteRecords([{ ...item }], dataBody),
                    ...actionDelete
                }
            ];
        ...
        if (actionAttachfilePer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_ATTACH_FILE,
                    onPress: (item, dataBody) =>
                        HouseholdWaitConfirmBusinessFunction.businessAttachFileRecord(item, dataBody),
                    ...actionDelete
                }
            ];
        }
...
export const HouseholdWaitConfirmBusinessFunction = {
    ...
    businessDeleteRecords: (items, dataBody) => {
        if (items.length === 0 && !dataBody) {
            ...
            HouseholdWaitConfirmBusinessFunction.confirmDelete({
            ...
            AlertSevice.alert({
                ...
                onConfirm: reason => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        HouseholdWaitConfirmBusinessFunction.setDelete(objValid);
                    }
                }
            });
        } else {
    ...
    businessSendMailRecords: (items, dataBody) => {
        if (items.length === 0 && !dataBody) {
            ...
            if (selectedID.length == 0) {
                ...
                HouseholdWaitConfirmBusinessFunction.setSendMail({
                    strResultID: selectedID,
                    message: ''
                });
            }
    ...
    setSendMail: objValid => {
        VnrLoadingSevices.show();
        HttpService.Post('[URI_HR]/Hre_GetData/processTransferHouseHoldCombine', {
            selectedIDs: objValid.strResultID
        }).then(res => {
            VnrLoadingSevices.hide();
            if (res && res == EnumName.E_Success) {
    ...
    businessModifyRecord: item => {
        const { reload } = _this;
        VnrLoadingSevices.show();
        HttpService.Get(`[URI_HR]/Att_GetData/GetByIdHouseHoldTotal?id=${item.ID}`).then(res => {
            VnrLoadingSevices.hide();
            try {
