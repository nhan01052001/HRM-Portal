        ...
        if (actionEditPer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_MODIFY,
                    onPress: (item) => LanguageLevelBusinessFunction.businessModifyRecord(item, screenName),
                    ...actionEdit
                }
            ];
        }
            ...
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    onPress: (item, dataBody) =>
                        LanguageLevelBusinessFunction.businessDeleteRecords([{ ...item }], dataBody, screenName),
                    ...actionDelete
                }
            ];
...
export const LanguageLevelBusinessFunction = {
    checkForLoadEditDelete: {
        [ScreenName.LanguageLevelConfirmed]: false,
        [ScreenName.LanguageLevelEdit]: false,
        [ScreenName.LanguageLevelWaitConfirm]: false
    },
    ...
    businessDeleteRecords: (items, dataBody, screenName) => {
        if (items.length === 0 && !dataBody) {
            ...
            AlertSevice.alert({
                ...
                onCancel: () => {},
                onConfirm: () => {
                    LanguageLevelBusinessFunction.checkingDataWorkingForTaskDelete(selectedID, screenName);
                }
            });
        }
    },
    ...
    checkingDataWorkingForTaskDelete: (selectedID, screenName) => {
        if (Array.isArray(selectedID) && selectedID.length > 0) {
            ...
            if (screenName == ScreenName.LanguageLevelWaitConfirm) {
                apiGetByID = '[URI_HR]/Hre_GetData/DeleteOrRemoveAdd';
            } else if (screenName == ScreenName.LanguageLevelEdit) {
    ...
    businessModifyRecord: (item, screenName) => {
        ...
        if (screenName == ScreenName.LanguageLevelConfirmed) {
            apiGetByID = '[URI_HR]/api/Hre_ProfileLanguage/GetById?ID=';
        } else if (screenName == ScreenName.LanguageLevelWaitConfirm) {
            apiGetByID = '[URI_HR]/Hre_GetData/GetByIdAdd_ProfileLanguageLevel?ID=';
        } else if (screenName == ScreenName.LanguageLevelEdit) {
            apiGetByID = '[URI_HR]//Hre_GetData/GetByIdEdit_ProfileLanguageLevel?ID=';
        }
            ...
            HttpService.Get(apiGetByID + item.ID).then(res => {
                ...
                try {
                    if (
                        ...
                        DrawerServices.navigate('LanguageLevelAddOrEdit', {
                            record: res,
                            reload: reload,
                            screenName: screenName
                        });
