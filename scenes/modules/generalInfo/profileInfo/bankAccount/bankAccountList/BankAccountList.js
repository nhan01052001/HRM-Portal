...
export default class BankAccountList extends React.Component {
    ...
    remoteData = (param = {}) => {
        const { dataSource, page } = this.state,
            { isLazyLoading } = param,
            { keyDataLocal, keyQuery, updateCurentWallet } = this.props;
        if (keyDataLocal) {
            getDataLocal(keyDataLocal)
                .then(resData => {
                    ...
                    if (res && res !== EnumName.E_EMPTYDATA) {
                        ...
                        if (res && res.data2 && Array.isArray(res.data2) && res.data2.length > 0) {
                            data = data.concat(res.data2);
                            _countAccountWallet = res.data2.length;
                        }
                        ...
                        if (page == 1) {
                            this.setState(
                                {
                                    itemSelected: [],
                                    countAccountWallet: _countAccountWallet,
                                    dataSource: data,
                                    totalRow: total,
                                    isLoading: false,
                                    refreshing: false,
                                ...
                                () => {
                                    updateCurentWallet && updateCurentWallet(_countAccountWallet);
                                    setTimeout(() => {
                                        this.endLoading = false;
                                    }, 1000);
                                }
                            ...
                            this.setState(
                                ...
                                () => {
                                    updateCurentWallet && updateCurentWallet(0);
                                    setTimeout(() => {
                                        this.endLoading = false;
                                    }, 1000);
                                }
    ...
    moveToDetail = item => {
        ...
        if (!Vnr_Function.CheckIsNullOrEmpty(rowTouch) && typeof rowTouch == 'function') {
            ...
            let screenDetail = item.EWalletNo ? detail.screenNameWallet : detail.screenDetail;
