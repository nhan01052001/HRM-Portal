        ...
        if (actionEditPer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_MODIFY,
                    onPress: (item) => DependantConfirmedBusinessFunction.businessModifyRecord(item),
                    ...actionEdit
                }
            ];
        }
...
export const DependantConfirmedBusinessFunction = {
    ...
    businessModifyRecord: item => {
        ...
        HttpService.Get(`[URI_HR]/api/Hre_ApprovedDependant/GetById?ID=${item.ID}`).then(res => {
