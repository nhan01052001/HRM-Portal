...
export const generateRowActionAndSelected = () => {
    ...
    if (ConfigList.value != null && ConfigList.value != undefined) {
        const _configList = ConfigList.value[DependantWaitConfirm],
            businessAction = _configList[enumName.E_BusinessAction],
            { E_MODIFY, E_ResourceName, E_Name, E_Rule, E_SENDMAIL, E_DELETE, E_CANCEL, E_ATTACH_FILE } = enumName;
            ...
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    onPress: (item, dataBody) =>
                        DependantWaitConfirmBusinessFunction.businessModifyRecord({ ...item }, dataBody),
                    ...actionEdit
                }
            ];
        }
        ...
        if (actionSendMailPer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_SENDMAIL,
                    onPress: (item, dataBody) =>
                        DependantWaitConfirmBusinessFunction.businessSendMailRecords([{ ...item }], dataBody),
                    ...actionSendMail
                }
            ];
            ...
            _selected = [
                ..._selected,
                {
                    ...
                    onPress: (items, dataBody) =>
                        DependantWaitConfirmBusinessFunction.businessSendMailRecords(items, dataBody),
                    ...actionSendMail
                }
            ];
        }
        ...
        if (actionDeletePer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_DELETE,
                    onPress: (item, dataBody) =>
                        DependantWaitConfirmBusinessFunction.businessDeleteRecords([{ ...item }], dataBody),
                    ...actionDelete
                }
            ];
        ...
        if (actionAttachfilePer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_ATTACH_FILE,
                    onPress: (item, dataBody) =>
                        DependantWaitConfirmBusinessFunction.businessAttachFileRecord(item, dataBody),
                    ...actionDelete
                }
            ];
        }
...
export const DependantWaitConfirmBusinessFunction = {
    ...
    businessDeleteRecords: (items, dataBody) => {
        if (items.length === 0 && !dataBody) {
            ...
            DependantWaitConfirmBusinessFunction.confirmDelete({
            ...
            AlertSevice.alert({
                ...
                onConfirm: reason => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        DependantWaitConfirmBusinessFunction.setDelete(objValid);
                    }
                }
            });
        } else {
    ...
    businessSendMailRecords: (items, dataBody) => {
        if (items.length === 0 && !dataBody) {
            ...
            if (selectedID.length == 0) {
                ...
                DependantWaitConfirmBusinessFunction.setSendMail({
                    strResultID: selectedID,
                    message: ''
                });
            }
    ...
    setSendMail: objValid => {
        ...
        VnrLoadingSevices.show();
        HttpService.Post('[URI_HR]/Hre_GetData/processTransferDependant', {
            selectedIDs: objValid.strResultID
        }).then(res => {
            console.log(res, 'res');
            VnrLoadingSevices.hide();
            if (res && res == EnumName.E_Success) {
                ...
                DrawerServices.navigate('DependantWaitConfirm');
                _this.reload();
            } else if (res && res.Message && typeof res.Message == 'string') {
                ToasterSevice.showError(res.Message, 4000);
            } else if (res && typeof res == 'string' && res.toLowerCase() === 'Fail'.toLowerCase()) {
                ToasterSevice.showWarning('HRM_Hre_Relative_Alert_DataAlreadyTransfer', 4000);
            } else if (res && typeof res == 'string' && res.toLowerCase() !== 'Fail'.toLowerCase()) {
                ...
                ToasterSevice.showWarning('HRM_Common_SendRequest_Error', 4000);
            }
        });
    },
    //#endregion
    ...
    businessModifyRecord: item => {
        const { reload } = _this;
        VnrLoadingSevices.show();
        HttpService.Get(`[URI_HR]/Hre_GetData/GetByIdTotal_DependantWaitToTransfer?ID=${item.ID}`).then(res => {
            VnrLoadingSevices.hide();
            try {
    ...
    businessSaveAttachFile: params => {
        VnrLoadingSevices.show();
        HttpService.Post('[URI_HR]/Hre_GetDataV2/saveAttachRequestInfoDependants', params).then(res => {
            VnrLoadingSevices.hide();
            if (res && res == EnumName.E_Success) {
                ToasterSevice.showSuccess('Hrm_Succeed', 4000);
