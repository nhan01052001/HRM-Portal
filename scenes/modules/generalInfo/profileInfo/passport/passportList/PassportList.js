...
export default class PassportList extends React.Component {
    ...
    remoteData = (param = {}) => {
        const { dataSource, page } = this.state,
            { isLazyLoading } = param,
            { keyDataLocal, keyQuery, detail } = this.props;
        if (keyDataLocal) {
            getDataLocal(keyDataLocal)
                .then(resData => {
                    ...
                    if (res && res !== EnumName.E_EMPTYDATA) {
                        ...
                        data.map(item => {
                            item.isSelect = false;
                            if (detail.screenName === 'PassportConfirmed') {
                                item.BusinessAllowAction = 'E_MODIFY';
                            } else if (detail.screenName === ScreenName.PassportWaitConfirm) {
                                item.BusinessAllowAction =
                                    item?.Status === 'E_SUBMIT_TEMP' ? 'E_MODIFY, E_DELETE, E_SENDMAIL' : null;
                                // hashcode because status just have E_SUBMIT && E_SUBMIT_TEMP
                                item.itemStatus = {
                                    bgStatus: item.Status === 'E_SUBMIT' ? '255,153,0,0.04' : null,
                                    borderStatus: item.Status === 'E_SUBMIT' ? '255,153,0,0.5' : null,
                                    colorStatus: item.Status === 'E_SUBMIT' ? '255,153,0,1' : null
                                };
                            }
                        });
                    ...
                    style={
                        {
                            ...
                            // paddingTop: !this.isHaveFilter ? Size.defineSpace : 0
                        }
                    }
