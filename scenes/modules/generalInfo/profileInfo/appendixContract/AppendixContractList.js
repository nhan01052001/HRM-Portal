...
import AppendixContractItem from './AppendixContractItem';
...
export default class AppendixContractList extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            itemSelected: [],
    ...
    remoteData = () => {
        ...
        HttpService.Post(api.urlApi, api.dataBody, null, this.reload).then(res => {
            try {
                let data = [],
                    total = 0;
                ...
                if (Object.hasOwnProperty.call(res, 'total')) {
                    total = res.total;
                } else if (Object.hasOwnProperty.call(res, 'Total')) {
                    total = res.Total;
                }
    ...
    _renderLoading = () => {
        return (
            <View style={stylesScreenDetailV2.loadingFooter}>
    ...
    openAction = index => {
        ...
        // Keyboard.dismiss();
        if (!isOpenAction && Array.isArray(selected) && selected.length > 0) {
