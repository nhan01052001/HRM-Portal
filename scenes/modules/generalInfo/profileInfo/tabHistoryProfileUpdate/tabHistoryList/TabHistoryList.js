...
import TabHistoryListItem from './TabHistoryListItem';
...
export default class TabHistoryList extends React.Component {
    constructor(props) {
        super(props);
    ...
    remoteData = (param = {}) => {
        const { dataSource, page } = this.state,
            { isLazyLoading } = param,
            { api, dataLocal, keyDataLocal, keyQuery, detail } = this.props;
        if (keyDataLocal) {
            
            if (detail.screenName == ScreenName.TopTabHisProfileBasicInfo) {
                this.setState({
                    itemSelected: [],
            ...
            getDataLocal(keyDataLocal)
                .then(resData => {
                    ...
                    if (res && res !== EnumName.E_EMPTYDATA) {
                        ...
                        data.map(item => {
                            item.isSelect = false;
                            // if (item.Status == EnumName.E_SUBMIT_TEMP)
                            //     item.BusinessAllowAction = 'E_DELETE,E_SENDMAIL,E_ATTACH_FILE';
                        });
    ...
    addItemChecked = indexItem => {
        ...
        if (itemChecked && itemChecked.Status == EnumStatus.E_SUBMIT_TEMP) {
