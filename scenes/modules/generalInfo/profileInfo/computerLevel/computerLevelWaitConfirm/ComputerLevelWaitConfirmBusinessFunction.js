...
let enumName = EnumName,
    QualificationWaitConfirm = ScreenName.ComputerLevelWaitConfirm;
            ...
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    onPress: (item, dataBody) =>
                        ComputerLevelWaitConfirmBusinessFunction.businessModifyRecord({ ...item }, dataBody),
                    ...actionEdit
                }
            ];
        }
        ...
        if (actionSendMailPer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_SENDMAIL,
                    onPress: (item, dataBody) =>
                        ComputerLevelWaitConfirmBusinessFunction.businessSendMailRecords([{ ...item }], dataBody),
                    ...actionSendMail
                }
            ];
            ...
            _selected = [
                ..._selected,
                {
                    ...
                    onPress: (items, dataBody) =>
                        ComputerLevelWaitConfirmBusinessFunction.businessSendMailRecords(items, dataBody),
                    ...actionSendMail
                }
            ];
        }
        ...
        if (actionDeletePer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_DELETE,
                    onPress: (item, dataBody) =>
                        ComputerLevelWaitConfirmBusinessFunction.businessDeleteRecords([{ ...item }], dataBody),
                    ...actionDelete
                }
            ];
...
export const ComputerLevelWaitConfirmBusinessFunction = {
    ...
    businessDeleteRecords: (items, dataBody) => {
        if (items.length === 0 && !dataBody) {
            ...
            ComputerLevelWaitConfirmBusinessFunction.confirmDelete({
            ...
            AlertSevice.alert({
                ...
                onConfirm: reason => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        ComputerLevelWaitConfirmBusinessFunction.setDelete(objValid);
                    }
                }
            });
        } else {
    ...
    businessSendMailRecords: (items, dataBody) => {
        if (items.length === 0 && !dataBody) {
            ...
            if (selectedID.length == 0) {
                ...
                ComputerLevelWaitConfirmBusinessFunction.setSendMail({
                    strResultID: selectedID,
                    message: ''
                });
            }
    ...
    businessModifyRecord: item => {
        const { reload } = _this;
        VnrLoadingSevices.show();
        HttpService.Get(`[URI_HR]/Hre_GetData/GetById_ComputerLevel?ID=${item.ID}`).then(res => {
            VnrLoadingSevices.hide();
            try {
                if (res && Object.keys(res).length > 0) {
                    DrawerServices.navigate('ComputerLevelAddOrEdit', {
                        record: res,
                        reload: reload,
                        screenName: 'ComputerLevelAddOrEdit'
                    });
                } else {
