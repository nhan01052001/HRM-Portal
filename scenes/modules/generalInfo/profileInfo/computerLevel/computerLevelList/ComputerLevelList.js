...
import ComputerLevelListItem from './ComputerLevelListItem';
...
export default class ComputerLevelList extends React.Component {
    constructor(props) {
        super(props);
            ...
            getDataLocal(keyDataLocal)
                .then(resData => {
                    ...
                    if (res && res !== EnumName.E_EMPTYDATA) {
                        ...
                        data.forEach(item => {
                            item.isSelect = false;
                            if (detail.screenName === ScreenName.ComputerLevelConfirmed) {
                                item.BusinessAllowAction = 'E_MODIFY';
                            } else if (detail.screenName === ScreenName.ComputerLevelWaitConfirm) {
                                item.BusinessAllowAction =
                                    item?.Status === 'E_SUBMIT_TEMP'
                                        ? 'E_MODIFY, E_DELETE, E_SENDMAIL'
                                        : item.BusinessAllowAction;
                            }
                        });
