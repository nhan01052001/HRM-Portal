            ...
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    onPress: (item, dataBody) =>
                        RelativeBusinessFunction.businessSendMailRecords([{ ...item }], dataBody),
                    ...actionSendMail
                }
            ];
            ...
            _selected = [
                ..._selected,
                {
                    ...
                    onPress: (items, dataBody) =>
                        RelativeBusinessFunction.businessSendMailRecords(items, dataBody),
                    ...actionSendMail
                }
            ];
        }
        ...
        if (actionEditPer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_MODIFY,
                    onPress: (item) => RelativeBusinessFunction.businessModifyRecord(item, screenName),
                    ...actionEdit
                }
            ];
        }
        ...
        if (actionDeletePer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_DELETE,
                    onPress: (item, dataBody) =>
                        RelativeBusinessFunction.businessDeleteRecords([{ ...item }], dataBody, screenName),
                    ...actionDelete
                }
            ];
...
export const RelativeBusinessFunction = {
    ...
    businessDeleteRecords: (items, dataBody, screenName) => {
        if (items.length === 0 && !dataBody) {
            ...
            AlertSevice.alert({
                ...
                onCancel: () => {},
                onConfirm: () => {
                    RelativeBusinessFunction.checkingDataWorkingForTaskDelete(selectedID, screenName);
                }
            });
        }
    },
        ...
        HttpService.Post('[URI_HR]/api/Hre_ApprovedProfileQualification', params).then(data => {
            if (
                ...
                if (isModify && screenName == ScreenName.RelativeConfirmed) {
                    RelativeBusinessFunction.checkForLoadEditDelete[ScreenName.RelativeEdit] = true;
                    // if(screenName == ScreenName.Quan)
                } else {
                    // Nếu tạo mới thì reload list Chờ xác nhận
                    RelativeBusinessFunction.checkForLoadEditDelete[ScreenName.RelativeWaitConfirm] = true;
                }
