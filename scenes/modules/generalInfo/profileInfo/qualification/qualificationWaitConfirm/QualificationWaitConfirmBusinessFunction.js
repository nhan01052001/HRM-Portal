...
export const generateRowActionAndSelected = () => {
    ...
    if (ConfigList.value != null && ConfigList.value != undefined) {
        const _configList = ConfigList.value[QualificationWaitConfirm],
            ...
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    onPress: (item, dataBody) =>
                        QualificationWaitConfirmBusinessFunction.businessModifyRecord({ ...item }, dataBody),
                    ...actionEdit
                }
            ];
        }
        ...
        if (actionSendMailPer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_SENDMAIL,
                    onPress: (item, dataBody) =>
                        QualificationWaitConfirmBusinessFunction.businessSendMailRecords([{ ...item }], dataBody),
                    ...actionSendMail
                }
            ];
            ...
            _selected = [
                ..._selected,
                {
                    ...
                    onPress: (items, dataBody) =>
                        QualificationWaitConfirmBusinessFunction.businessSendMailRecords(items, dataBody),
                    ...actionSendMail
                }
            ];
        }
        ...
        if (actionDeletePer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_DELETE,
                    onPress: (item, dataBody) =>
                        QualificationWaitConfirmBusinessFunction.businessDeleteRecords([{ ...item }], dataBody),
                    ...actionDelete
                }
            ];
        ...
        if (actionAttachfilePer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_ATTACH_FILE,
                    onPress: (item, dataBody) =>
                        QualificationWaitConfirmBusinessFunction.businessAttachFileRecord(item, dataBody),
                    ...actionDelete
                }
            ];
        }
...
export const QualificationWaitConfirmBusinessFunction = {
    ...
    businessDeleteRecords: (items, dataBody) => {
        if (items.length === 0 && !dataBody) {
            ...
            QualificationWaitConfirmBusinessFunction.confirmDelete({
            ...
            AlertSevice.alert({
                ...
                onConfirm: reason => {
                    if (isValidInputText && (!reason || reason === '')) {
                        ...
                        QualificationWaitConfirmBusinessFunction.setDelete(objValid);
                    }
                }
            });
        } else {
    ...
    businessSendMailRecords: (items, dataBody) => {
        if (items.length === 0 && !dataBody) {
            ...
            if (selectedID.length == 0) {
                ...
                QualificationWaitConfirmBusinessFunction.setSendMail({
                    strResultID: selectedID,
                    message: ''
                });
            }
    ...
    setSendMail: objValid => {
        ...
        HttpService.Post('[URI_HR]/Hre_GetData/processTransferAllGrids', {
            ...
            if (res && res == EnumName.E_Success) {
                ...
                DrawerServices.navigate('QualificationWaitConfirm');
                _this.reload();
            } else if (res && res.Message && typeof res.Message == 'string') {
                ToasterSevice.showError(res.Message, 4000);
            } else if (res && typeof res == 'string') {
    ...
    businessModifyRecord: item => {
        const { reload } = _this;
        VnrLoadingSevices.show();
        HttpService.Get(`[URI_HR]/Hre_GetData/GetById_ProfileQualification?ID=${item.ID}`).then(res => {
            VnrLoadingSevices.hide();
            try {
                if (res && Object.keys(res).length > 0) {
                    DrawerServices.navigate('QualificationAddOrEdit', {
                        record: res,
                        reload: reload,
                        screenName: 'QualificationAddOrEdit'
                    });
                } else {
