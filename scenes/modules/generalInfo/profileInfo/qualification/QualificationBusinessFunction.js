        ...
        if (actionEditPer) {
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    type: E_MODIFY,
                    onPress: (item) => QualificationBusinessFunction.businessModifyRecord(item, screenName),
                    ...actionEdit
                }
            ];
        }
            ...
            _rowActions = [
                ..._rowActions,
                {
                    ...
                    onPress: (item, dataBody) =>
                        QualificationBusinessFunction.businessDeleteRecords([{ ...item }], dataBody, screenName),
                    ...actionDelete
                }
            ];
...
export const QualificationBusinessFunction = {
    checkForLoadEditDelete: {
        [ScreenName.QualificationConfirmed]: false,
        [ScreenName.QualificationEdit]: false,
        [ScreenName.QualificationWaitConfirm]: false
    },
    ...
    businessDeleteRecords: (items, dataBody, screenName) => {
        if (items.length === 0 && !dataBody) {
            ...
            AlertSevice.alert({
                ...
                onCancel: () => {},
                onConfirm: () => {
                    QualificationBusinessFunction.checkingDataWorkingForTaskDelete(selectedID, screenName);
                }
            });
        }
    },
    checkingDataWorkingForTaskDelete: (selectedID, screenName) => {
        if (Array.isArray(selectedID) && selectedID.length > 0) {
            ...
            if (screenName == ScreenName.QualificationWaitConfirm) {
                apiGetByID = '[URI_HR]/Hre_GetData/DeleteOrRemoveAdd';
            } else if (screenName == ScreenName.QualificationEdit) {
    ...
    businessModifyRecord: (item, screenName) => {
        ...
        if (screenName == ScreenName.QualificationWaitConfirm) {
            apiGetByID = '[URI_HR]/Hre_GetData/GetByIdAdd_ProfileQualification?ID=';
        } else if (screenName == ScreenName.QualificationConfirmed) {
            apiGetByID = '[URI_HR]/api/Hre_ApprovedProfileQualification/GetById?ID=';
        } else if (screenName == ScreenName.QualificationEdit) {
            apiGetByID = '[URI_HR]/Hre_GetData/GetByIdEdit_ProfileQualification?ID=';
        }
            ...
            HttpService.Get(apiGetByID + item.ID).then(res => {
                ...
                try {
                    if (
                        ...
                        DrawerServices.navigate('QualificationAddOrEdit', {
                            record: res,
                            reload: reload,
                            screenName: screenName
                        });
    ...
    businessCreateOrEditRecord: (params, isModify, screenName) => {
        ...
        HttpService.Post('[URI_HR]/api/Hre_ApprovedProfileQualification', params).then(data => {
            if (
                ...
                if (isModify && screenName == ScreenName.QualificationConfirmed) {
                    QualificationBusinessFunction.checkForLoadEditDelete[ScreenName.QualificationEdit] = true;
                    // if(screenName == ScreenName.Quan)
                } else {
                    // Nếu tạo mới thì reload list Chờ xác nhận
                    QualificationBusinessFunction.checkForLoadEditDelete[ScreenName.QualificationWaitConfirm] = true;
                }
