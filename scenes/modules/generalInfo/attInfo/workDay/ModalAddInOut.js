...
export default class ModalAddInOut extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            textWarning: null,
            // nhan.nguyen/ feat for task 0160511 ECOPARK
            FileAttachment: {
                ...
                if (dataItem['Type'] && dataItem['Type'] == 'E_GROUP') {
                    viewContent = (
                        <View style={[styleViewTitleGroup, styles.styViewTitleGroup]}>
                            <VnrText
                                style={[styleSheets.text, styles.styTextTitleGroupError]}
                        ...
                        if (data.ErrorRespone.IsBlock == true) {
                            if (data.ErrorRespone.IsShowRemoveAndContinue) {
                                this.AlertSevice.alert({
                                    ...
                                    //lưu và tiếp tục
                                    colorSecondConfirm: Colors.Secondary,
    ...
    getTypeInOut = (TimeLogTimeFromRoster) => {
        ...
        let FileAttachmentVisibleConfig = _configField.findIndex((key) => key == 'FileAttachment') > -1 ? false : true;

        HttpService.Get('[URI_SYS]/Sys_GetData/GetEnum?text=TamScanLogType').then((res) => {
            try {
                let { Type, MissInOutReason, FileAttachment } = this.state,
                    data = [];
                if (this.isModify) {
                ...
                this.setState(
                    {
                        ...
                        FileAttachment: {
                            ...FileAttachment,
                            visibleConfig: FileAttachmentVisibleConfig
                        }
                    },
                    () => {
                        const { workDayItem } = this.props;
                        if (workDayItem) {
                            ...
                            this.setState({
                                TimeLog: {
                                    ...TimeLog,
                                    value: TimeLogTimeFromRoster
                                        ? moment(TimeLogTimeFromRoster).format('YYYY-MM-DD HH:mm:ss')
    ...
    getTimeFromRoster = () => {
        const { TimeLog, TimeLogTime, Type } = this.state;
        let { workDayItem } = this.props,
            _id = workDayItem.ID,
                                        ...
                                        <View style={styles.groupTitle}>
                                            <View style={styles.titleModal}>
                                                <VnrText
                                                    ...
                                                    i18nKey={'HRM_Att_TamScanLogRegister_List'}
                                                />
                                            </View>
                                        </View>
                                    </View>
                                        ...
                                        <View style={styles.styleViewContentModal}>
                                            ...
                                            {MissInOutReason.visibleConfig && (
                                                <View style={styles.viewReason}>
                                                    <VnrPicker
                                                        stylePlaceholder={styles.styViewPlaceholderPicker}
                                            ...
                                            <View style={styles.viewCommentUpdateInOut}>
                                                <View style={styles.bntShowNotes}>
                                                    <IconEdit color={Colors.Secondary} size={Size.iconSize} />
                                                </View>
                                                ...
                                                >
                                                    <View style={stylesListPickerControl.viewLable}>
                                                        <VnrText
                                                            style={[
                                                                styleSheets.text,
                                                                stylesListPickerControl.textLableInfo
                                                            ]}
                                                            i18nKey={'HRM_Rec_JobVacancy_FileAttachment'}
                                                        />
                                                    </View>
                                                    <View style={stylesListPickerControl.viewControl}>
                                                        <VnrAttachFile
...
const styles = StyleSheet.create({
    ...
    styViewPlaceholderPicker: { color: Colors.grey, opacity: 0.8 },
    styViewfileAttach: { paddingHorizontal: 5, marginTop: 12 },
    styViewPickerQuickly: { position: 'absolute', bottom: -200 },
    styTextTitleGroupError: { fontWeight: '500', color: Colors.Secondary },
    styViewTitleGroup: { marginHorizontal: 0, paddingBottom: 5, marginBottom: 10, marginTop: 0 },
    textError: {
        color: Colors.orange,
        fontSize: Size.text
    },
