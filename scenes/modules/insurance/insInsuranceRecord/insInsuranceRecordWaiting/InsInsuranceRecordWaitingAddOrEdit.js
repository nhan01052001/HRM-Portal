...
export default class InsInsuranceRecordWaitingAddOrEdit extends Component {
    constructor(props) {
        super(props);
    ...
    handleSetState = (item, configRealOnly) => {
        ...
        let nextState = {},
            ShowHideFields = item.ShowHideFields ? item.ShowHideFields.HRM9Default : null;
        ...
        nextState = {
            ...
            ParentID: {
                ...ParentID,
                value: [{ Code: item.InsuranceType, Name: translate(item.InsuranceType), id: item.InsuranceTypeID }],
                refresh: !ParentID.refresh
            },
            // InsuranceType: {
            //   ...InsuranceType,
            ...
            InsuranceCodeV2: {
                ...InsuranceCodeV2,
                value: item.InsuranceCodeV2 ? item.InsuranceCodeV2 : null,
                refresh: !InsuranceCodeV2.refresh
            },
            FileAttachment: {
                ...FileAttachment,
            ...
            strSettlement: {
                ...strSettlement,
                value: item.strSettlement ? { Text: item.strSettlementTime, Value: item.strSettlement } : null,
                refresh: !strSettlement.refresh
            },
            AdditionalMonthYearSettlement: {
                ...AdditionalMonthYearSettlement,
            ...
            AdditionalSettlementString: {
                ...
                value: item.AdditionalSettlementString
                    ? { Text: item.AdditionalSettlementString, Value: item.AdditionalSettlement }
                    : null,
                refresh: !AdditionalSettlementString.refresh
            },
            SeriNo: {
    ...
    initHandleTreeView = (isUpdate = false) => {
    ...
    changeProfileV2 = (isUpdate = false) => {
        const { InsuranceType, ProfileID, IDNoV2, AccountNumber, InsuranceCodeV2, DateOfBirth } = this.state;
    ...
    fnChangeWife = () => {
        ...
        if (WifeID.value && WifeID.value.ID) {
            HttpService.Post('[URI_HR]/Ins_GetData/getInfoByWifeID', { relativeid: WifeID.value.ID }).then((data) => {
                let nextState = {};
    ...
    onSave = (navigation, isCreate, isSend) => {
        ...
        HttpService.Post('[URI_HR]/api/Ins_InsuranceRecordV2_Waiting', param).then((data) => {
            VnrLoadingSevices.hide();
            ...
            if (data) {
                if (data.ErrorRespone) {
                    ...
                    //navigation.goBack();
                    DrawerServices.navigate('InsInsuranceRecordWaiting');
        ...
        if (data) {
            ...
            return dataSourceError.map((dataItem, index) => {
                ...
                if (dataItem['Type'] && dataItem['Type'] == 'E_GROUP') {
                    viewContent = (
                        <View style={[styleViewTitleGroup, styles.styViewTitleGroupError]}>
                            <VnrText
                                style={[styleSheets.text, styles.styTextTitleGroupExtend]}
            ...
            HttpService.Post('[URI_HR]/Ins_GetData/getWeeklyDaysOff', {
                ...
                if (data) {
                    if (data == 'NotExisted' || data == 'Error') {
                        ...
                        let a = data.split(','),
                            arrValue = a.map((item) => {
                                return { Value: item, Text: translate('DaysOFWeeks__' + item) };
                            });
...
const styles = StyleSheet.create({
    styTextTitleGroupExtend: { fontWeight: '500', color: Colors.primary },
    styViewTitleGroupError: { marginHorizontal: 0, paddingBottom: 5, marginBottom: 10 },
