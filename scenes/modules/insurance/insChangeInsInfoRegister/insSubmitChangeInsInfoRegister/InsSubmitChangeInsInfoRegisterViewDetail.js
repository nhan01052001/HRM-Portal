...
export default class InsSubmitChangeInsInfoRegisterViewDetail extends Component {
    constructor(props) {
        super(props);
    ...
    getDataItem = async (isReload = false) => {
        try {
            ...
            if (!Vnr_Function.CheckIsNullOrEmpty(dataId) || isReload) {
                const dataBody = { id: !Vnr_Function.CheckIsNullOrEmpty(dataId) ? dataId : dataItem.ID };
                const response = await HttpService.Get('[URI_HR]/api/Ins_ChangeInsInfoRegister/' + dataBody.id);
